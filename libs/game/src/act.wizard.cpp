...
#include "dbat/game/const/ExitFlag.hpp"
...
static void do_stat_room(Character *ch, Room *rm)
{
    ...
    for (auto &[d, ex] : rm->getDirections())
    {
        ...
        ch->send_to("Exit @c%-5s@n:  To: [%s], Key: [%5d], Keywrd: %s, Type: %s\r\n%s", dirs[i], buf1, ex.key == NOTHING ? -1 : ex.key, !ex.keyword.empty() ? ex.keyword : "None", ex.exit_flags.getFlagNames(), !ex.general_description.empty() ? ex.general_description : "  No exit description.\r\n");
    }
