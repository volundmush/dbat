  ...
  for (i = 0; i <= top_of_mobt; i++) {
    if (mob_index[i].vnum >= bottom && mob_index[i].vnum <= top) {
      ...
      send_to_char(ch, "@g%4d@n) [@g%-5d@n] @[3]%-*s @C%-9s @c%-9s @y[%4d]@n %s\r\n",
                   counter, mob_index[i].vnum, count_color_chars(mob_proto[i].short_descr)+30, mob_proto[i].short_descr, TRUE_RACE((&mob_proto[i])), get_sensei(mob_proto[i].chclass)->getName().c_str(),
