...
namespace atk
{
    ...
    bool Attack::checkSkills()
    {
        ...
        if (needed == minimumSkillRequired.end())
            return true;
    ...
    bool Attack::getOpponent()
    {
        ...
        {
            if (!tech_handle_targeting(user, (char *)args[0].c_str(), &victim, &obj))
                return false;
        }
        return true;
    }
    ...
    DefenseResult Attack::calculateDefense()
    {
        ...
        if (canParry() && calculateDeflect())
            return DefenseResult::Parried;
        if (canDodge() && dodgeChance > overcomeDodge)
            return DefenseResult::Dodged;
        if (canBlock() && blockChance > overcomeBlock)
    ...
    {
        if (!can_kill(user, victim, nullptr, canKillType()))
            return Result::Canceled;
        ...
        if (result == DefenseResult::Parried)
            return handleParry();
        if (result == DefenseResult::Dodged)
            return handleDodge();
        if (result == DefenseResult::Perfect_Dodged)
            return handlePerfectDodge();
        if (result == DefenseResult::Failed)
            return handleCleanHit();
        if (result == DefenseResult::Missed)
            return handleMiss();
    ...
    Result Attack::handleOtherHit()
    {
        ...
        if (victim->getCurVital(CharVital::stamina) > 0)
        {
            if (canParry() && calculateDeflect())
                return handleParry();
            if (canDodge() && currentDodgeCheck > axion_dice(10))
                return handleDodge();
            if (canBlock() && currentBlockCheck > axion_dice(10))
                return handleBlock();
            return handleMiss();
        }
        else
    ...
    {
        if (!can_kill(user, nullptr, obj, canKillType()))
            return Result::Canceled;
    ...
    void Attack::onLanded()
    {
        if (victim)
        {
            ...
            {
                if (isPhysical())
                    tech_handle_fireshield(user, victim, getBodyPart().c_str());
                if (canCombo())
                    handle_multihit(user, victim);
            }
            attackPostprocess();
        }
        else if (obj)
    ...
    void Attack::onLandedOrMissed()
    {
        ...
        if (auto hlimb = hurtInjuredLimbs(); hlimb)
        {
            ...
            auto checkLimb = [&](int l)
            { return GET_LIMBCOND(user, l) > 0 && GET_LIMBCOND(user, l) < 50; };
    ...
    {
        if (IS_NAIL(user) && GET_SKILL_BASE(user, SKILL_STYLE) >= 75)
            return 5;
        return 7;
    }
    ...
    void PsychicBlast::attackPostprocess()
    {
        ...
        if (!AFF_FLAGGED(victim, AFF_SHOCKED) && Random::get<int>(1, 4) == 4 && !AFF_FLAGGED(victim, AFF_SANCTUARY))
        {
            ...
            victim->affect_flags.set(AFF_SHOCKED, true);
        }
    }
    ...
    void Honoo::attackPreprocess()
    {
        ...
        victim->affect_flags.set(AFF_ASHED, true);
    }
        ...
        if (victim)
        {
            if (GET_HIT(victim) > 1)
            {
                if (Random::get<int>(1, 4) == 1 && !AFF_FLAGGED(victim, AFF_FROZEN) && !IS_DEMON(victim))
                {
                    ...
                    victim->affect_flags.set(AFF_FROZEN, true);
                }
            }
        }
    }
    ...
    void StarBreaker::attackPreprocess()
    {
        theft = 0;
        taken = 0;
        auto userInt = GET_INT(user);
        auto vInt = GET_INT(victim);
        auto vExp = GET_EXP(victim);
        if (userInt - 30 > userInt)
        {
            theft = 1;
        }
        else if (userInt - 20 > vInt)
        {
            theft = vExp / 1000;
        }
        else if (userInt - 10 > vInt)
        {
            theft = vExp / 100;
        }
        else if (userInt >= vInt)
        {
            theft = vExp / 50;
        }
        else if (userInt + 10 >= vInt)
