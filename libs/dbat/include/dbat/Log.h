...
template <typename... Args>
inline void log_runtime(Level lvl,
...
template <typename... Args> inline void trace(fmt::format_string<Args...> s, Args&&... a) { log(Level::trace,    s, std::forward<Args>(a)...); }
template <typename... Args> inline void debug(fmt::format_string<Args...> s, Args&&... a) { log(Level::debug,    s, std::forward<Args>(a)...); }
template <typename... Args> inline void info (fmt::format_string<Args...> s, Args&&... a) { log(Level::info,     s, std::forward<Args>(a)...); }
template <typename... Args> inline void warn (fmt::format_string<Args...> s, Args&&... a) { log(Level::warn,     s, std::forward<Args>(a)...); }
template <typename... Args> inline void error(fmt::format_string<Args...> s, Args&&... a) { log(Level::error,    s, std::forward<Args>(a)...); }
template <typename... Args> inline void crit (fmt::format_string<Args...> s, Args&&... a) { log(Level::critical, s, std::forward<Args>(a)...); }
...
inline void basic_mud_log(fmt::string_view printf_style_fmt, Args&&... args)
{
    try {
        ...
        error("SYSERR: Format error in basic_mud_log: {} (template: {})", e.what(), printf_style_fmt);
    }
}
...
{
    try {
        log_runtime(Level::info, fmt_style_fmt, std::forward<Args>(args)...);
    } catch (const std::exception& e) {
        error("SYSERR: Format error in template_mud_log: {} (template: {})", e.what(), fmt_style_fmt);
    }
}
...
#define LOG_WARN_ONCE(msg, ...)                                           \
    do { static std::atomic<bool> _done{false};                           \
         if (!_done.exchange(true)) LOG_WARN(msg, ##__VA_ARGS__);         \
    } while (0)

#define LOG_INFO_EVERY_N(n, msg, ...)                                     \
    do { static std::atomic<unsigned> _cnt{0};                            \
         auto _v = _cnt.fetch_add(1) + 1;                                 \
         if ((_v % (n)) == 0) LOG_INFO(msg, ##__VA_ARGS__);               \
    } while (0)
