...
SPECIAL(postmaster)
{
    ...
    if (no_mail)
    {
        ch->sendText("Sorry, the mail system is having technical difficulties.\r\n");
        return (0);
    }
                          ...
                          int cmd, char *arg)
{
    ...
    ch->modBaseStat("money_carried", -STAMP_PRICE);
                             ...
                             int cmd, char *arg)
{
    ...
    while (has_mail(GET_IDNUM(ch)))
    {
        ...
        for (auto f : {ITEM_WEAR_TAKE, ITEM_WEAR_HOLD})
            obj->wear_flags.set(f);
