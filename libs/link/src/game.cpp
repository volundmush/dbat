...
namespace dbat::link {
    ...
    boost::asio::awaitable<dbat::web::HttpAnswer> handle_request(RequestMessage& msg);

    std::size_t drain_request_channel(std::chrono::steady_clock::duration budget) {
    ...
    void run_game(double heartbeat_interval, double save_interval) {
        ...
        try {
            while(running) {
                ...
                co_await dbat::link::acceptNewConnections();
                co_await dbat::link::handleNewInput();
                runOneLoop(heartbeat_interval);
                co_await dbat::link::handleNewOutput();
                co_await dbat::link::handleDisconnections();
                auto end =  clock::now();
                ...
                start = clock::now();
                auto processed = drain_request_channel(remaining);
