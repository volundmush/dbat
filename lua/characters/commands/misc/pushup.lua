...
local GRAVITY_BONUS = {
    [0]     = { 1,     1,     "@gYou do a pushup.@n", "@g$n does a pushup.@n" },
    [10]    = { 3,     7,     "@gYou do a pushup, feeling the strain of gravity.@n", "@g$n does a pushup, while sweating.@n" },
    [20]    = { 8,     14,    "@gYou do a pushup, and feel gravity's pull.@n", "@g$n does a pushup, while sweating.@n" },
    [30]    = { 14,    20,    "@gYou do a pushup, and feel the burn.@n", "@g$n does a pushup, while sweating.@n" },
    [40]    = { 20,    35,    "@gYou do a pushup, and feel the burn.@n", "@g$n does a pushup, while sweating.@n" },
    [50]    = { 40,    60,    "@gYou do a pushup, and feel the burn.@n", "@g$n does a pushup, while sweating.@n" },
    [100]   = { 180,   250,   "@gYou do a pushup, and really strain against the gravity.@n", "@g$n does a pushup, while sweating.@n" },
    [200]   = { 400,   600,   "@gYou do a pushup, and really strain against the gravity.@n", "@g$n does a pushup, while sweating.@n" },
    [300]   = { 800,   1200,  "@gYou do a pushup, and really strain against the gravity.@n", "@g$n does a pushup, while sweating.@n" },
    [400]   = { 2000,  3000,  "@gYou do a pushup, and really strain against the gravity.@n", "@g$n does a pushup, while sweating.@n" },
    [500]   = { 4000,  6000,  "@gYou do a pushup, and really strain against the gravity.@n", "@g$n does a pushup, while sweating.@n" },
    [1000]  = { 9000,  10000, "@gYou do a pushup, and it was a really hard one to finish.@n", "@g$n does a pushup, while sweating profusely.@n" },
    [5000]  = { 15000, 20000, "@gYou do a pushup, and it was a really hard one to finish.@n", "@g$n does a pushup, while sweating profusely.@n" },
    [10000] = { 25000, 30000, "@gYou do a pushup, and it was a really hard one to finish.@n", "@g$n does a pushup, while sweating profusely.@n" },
}
...
local function execute(ctx)
    ...
    if ch:condition_has("flying") then
        ch:send_line("You can't do pushups in midair!")
        return
    end
    ...
    bonus = apply_challenge_message(ch, cost, ch:meter_max("powerlevel"), bonus)
    ...
    ch:send_line("You feel slightly stronger @D[@G+%s@D]@n.", text.add_commas(bonus))

    if ch:race_get() == "human" then
        bonus = math.floor(bonus * 0.8)
    end

    ch:stat_mod("powerlevel", bonus)
