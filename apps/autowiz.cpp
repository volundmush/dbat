...
{
  {ADMLVL_IMMORT, "@BImmortals@D"},
  {ADMLVL_GOD, "@GGreater Immortals@D"},
  {ADMLVL_GRGOD, "@YAdministrators@D"},
  {ADMLVL_IMPL, "   @ROwners   @D"},
  {0, ""}
};
...
void read_file(void)
{
  void add_name(int8_t level, char *name);
  char *CAP(char *txt);
...
void write_wizlist(FILE * out, int minlev, int maxlev)
{
  ...
  fprintf(out,
"@m*********************************************************************************\n"
"@m*@W           The following people have reached immortality on DBAT.              @m*\n"
"@m*********************************************************************************@n\n\n");
  ...
  for (curr_level = levels; curr_level; curr_level = curr_level->next) {
	...
	curr_level->params->level > maxlev)
      continue;
    i = 39 - ((strlen(curr_level->params->level_name) - 4) >> 1);
    for (j = 1; j <= i; j++)
      fputc(' ', out);
    fprintf(out, "%s\n", curr_level->params->level_name);
    for (j = 1; j <= i; j++)
      fputc(' ', out);
    for (j = 1; j <= (strlen(curr_level->params->level_name) - 4); j++)
      fputc('~', out);
    fprintf(out, "\n");
	...
	if (curr_level->params->level > COL_LEVEL) {
         if (!strcmp(curr_name->name, "Iovan"))
	  strcat(buf, "    ");
         else
          strcat(buf, "    ");
        }
