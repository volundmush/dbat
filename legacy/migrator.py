...
class Migrator:
    ...
    def _migrate_mobile_prototypes(self):
        ...
        INSERT INTO mobile_prototypes (
            ...
            title, sex, race, chclass,
            act0, act1, act2, act3,
            affected_by0, affected_by1, affected_by2, affected_by3,
            level, alignment, str, intel, wis, dex, con, cha, default_position
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        query2 = """
    ...
    def _migrate_users(self):
        query = """
        INSERT INTO users (
            username, email, password_hash, rpp_points,
            rpp_bank, extra_slots, rpp_awarded
        ) VALUES (?, ?, ?, ?, ?, ?, ?)
        """
