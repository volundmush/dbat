...
def text(entry, option_key="Text", **kwargs):
    try:
        ...
        raise ValueError(
            _("Input could not be converted to text ({err})").format(err=err)
        )
...
def color(entry, option_key="Color", **kwargs):
    ...
    if not entry:
        raise ValueError(
            _("Nothing entered for a {option_key}!").format(option_key=option_key)
        )
    test_str = strip_ansi(f"|{entry}|n")
    if test_str:
        raise ValueError(
            _("'{entry}' is not a valid {option_key}.").format(
...
def datetime(entry, option_key="Datetime", account=None, from_tz=None, **kwargs):
    ...
    if not from_tz:
        ...
        if account:
            ...
            try:
                ...
                raise ValueError(
                    _(
                        "Timezone string '{acct_tz}' is not a valid timezone ({err})"
                    ).format(acct_tz=acct_tz, err=err)
                )
        else:
            from_tz = _pytz.UTC
    ...
    if len(split_time) == 3:
        ...
        raise ValueError(
            _(
                "{option_key} must be entered in a 24-hour format such as: {timeformat}"
            ).format(option_key=option_key, timeformat=now.strftime("%b %d %H:%M"))
        )
    try:
...
def signed_integer(entry, option_key="Signed Integer", **kwargs):
    if not entry:
        raise ValueError(
            _("Must enter a whole number for {option_key}!").format(
                option_key=option_key
            )
        )
    try:
        ...
        raise ValueError(
            _(
                "Could not convert '{entry}' to a whole " "number for {option_key}!"
            ).format(entry=entry, option_key=option_key)
        )
    return num
...
def unsigned_integer(entry, option_key="Unsigned Integer", **kwargs):
    ...
    if not num >= 0:
        raise ValueError(
            _(
                "{option_key} must be a whole number greater than " "or equal to 0!"
            ).format(option_key=option_key)
        )
    return num
...
def boolean(entry, option_key="True/False", **kwargs):
    ...
    """
    error = _(
        "Must enter a true/false input for {option_key}. Accepts {alternatives}."
    ).format(
        option_key=option_key,
...
def timezone(entry, option_key="Timezone", **kwargs):
    ...
    if not entry:
        raise ValueError(_("No {option_key} entered!").format(option_key=option_key))
    found = partial_match(entry, list(_TZ_DICT.keys()))
    if len(found) > 1:
        raise ValueError(
...
def email(entry, option_key="Email Address", **kwargs):
    ...
    if not valid:
        raise ValueError(
            _("That isn't a valid {option_key}!").format(option_key=option_key)
        )
    return entry
...
def lock(entry, option_key="locks", access_options=None, **kwargs):
    ...
    if not entry:
        raise ValueError(
            _("No {option_key} entered to set!").format(option_key=option_key)
        )
