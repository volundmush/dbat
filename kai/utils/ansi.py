...
from kai.utils.utils import to_str

from kai.utils import utils
    ...
    def sub_xterm256(self, rgbmatch, use_xterm256=False, color_type="fg"):
        ...
        if not grayscale:
            ...
            try:
                ...
                logging.exception("Error converting color codes.")
        ...
        if use_xterm256:
            ...
            if (max(rgb) - min(rgb)) <= 1:
                match rgb:
                    case (0, 0, 0):
                        return (
                            ANSI_BACK_BLACK if background else ANSI_NORMAL + ANSI_BLACK
                        )
                    case ((1 | 2), (1 | 2), (1 | 2)):
                        return (
                            ANSI_BACK_BLACK if background else ANSI_HILITE + ANSI_BLACK
                        )
                    case ((2 | 3), (2 | 3), (2 | 3)):
                        return (
                            ANSI_BACK_WHITE if background else ANSI_NORMAL + ANSI_WHITE
                        )
                    case ((3 | 4), (3 | 4), (3 | 4)):
                        return (
                            ANSI_BACK_WHITE if background else ANSI_NORMAL + ANSI_WHITE
                    ...
                    case ((4 | 5), (4 | 5), (4 | 5)):
                        return (
                            ANSI_BACK_WHITE if background else ANSI_HILITE + ANSI_WHITE
                        )
            ...
            match tuple(_convert_for_ansi(c) for c in rgb):
                ...
                case ((2 | 3), (0 | 1 | 2), (2 | 3)) if green == min(rgb):
                    return (
                        ANSI_BACK_MAGENTA if background else ANSI_HILITE + ANSI_MAGENTA
                    )
                case ((1 | 2), 0, (1 | 2)):
                    return (
                        ANSI_BACK_MAGENTA if background else ANSI_NORMAL + ANSI_MAGENTA
                    )
...
def ev_to_rich(text: str):
    ...
    """
    ev = parse_ansi(str(text), xterm256=True, mxp=False)
