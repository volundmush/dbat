...
// functions
extern void do_start(BaseCharacter *ch);

extern int invalid_class(BaseCharacter *ch, Object *obj);

extern int64_t level_exp(BaseCharacter *ch, int level);

extern int load_levels();

extern void cedit_creation(BaseCharacter *ch);

extern void advance_level(BaseCharacter *ch);

extern int8_t ability_mod_value(int abil);

extern int8_t dex_mod_capped(BaseCharacter *ch);

extern int total_skill_levels(BaseCharacter *ch, int skill);

extern int highest_skill_value(int level, int type);

extern time_t birth_age(BaseCharacter *ch);
