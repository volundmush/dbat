...
rapidjson::Document guild_data::rserialize() {
    ...
    // Adding 'skills' as an array
    rapidjson::Value skillsArray(rapidjson::kArrayType);
    for (auto s : skills) {
        skillsArray.PushBack(s, allocator);
    }
    if (!skillsArray.Empty()) {
        d.AddMember("skills", skillsArray, allocator);
    }

    // Adding 'feats' as an array
    rapidjson::Value featsArray(rapidjson::kArrayType);
    for (auto f : feats) {
        featsArray.PushBack(f, allocator);
    }
    if (!featsArray.Empty()) {
        d.AddMember("feats", featsArray, allocator);
    }

    // Adding other members if they are set
    if (charge != 1.0) {
        d.AddMember("charge", charge, allocator);
    }
    if (!no_such_skill.empty()) {
        d.AddMember("no_such_skill", rapidjson::Value(no_such_skill.c_str(), allocator).Move(), allocator);
    }
    if (!not_enough_gold.empty()) {
        d.AddMember("not_enough_gold", rapidjson::Value(not_enough_gold.c_str(), allocator).Move(), allocator);
    }
    if (minlvl) {
        d.AddMember("minlvl", minlvl, allocator);
    }
    if (gm != NOBODY) {
        d.AddMember("gm", gm, allocator);
    }
    ...
    if (open) {
        d.AddMember("open", open, allocator);
    }
    if (close) {
        d.AddMember("close", close, allocator);
    }
