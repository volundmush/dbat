...
const char *custom_types[17] = {
    "Error",         "Armor Slot", "Gi Slot",    "Wrist Slot", "Ear Slot",
    "Finger Slot",   "Eye Slot",   "Hands Slot", "Feet Slot",  "Belt Slot",
    "Legs Slot",     "Arms Slot",  "Head Slot",  "Neck Slot",  "Back Slot",
    "Shoulder Slot", "Weapon"};

/* This shows what weapon type it is */
const char *custom_weapon[7] = {"Not A Weapon", "Sword", "Dagger",  "Spear",
                                "Club",         "Gun",   "Brawling"};
...
void pobj_edit_parse(struct descriptor_data *d, char *arg) {
  ...
  if (d->obj_editflag == EDIT_RESTRING) {
    ...
    case EDIT_RESTRING_MAIN:
      ...
      case '1': /* Name Editing */
        write_to_output(d,
                        "@wCurrent Equipment's Keyword Name @D<@Y%s@D>@n\r\n",
                        d->obj_name);
        write_to_output(d,
                        "@wEnter Name @D(@RDo not use colorcode!@D)@w:@n \n");
      ...
      case 'q':
        write_to_output(
            d, "Save current changes and be charged?\r\nYes or No\r\n");
    ...
    case EDIT_RESTRING_NAME: /* Name menu */
      if (!*arg) {
        write_to_output(
            d, "@wNothing entered. Keeping what was previously entered.@n\r\n");
        disp_restring_menu(d);
    ...
    case EDIT_RESTRING_QUIT:
      if (!*arg) {
        write_to_output(
            d, "Save current changes and be charged?\r\nYes or No\r\n");
        return;
      } else if (!strcasecmp(arg, "yes") || !strcasecmp(arg, "Yes") ||
                 !strcasecmp(arg, "y") || !strcasecmp(arg, "Y")) {
        obj = d->obj_point;
        ...
        *buf3 = '\0';
        ...
        char_stat_mod(d->character, "money", -5000);
        SET_BIT_AR(GET_OBJ_EXTRA(obj), ITEM_RESTRING);
        write_to_output(d, "Purchase complete.");
        send_to_imm("Restring Eq: %s has bought: %s, which was %s.",
                    GET_NAME(d->character), obj->short_description, d->obj_was);
        STATE(d) = CON_PLAYING;
      } else if (!strcasecmp(arg, "No") || !strcasecmp(arg, "no") ||
                 !strcasecmp(arg, "n") || !strcasecmp(arg, "N")) {
        write_to_output(d, "Canceling purchase at no cost.\r\n");
        send_to_imm("Restring Eq: %s has canceled their equipment restring.",
  ...
  if (d->obj_editflag == EDIT_CUSTOM) {
    ...
    case EDIT_CUSTOM_MAIN: /* The main menu of custom equipment edit */
      ...
      case '1': /* Let them name it. */
        write_to_output(d, "@wEnter Equipment's Keyword Name @D(@RDo not use "
                           "colorcode!@D)@w.@n\r\n");
        write_to_output(d, "@wEnter: @n\n");
      ...
      case '2': /* Let them give it a short description. */
        write_to_output(
            d, "@wEnter Equipment's Short Description. This is the colored "
               "name you see in your inventory or when the eq is used.@n\r\n");
        write_to_output(d, "@wEnter: @n\n");
        d->obj_editval = EDIT_CUSTOM_SDESC;
        break;
      case '3': /* Let them give it a long description. */
        write_to_output(d, "@wEnter Equipment's Long Description. This is the "
                           "colored name you see when it's seen in a room. "
                           "@D(@RNo punctuation at the end!@D)@n\r\n");
        write_to_output(d, "@wEnter: @n\n");
        d->obj_editval = EDIT_CUSTOM_LDESC;
        break;
      case '4': /* Let them choose what type of equipment it is. */
        write_to_output(d, "@wEnter number to select type of equipment you "
                           "want it to be: @n\n");
        write_to_output(d, "@D[ @C--1@W) @cArmor Slot  @C--2@W) @cGi Slot     "
                           "@C--3@W) @cWrist Slot   @D]@n\n");
        write_to_output(d, "@D[ @C--4@W) @cEar Slot    @C--5@W) @cFinger Slot "
                           "@C--6@W) @cEye Slot     @D]@n\n");
        write_to_output(d, "@D[ @C--7@W) @cHands Slot  @C--8@W) @cFeet Slot   "
                           "@C--9@W) @cBelt Slot    @D]@n\n");
        write_to_output(d, "@D[ @C-10@W) @cLegs Slot   @C-11@W) @cArms Slot   "
                           "@C-12@W) @cHead Slot    @D]@n\n");
        write_to_output(d, "@D[ @C-13@W) @cNeck Slot   @C-14@W) @cBack Slot   "
                           "@C-15@W) @cShoulder Slot@D]@n\n");
      ...
      case '5': /* If it's a weapon then let them choose what type. */
        if (d->obj_type != 16) {
          write_to_output(d, "@wYou can only use this part of the menu if you "
                             "select the weapon type.@n\r\n");
          return;
        } else {
          write_to_output(d, "@wEnter number to select type of weapon you want "
                             "it to be: @n\n");
          write_to_output(d, "@D[ @C--1@W) @cSword       @C--2@W) @cDagger     "
                             " @C--3@W) @cSpear        @D]@n\n");
          write_to_output(d, "@D[ @C--4@W) @cClub        @C--5@W) @cGun        "
                             " @C--6@W) @cBrawling     @D]@n\n");
          write_to_output(d, "@wEnter: @n\n");
    ...
    case EDIT_CUSTOM_QUIT: /* Quiting */
      if (!*arg) {
        ...
        send_to_imm("Custom Eq: %s has bought: %s.", GET_NAME(d->character),
                    obj->short_description);
        customWrite(d->character, obj);
        log_custom(d, obj);
      } else if (!strcasecmp(arg, "n") || !strcasecmp(arg, "N")) {
        write_to_output(d, "Canceling purchase at no cost.\r\n");
        send_to_imm("Custom Eq: %s has canceled their custom eq construction.",
                    GET_NAME(d->character));
        d->obj_editval = EDIT_NONE;
