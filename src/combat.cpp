    ...
    if (!ch->isSparring()) {
        if (area == 0) { /* Arms */
            if (GET_LIMBCOND(vict, 1) - dmg <= 0) {
                ...
                GET_LIMBCOND(vict, 1) = 0;
                for(auto f : {PLR_THANDW}) vict->player_flags.set(f, false);
                remove_limb(vict, 2);
...
void saiyan_gain(Character *ch, Character *vict) {
    ...
    if(ch->technique == Form::tiger_stance) stats.push_back((int) CharVital::health);
...
void pcost(Character *ch, double ki, int64_t st) {
    if (IS_NPC(ch)) {
        if(ki) ch->modCurVital(CharVital::ki, -ki);
        if(st) ch->modCurVital(CharVital::stamina, -st);
        return;
    }
