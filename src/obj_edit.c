  ...
  switch (d->obj_editval) {
   ...
   case EDIT_RESTRING_QUIT:
     ...
     *buf3 = '\0';
     ...
     d->character->rbank -= 2;
     GET_RBANK(d->character) = d->character->rbank;
     d->rbank -= 2;
   ...
   case EDIT_CUSTOM_MAIN: /* The main menu of custom equipment edit */
    switch (*arg) {
     ...
     case '2': /* Let them give it a short description. */
      write_to_output(d, "@wEnter Equipment's Short Description. This is the colored name you see in your inventory or when the eq is used.@n\r\n");
      write_to_output(d, "@wEnter: @n\n");
      d->obj_editval = EDIT_CUSTOM_SDESC;
      break;
     case '3': /* Let them give it a long description. */
      write_to_output(d, "@wEnter Equipment's Long Description. This is the colored name you see when it's seen in a room. @D(@RNo punctuation at the end!@D)@n\r\n");
      write_to_output(d, "@wEnter: @n\n");
     ...
     case '4': /* Let them choose what type of equipment it is. */
      ...
      write_to_output(d, "@D[ @C--1@W) @cArmor Slot  @C--2@W) @cGi Slot     @C--3@W) @cWrist Slot   @D]@n\n");
      write_to_output(d, "@D[ @C--4@W) @cEar Slot    @C--5@W) @cFinger Slot @C--6@W) @cEye Slot     @D]@n\n");
      write_to_output(d, "@D[ @C--7@W) @cHands Slot  @C--8@W) @cFeet Slot   @C--9@W) @cBelt Slot    @D]@n\n");
      write_to_output(d, "@D[ @C-10@W) @cLegs Slot   @C-11@W) @cArms Slot   @C-12@W) @cHead Slot    @D]@n\n");
      write_to_output(d, "@D[ @C-13@W) @cNeck Slot   @C-14@W) @cBack Slot   @C-15@W) @cShoulder Slot@D]@n\n");
     ...
     case '5': /* If it's a weapon then let them choose what type. */
      if (d->obj_type != 16) {
       write_to_output(d, "@wYou can only use this part of the menu if you select the weapon type.@n\r\n");
       return;
      } else {
       write_to_output(d, "@wEnter number to select type of weapon you want it to be: @n\n");
       write_to_output(d, "@D[ @C--1@W) @cSword       @C--2@W) @cDagger      @C--3@W) @cSpear        @D]@n\n");
       write_to_output(d, "@D[ @C--4@W) @cClub        @C--5@W) @cGun         @C--6@W) @cBrawling     @D]@n\n");
       write_to_output(d, "@wEnter: @n\n");
