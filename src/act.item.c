...
ACMD(do_refuel)
{
 ...
 send_to_char(ch, "You place the fuel canister into the ship. Within seconds the fuel has been extracted from the canister into the ships' internal tanks.\r\n");
...
void harvest_plant(struct char_data *ch, struct obj_data *plant)
{
 ...
 if (skill >= 100) {
  ...
  reward += 2;
  send_to_char(ch, " and your mastery of the basics of gardening has also helped the plant be more bountiful yet!@n\r\n");
 } else if (skill >= 30) {
  reward += 1;
  send_to_char(ch, " and you somehow managed to make the plant slightly more bountiful with what little you know!@n\r\n");
 } else {
  send_to_char(ch, ".@n\r\n");
 }
 ...
 if (*arg) {
  if (!str_cmp(arg, "collect")) {
   ...
   if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_FERTILE1)) {
    ...
    act("@w$n@y sinks $s shovel into the soft ground and manages to dig up a pile of fertile soil!@n", TRUE, ch, 0, 0, TO_ROOM);
    ...
    act("@w$n@y sinks $s shovel into the soft ground and manages to dig up a pile of good soil!@n", TRUE, ch, 0, 0, TO_ROOM);
    ...
    struct obj_data *soil = read_object(255, VIRTUAL);
    obj_to_char(soil, ch);
    act("@yYou sink your shovel into the soft ground and manage to dig up a pile of soil!@n", TRUE, ch, 0, 0, TO_CHAR);
    act("@w$n@y sinks $s shovel into the soft ground and manages to dig up a pile of soil!@n", TRUE, ch, 0, 0, TO_ROOM);
 ...
 if (GET_MOVE(ch) < cost) {
  ...
  if (!str_cmp(arg2, "water")) {
   ...
   if (!OBJ_FLAGGED(obj, ITEM_MATURE)) {
    ...
    act("@GYou end up shredding @g$p@G with your clumsy unskilled hands.@n", TRUE, ch, obj, 0, TO_CHAR);
    act("@g$n@G grabs a hold of @g$p@G and shreds it in an attempt to pick it.@n", TRUE, ch, obj, 0, TO_ROOM);
    return;
   } else {
    act("@GYou grab a hold of @g$p@G and carefully pick it out of the soil.@n", TRUE, ch, obj, 0, TO_CHAR);
...
ACMD(do_pack)
{
 ...
 if (!(obj = get_obj_in_list_vis(ch, arg, NULL, world[IN_ROOM(ch)].contents))) {
  ...
  if ((GET_OBJ_VNUM(obj) >= 19090 && GET_OBJ_VNUM(obj) <= 19099) || GET_OBJ_VNUM(obj) == 11) {
   act("@CYou push a hidden button on $p@C and a cloud of smoke erupts and covers it. As the smoke clears a small capsule can be seen on the ground.@n", TRUE, ch, obj, 0, TO_CHAR);
   act("@c$n@C pushes a hidden button on $p@C and a cloud of smokes erupts and covers it. As the smoke clears a small capsule can be seen on the ground.@n", TRUE, ch, obj, 0, TO_ROOM);
   ...
   if (!*arg2) {
    ...
    act("@CYou push a hidden button on $p@C and a cloud of smoke erupts and covers it. As the smoke clears a pile of money can be seen on the ground!@n", TRUE, ch, obj, 0, TO_CHAR);
    act("@c$n@C pushes a hidden button on $p@C and a cloud of smokes erupts and covers it. As the smoke clears a pile of money can be seen on the ground!@n", TRUE, ch, obj, 0, TO_ROOM);
...
ACMD(do_deploy)
{
 ...
 struct obj_data *obj3, *next_obj, *obj4, *obj = NULL;
 ...
 if (!*arg) {
  for (obj4 = ch->carrying; obj4; obj4 = next_obj) {
...
ACMD(do_twohand)
{
 ...
 if (!GET_EQ(ch, WEAR_WIELD1) && !PLR_FLAGGED(ch, PLR_THANDW)) {
  ...
  act("$n stops wielding $s weapon with both hands.", TRUE, ch, 0, 0, TO_ROOM);
  REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_THANDW);
  return;
 }
 else {
  send_to_char(ch, "You grab your weapon with both hands.\r\n");
  act("$n starts wielding $s weapon with both hands.", TRUE, ch, 0, 0, TO_ROOM);
...
if (dballtime == 0) {
 ...
 else if (dballtime == 518400 || dballtime == 432000 || dballtime == 345600 || dballtime == 259200 || dballtime == 172800 || dballtime == 86400) {
  dballtime -= 1;
 }
 else {
  if (WISHTIME == 0) {
  ...
  if (!str_cmp(arg, "list")) {
   ...
   for (obj = world[auct_room].contents; obj; obj = next_obj) {
     ...
     if (obj) {
      ...
      send_to_char(ch, "@D[@R#@W%3d@D][@mOwner@W: @w%10s@D][@GItem Name@W: @w%-*s@D][@GCost@W: @Y%s@D]@n\r\n", list, get_name_by_id(GET_AUCTER(obj)) != NULL ? CAP(get_name_by_id(GET_AUCTER(obj))) : "Nobody", count_color_chars(obj->short_description)+30, obj->short_description, add_commas(GET_BID(obj)));
      } else if (GET_AUCTIME(obj) + 86400 > time(0) && GET_CURBID(obj) > -1) {
      send_to_char(ch, "@D[@R#@W%3d@D][@mOwner@W: @w%10s@D][@GItem Name@W: @w%-*s@D][@RTop Bid@W: %s @Y%s@D]@n\r\n", list, get_name_by_id(GET_AUCTER(obj)) != NULL ? CAP(get_name_by_id(GET_AUCTER(obj))) : "Nobody", count_color_chars(obj->short_description)+30, obj->short_description, get_name_by_id(GET_CURBID(obj)) != NULL ? CAP(get_name_by_id(GET_CURBID(obj))) : "Nobody", add_commas(GET_BID(obj)));
      } else if (GET_AUCTIME(obj) + 86400 < time(0) && GET_CURBID(obj) > -1) {
      send_to_char(ch, "@D[@R#@W%3d@D][@mOwner@W: @w%10s@D][@GItem Name@W: @w%-*s@D][@RBid Winner@W: %s @Y%s@D]@n\r\n", list, get_name_by_id(GET_AUCTER(obj)) != NULL ? CAP(get_name_by_id(GET_AUCTER(obj))) : "Nobody", count_color_chars(obj->short_description)+30, obj->short_description, get_name_by_id(GET_CURBID(obj)) != NULL ? CAP(get_name_by_id(GET_CURBID(obj))) : "Nobody", add_commas(GET_BID(obj)));
      } else {
      send_to_char(ch, "@D[@R#@W%3d@D][@mOwner@W: @w%10s@D][@GItem Name@W: @w%-*s@D][@RClosed@D]@n\r\n", list, get_name_by_id(GET_AUCTER(obj)) != NULL ? CAP(get_name_by_id(GET_AUCTER(obj))) : "Nobody", count_color_chars(obj->short_description)+30, obj->short_description);
      }
      found = TRUE;
     }
   }
   ...
   if (!obj2) {
    ...
    if (GET_SKILL(ch, SKILL_APPRAISE) < rand_number(1, 101)) {
     ...
     act("@c$n@w looks stumped about something they viewed on their scouter screen.@n", TRUE, ch, 0, 0, TO_ROOM);
     WAIT_STATE(ch, PULSE_2SEC);
     return;
    } else {
    send_to_char(ch, "You look at images of the object on your scouter.\r\n");
    act("@c$n@w looks at something on their scouter screen.@n", TRUE, ch, 0, 0, TO_ROOM);
    send_to_char(ch, "@c------------------------------------------------------------------------\n");
    send_to_char(ch, "@GOwner       @W: @w%s@n\n", get_name_by_id(GET_AUCTER(obj2)) != NULL ? CAP(get_name_by_id(GET_AUCTER(obj2))) : "Nobody");
   ...
   if (!obj2) {
    ...
    return;
      ...
      return;
   } else if (GET_CURBID(obj2) > 0 && atoi(arg2) <= (GET_BID(obj2) + (GET_BID(obj2) * .1)) && GET_CURBID(obj2) > -1) {
      ...
      return;
   } else if (atoi(arg2) > (((GET_GOLD(ch) + GET_BANK_GOLD(ch)) / 100) * 50) + (GET_GOLD(ch) + GET_BANK_GOLD(ch))) {
    ...
    for (d = descriptor_list; d; d = d->next) {
      ...
      if (GET_EQ(d->character, WEAR_EYE)) {
       send_to_char(d->character, "@RScouter Auction News@D: @GThe bid on, @w%s@G, has been raised to @Y%s@n\r\n", obj2->short_description, add_commas(GET_BID(obj2)));
      }
    }
   }
...
ACMD(do_assemble)
{
  ...
  if (str_cmp(argument, "campfire")) {
   ...
   if (GET_SKILL(ch, SKILL_SURVIVAL) <= roll) {
    ...
    if (tool && rand_number(1, 3) == 3 && GET_OBJ_VAL(tool, VAL_ALL_HEALTH) > 0) {
     ...
     act("@RYour toolset is looking a bit more worn.@n", TRUE, ch, 0, 0, TO_CHAR);
  ...
  if (axion_dice(0) - (GET_INT(ch) / 5) > 95) {
    ...
    extract_obj(pObject);
    send_to_char(ch, "You start to %s %s %s, but put it together wrong and have to stop. You take it apart and give up.\r\n", CMD_NAME, AN(argument), argument);
    WAIT_STATE(ch, PULSE_4SEC);
    return;
  }
		      ...
		      struct obj_data *cont)
{
  ...
  if ((GET_OBJ_TYPE(cont) == ITEM_CONTAINER) && (GET_OBJ_VAL(cont, VAL_CONTAINER_CAPACITY) == 0))
   ...
   send_to_char(ch, "The save room can not hold anymore items. (150 max, count of items in containers is halved)\r\n");
  } else {
    obj_from_char(obj);
    obj_to_obj(obj, cont);
...
int can_take_obj(struct char_data *ch, struct obj_data *obj)
{
  if (!(CAN_WEAR(obj, ITEM_WEAR_TAKE))) {
    ...
    return (0);
  } else if ((gear_weight(ch) + GET_OBJ_WEIGHT(obj)) > CAN_CARRY_W(ch)) {
    act("$p: you can't carry that much weight.", FALSE, ch, obj, 0, TO_CHAR);
    return (0);
  } else if ((GET_OBJ_WEIGHT(obj) + ROOM_GRAVITY(IN_ROOM(ch))) + gear_weight(ch) > CAN_CARRY_W(ch)) {
...
ACMD(do_drop)
{
  ...
  if (!*arg) {
    ...
    if (!str_cmp("zenni", arg) || !str_cmp("gold", arg))
      ...
      send_to_char(ch, "The save room you are in can not hold anymore items! (150 max, count of items in containers is halved)\r\n");
    else {
      do {
	...
	send_to_char(ch, "You don't seem to have %s %s.\r\n", AN(arg), arg);
        ...
        send_to_char(ch, "The item couldn't be dropped into this save room. It is too full. (150 max, containers half the count inside)\r\n");
      } else {
	amount += perform_drop(ch, obj, mode, sname, RDR);
      }
    }
		       ...
		       struct obj_data *obj)
{
  ...
  if (GET_OBJ_WEIGHT(obj) + gear_weight(vict) > CAN_CARRY_W(vict)) {
  ...
  if ((GET_OBJ_WEIGHT(obj) + ROOM_GRAVITY(IN_ROOM(vict))) + gear_weight(vict) > CAN_CARRY_W(vict)) {
  ...
  act("$n gives $p to $N.", TRUE, ch, obj, vict, TO_NOTVICT);
      ...
      if (OBJ_FLAGGED(obj, ITEM_HOT)) {
       if (GET_BONUS(vict, BONUS_FIREPROOF) <= 0 && !IS_DEMON(vict)) {
        GET_HIT(vict) -= GET_HIT(vict) * 0.25;
  ...
  if (!*arg)
	...
	perform_give_gold(ch, vict, amount);
        if (GET_ADMLEVEL(ch) > 0 && !IS_NPC(vict)) {
         send_to_imm("IMM GIVE: %s has given %s zenni to %s.", GET_NAME(ch), add_commas(amount), GET_NAME(vict));
         log_imm_action("IMM GIVE: %s has given %s zenni to %s.", GET_NAME(ch), add_commas(amount), GET_NAME(vict));
        }
      }
      return;
	...
	perform_give(ch, vict, obj);
        if (GET_ADMLEVEL(ch) > 0 && !IS_NPC(vict)) {
         send_to_imm("IMM GIVE: %s has given %s to %s.", GET_NAME(ch), obj->short_description, GET_NAME(vict));
         log_imm_action("IMM GIVE: %s has given %s to %s.", GET_NAME(ch), obj->short_description, GET_NAME(vict));
        }
	obj = next_obj;
      }
    }
...
ACMD(do_drink)
{
  ...
  if (!*arg && !IS_NPC(ch)) {
    ...
    switch (SECT(IN_ROOM(ch))) {
      ...
      case SECT_UNDERWATER:
        ...
        if (GET_SKILL(ch, SKILL_WELLSPRING) && GET_MANA(ch) < GET_MAX_MANA(ch) && wasthirsty <= 30 && subcmd != SCMD_SIP) {
         GET_MANA(ch) += ((GET_MAX_MANA(ch) * 0.005) + (GET_WIS(ch) * rand_number(80, 100))) * GET_SKILL(ch, SKILL_WELLSPRING);
  ...
  if ((GET_OBJ_TYPE(temp) != ITEM_DRINKCON) &&
    ...
    if (GET_OBJ_VNUM(temp) == 86 && on_ground != 1) {
     act("@wYou uncork the $p and tip it to your lips. Drinking it down you feel a warmth flow through your body and your ki returns to full strength .@n", TRUE, ch, temp, 0, TO_CHAR);
  ...
  if (GET_FOODR(ch) == 0 && subcmd != SCMD_SIP) {
   GET_MOVE(ch) += (GET_MAX_MOVE(ch) / 100) * amount;
   GET_FOODR(ch) = 2;
...
ACMD(do_eat)
{
  ...
  if (GET_OBJ_VNUM(food) >= MEAL_START && GET_OBJ_VNUM(food) <= MEAL_LAST && GET_COND(ch, HUNGER) < 48 && (!ROOM_FLAGGED(IN_ROOM(ch), ROOM_AL) && !ROOM_FLAGGED(IN_ROOM(ch), ROOM_RHELL))) {
   ...
   if (!GET_OBJ_VAL(food, VAL_FOOD_POISON) && GET_HIT(ch) < gear_pl(ch) && subcmd != SCMD_TASTE) {
    ...
    if (GET_WEIGHT(food) < 6) {
     ...
     GET_HIT(ch) += gear_pl(ch) * 0.1;
    }
    if (OBJ_FLAGGED(food, ITEM_YUM)) {
     GET_HIT(ch) += gear_pl(ch) * 0.20;
    }
    if (GET_HIT(ch) > gear_pl(ch)) {
     GET_HIT(ch) = gear_pl(ch);
    }
    if (suppress > 0) {
     if (GET_HIT(ch) > suppress) {
      GET_HIT(ch) = suppress;
     }
    }
...
ACMD(do_pour)
{
  ...
  if (subcmd == SCMD_FILL) {
    ...
    if (GET_OBJ_TYPE(from_obj) != ITEM_FOUNTAIN && !OBJ_FLAGGED(from_obj, ITEM_BROKEN)) {
      ...
      act("You can't fill something from a broken fountain.", FALSE, ch, from_obj, 0, TO_CHAR);
      return;
    }
  }
  ...
  if (!*arg)
    ...
    if (GET_OBJ_TYPE(obj) == ITEM_LIGHT) {
      ...
      if (GET_OBJ_VAL(obj, VAL_LIGHT_HOURS) == 0) {
       act("@wYou try to light $p@w but it is burnt out.", TRUE, ch, obj, 0, TO_CHAR);
       act("@C$n@w tries to light $p@w but nothing happens.", TRUE, ch, obj, 0, TO_ROOM);
      }
    }
    else {
