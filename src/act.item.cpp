...
ACMD(do_auction)
{
  ...
  if (PRF_FLAGGED(ch, PRF_HIDE)) {
	...
	else if (!(obj = get_obj_in_list_vis(ch, arg1, nullptr, ch->carrying))) {
  ...
  if (!strcasecmp(arg, "list")) {
   ...
   for (obj = world[auct_room].contents; obj; obj = next_obj) {
     ...
     if (obj) {
      ...
      send_to_char(ch, "@D[@R#@W%3d@D][@mOwner@W: @w%10s@D][@GItem Name@W: @w%-*s@D][@GCost@W: @Y%s@D]@n\r\n", list, get_name_by_id(GET_AUCTER(obj)) != nullptr ? CAP(get_name_by_id(GET_AUCTER(obj))) : "Nobody", count_color_chars(obj->short_description)+30, obj->short_description, add_commas(GET_BID(obj)));
      } else if (GET_AUCTIME(obj) + 86400 > time(nullptr) && GET_CURBID(obj) > -1) {
      send_to_char(ch, "@D[@R#@W%3d@D][@mOwner@W: @w%10s@D][@GItem Name@W: @w%-*s@D][@RTop Bid@W: %s @Y%s@D]@n\r\n", list, get_name_by_id(GET_AUCTER(obj)) != nullptr ? CAP(get_name_by_id(GET_AUCTER(obj))) : "Nobody", count_color_chars(obj->short_description)+30, obj->short_description, get_name_by_id(GET_CURBID(obj)) != nullptr ? CAP(get_name_by_id(GET_CURBID(obj))) : "Nobody", add_commas(GET_BID(obj)));
      } else if (GET_AUCTIME(obj) + 86400 < time(nullptr) && GET_CURBID(obj) > -1) {
...
ACMD(do_put)
{
  ...
  if (!*theobj)
	...
	if (!(obj = get_obj_in_list_vis(ch, theobj, nullptr, ch->carrying)))
...
static void get_from_room(struct char_data *ch, char *arg, int howmany)
{
  ...
  if (find_exdesc(arg, world[IN_ROOM(ch)].ex_description) != nullptr) {
  ...
  if (dotmode == FIND_INDIV) {
   if ((descword = find_exdesc_keywords(arg, world[IN_ROOM(ch)].ex_description)) != nullptr)
...
ACMD(do_drink)
{
  ...
  if (!(temp = get_obj_in_list_vis(ch, arg, nullptr, ch->carrying))) {
    if (!(temp = get_obj_in_list_vis(ch, arg, nullptr, world[IN_ROOM(ch)].contents))) {
...
ACMD(do_eat)
{
  ...
  if (!(food = get_obj_in_list_vis(ch, arg, nullptr, ch->carrying))) {
    send_to_char(ch, "You don't seem to have %s %s.\r\n", AN(arg), arg);
    return;
  }
...
ACMD(do_pour)
{
  ...
  if (subcmd == SCMD_POUR) {
    ...
    if (!(from_obj = get_obj_in_list_vis(ch, arg1, nullptr, ch->carrying))) {
      send_to_char(ch, "You can't find it!\r\n");
      return;
    }
  ...
  if (subcmd == SCMD_FILL) {
    ...
    if (!(to_obj = get_obj_in_list_vis(ch, arg1, nullptr, ch->carrying))) {
      send_to_char(ch, "You can't find it!\r\n");
      return;
    }
  ...
  if (subcmd == SCMD_POUR) {	/* pour */
    ...
    if (!(to_obj = get_obj_in_list_vis(ch, arg2, nullptr, ch->carrying))) {
      send_to_char(ch, "You can't find it!\r\n");
      return;
    }
    if ((GET_OBJ_TYPE(to_obj) != ITEM_DRINKCON) &&
...
ACMD(do_wear)
{
  ...
  if (dotmode == FIND_ALL) {
    ...
    if (!(obj = get_obj_in_list_vis(ch, arg1, nullptr, ch->carrying)))
      send_to_char(ch, "You don't seem to have any %ss.\r\n", arg1);
...
{ 
  ...
  if (!(j = get_obj_in_list_vis(ch, arg, nullptr, world[IN_ROOM(ch)].contents)) && (!(j = get_obj_in_list_vis(ch, arg, nullptr, ch->carrying)))) {
