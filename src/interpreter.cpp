	  ...
	  */
        ...
        case CON_NAME_CNFRM:        /* wait for conf. of new name    */
            if (UPPER(*arg) == 'Y') {
                if (isbanned(d->host) >= BAN_NEW) {
                    mudlog(NRM, ADMLVL_GOD, true, "Request for new char %s denied from [%s] (siteban)",
                ...
                if (circle_restrict) {
                    ...
                    mudlog(NRM, ADMLVL_GOD, true, "Request for new char %s denied from [%s] (wizlock)",
                           GET_PC_NAME(d->character), d->host);
                    STATE(d) = CON_CLOSE;
                    return;
                }
        ...
        case CON_GET_USER:
            ...
            if (!*arg) {
                ...
                return;
            } else if (strstr(arg, "1") || strstr(arg, "2") || strstr(arg, "3") || strstr(arg, "4") ||
                       strstr(arg, "5") || strstr(arg, "6") || strstr(arg, "7") || strstr(arg, "8") ||
                       strstr(arg, "9") || strstr(arg, "0")) {
                write_to_output(d, "No numbers. Username?\r\n");
                return;
            } else if (strstr(arg, ".") || strstr(arg, ",") || strstr(arg, "!") || strstr(arg, "?") ||
        ...
        case CON_UMENU:
            ...
            if (!strcasecmp(arg, "Q")) {
                ...
                write_to_output(d,
                                "Are you sure you want to delete your user file and all its characters? Yes or no:\n");
                STATE(d) = CON_DELCNF1;
        ...
        case CON_QSEX:        /* query sex of new user         */
            ...
            if (IS_HUMAN(d->character) || IS_SAIYAN(d->character) || IS_KONATSU(d->character) ||
                IS_MUTANT(d->character) || IS_ANDROID(d->character) || IS_KAI(d->character) ||
        ...
        case CON_RACE_HELP:
            ...
            if (IS_HALFBREED(d->character)) {
                ...
                write_to_output(d,
                                "@cThis controls how others first view you and whether you start with\na tail and how fast it regrows when missing.\r\n");
                write_to_output(d, "@D---------------------------------------@n\r\n");
        ...
        case CON_Q3:
            ...
            write_to_output(d,
                            "@B3@W)@C Hmm. With more money I can live better, certainly that is worth the time.@n\r\n");
        ...
        case CON_QX:
            ...
            write_to_output(d, "@cCurrent Alignment@D: %s%s@n\r\n",
                            GET_ALIGNMENT(d->character) < -50 ? "@R" : (GET_ALIGNMENT(d->character) > 50 ? "@C" : "@G"),
        ...
        case CON_GENOME:
            if (IS_MUTANT(d->character)) {
                ...
                write_to_output(d, "@B8@w) @CKonatsu @c- @C40%s higher chance to multihit on physical attacks.\r\n",
                                "%");
                write_to_output(d, "@D----[@gGenome 1@W: @G%s@D]----[@gGenome 2@W: @G%s@D]----",
                                display_genome[d->character->genome[0]], display_genome[d->character->genome[1]]);
        ...
        case CON_ANDROID:
            switch (*arg) {
                case '1':
                    ...
                    write_to_output(d, "@B3@W)@C Search for something magical to increase my strength.@n\r\n");
                    write_to_output(d,
                                    "@B4@W)@C Practice my favorite skills instead of working just on my body.@n\r\n");
                    write_to_output(d, "@B5@W)@C Spar with a friend so we can both improve our abilities.@n\r\n");
                    write_to_output(d, "@w\r\nMake a selection:@n\r\n");
                    STATE(d) = CON_Q1;
                    break;
            ...
            switch (*arg) {
                ...
                case '1':
                    if (lockRead(GET_NAME(d->character)) && d->level <= 0) {
                        ...
                        if (!strcasecmp(GET_NAME(d->character), "Codezan") ||
                            !strcasecmp(GET_NAME(d->character), "codezan")) {
