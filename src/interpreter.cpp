...
static void display_races_sub(descriptor_data *d) {
  ...
  for (const auto &r : v_races)
    send_to_char(d->character, "@C%-15s@D[@R%i RPP@D]@n%s",
                 r.second->getName().c_str(), r.second->getRPPCost(),
                 !(++i % 2) ? "\r\n" : "   ");
}
...
static void display_classes_sub(descriptor_data *d) {
  ...
  for (const auto &s : v_classes)
    send_to_char(d->character, "@C%s@n%s", s.second->getName().c_str(),
                 !(++i % 2) ? "\r\n" : "	");
}
...
int perform_dupe_check(struct descriptor_data *d) {
  ...
  for (k = descriptor_list; k; k = next_k) {
    ...
    if (k->original && (GET_IDNUM(k->original) == id)) {
      /* Original descriptor was switched, booting it and restoring normal body
       * control. */
      ...
      /* Character taking over their own body, while an immortal was switched to
       * it. */
  ...
  case RECON:
    ...
    mudlog(NRM, MAX(ADMLVL_NONE, GET_INVIS_LEV(d->character)), TRUE,
    ...
    if (GET_ADMLEVEL(d->character) >= 1 &&
        BOARDNEWIMM > GET_BOARD(d->character, 1))
      send_to_char(d->character, "\r\n@GMake sure to check the immortal board, "
                                 "there is a new post there.@n\r\n");
    if (GET_ADMLEVEL(d->character) >= 1 &&
        BOARDNEWCOD > GET_BOARD(d->character, 2))
      send_to_char(d->character, "\r\n@GMake sure to check the request file, "
                                 "it has been updated.@n\r\n");
    if (GET_ADMLEVEL(d->character) >= 1 &&
        BOARDNEWBUI > GET_BOARD(d->character, 4))
      send_to_char(d->character, "\r\n@GMake sure to check the builder board, "
                                 "there is a new post there.@n\r\n");
    if (GET_ADMLEVEL(d->character) >= 1 &&
        BOARDNEWDUO > GET_BOARD(d->character, 3))
      send_to_char(d->character, "\r\n@GMake sure to check punishment board, "
                                 "there is a new post there.@n\r\n");
...
void payout(int num) {
  ...
  for (k = descriptor_list; k; k = k->next) {
    if (GET_ADMLEVEL(k->character) <= 0 && IS_PLAYING(k) &&
      ...
      if (num == 0) {
        ...
        send_to_char(k->character,
                     "@D[@G+ 1 RPP@D] @cA total logon count within 4 of the "
                     "highest has been achieved.@n\r\n");
      } else if (num == 1) {
        k->rpp += 2;
        GET_RP(k->character) = k->rpp;
        GET_TRP(k->character) += 2;
        userWrite(k, 0, 0, 0, "index");
        send_to_char(k->character, "@D[@G+ 2 RPP@D] @cThe total logon count "
                                   "has tied with the highest ever.@n\r\n");
      } else {
        k->rpp += 3;
        GET_RP(k->character) = k->rpp;
        GET_TRP(k->character) += 3;
        userWrite(k, 0, 0, 0, "index");
        send_to_char(k->character, "@D[@G+ 3 RPP@D] @cA new logon count record "
                                   "has been achieved!@n\r\n");
      }
      GET_RTIME(k->character) = LASTPAYOUT;
    }
...
void userWrite(struct descriptor_data *d, int setTot, int setRpp, int setRBank,
               char *name) {
  char fname[40];
  FILE *fl;
...
void userRead(struct descriptor_data *d) {
  ...
  write_to_output(
      d, "                @B(@WB@B) @CBuy New C. Slot @D(@R15 RPP@D)@n\n");
...
void display_bonus_menu(struct char_data *ch, int type) {
  ...
  const char *bonus[] = {
      "Thrifty     - -10% Shop Buy Cost and +10% Shop Sell Cost             "
      "@D[@G-2pts @D]", /* Bonus 0 */
      "Prodigy     - +25% Experience Gained Until Level 80                  "
      "@D[@G-5pts @D]", /* Bonus 1 */
      "Quick Study - Character auto-trains skills faster                    "
      "@D[@G-3pts @D]", /* Bonus 2 */
      "Die Hard    - Life Force's PL regen doubled, but cost is the same    "
      "@D[@G-6pts @D]", /* Bonus 3 */
      "Brawler     - Physical attacks do 20% more damage                    "
      "@D[@G-4pts @D]", /* Bonus 4 */
      "Destroyer   - Damaged Rooms act as regen rooms for you               "
      "@D[@G-3pts @D]", /* Bonus 5 */
      "Hard Worker - Physical rewards better + activity drains less stamina "
      "@D[@G-3pts @D]", /* Bonus 6 */
      "Healer      - Heal/First-aid/Vigor/Repair restore +10%               "
      "@D[@G-3pts @D]", /* Bonus 7 */
      "Loyal       - +20% Experience When Grouped As Follower               "
      "@D[@G-2pts @D]", /* Bonus 8 */
      "Brawny      - Strength gains +2 every 10 levels, Train STR + 75%     "
      "@D[@G-5pts @D]", /* Bonus 9 */
      "Scholarly   - Intelligence gains +2 every 10 levels, Train INT + 75% "
      "@D[@G-5pts @D]", /* Bonus 10 */
      "Sage        - Wisdom gains +2 every 10 levels, Train WIS + 75%       "
      "@D[@G-5pts @D]", /* Bonus 11 */
      "Agile       - Agility gains +2 every 10 levels, Train AGL + 75%      "
      "@D[@G-4pts @D]", /* Bonus 12 */
      "Quick       - Speed gains +2 every 10 levels, Train SPD + 75%        "
      "@D[@G-6pts @D]", /* Bonus 13 */
      "Sturdy      - Constitution +2 every 10 levels, Train CON + 75%       "
      "@D[@G-5pts @D]", /* Bonus 14 */
      "Thick Skin  - -20% Physical and -10% ki dmg received                 "
      "@D[@G-5pts @D]", /* Bonus 15 */
      "Recipe Int. - Food cooked by you lasts longer/heals better           "
      "@D[@G-2pts @D]", /* Bonus 16 */
      "Fireproof   - -50% Fire Dmg taken, -10% ki, immunity to burn         "
      "@D[@G-4pts @D]", /* Bonus 17 */
      "Powerhitter - 15% critical hits will be x4 instead of x2             "
      "@D[@G-4pts @D]", /* Bonus 18 */
      "Healthy     - 40% chance to recover from ill effects when sleeping   "
      "@D[@G-3pts @D]", /* Bonus  19 */
      "Insomniac   - Can't Sleep. Immune to yoikominminken and paralysis    "
      "@D[@G-2pts @D]", /* Bonus  20 */
      "Evasive     - +15% to dodge rolls                                    "
      "@D[@G-3pts @D]", /* Bonus  21 */
      "The Wall    - +20% chance to block                                   "
      "@D[@G-3pts @D]", /* Bonus  22 */
      "Accurate    - +20% chance to hit physical, +10% to hit with ki       "
      "@D[@G-4pts @D]", /* Bonus  23 */
      "Energy Leech- -2% ki damage received for every 5 character levels,   "
      "@D[@G-5pts @D]\n                  @cas long as you can take that ki to "
      "your charge pool.@D        ", /* Bonus  24*/
      "Good Memory - +2 Skill Slots initially, +1 every 20 levels after     "
      "@D[@G-6pts @D]", /* Bonus 25 */
      "Soft Touch  - Half damage for all hit locations                      "
      "@D[@G+5pts @D]", /* Neg 26 */
      "Late Sleeper- Can only wake automatically. 33% every hour if maxed   "
      "@D[@G+5pts @D]", /* Neg 27 */
      "Impulse Shop- +25% shop costs                                        "
      "@D[@G+3pts @D]", /* Neg 28 */
      "Sickly      - Suffer from harmful effects longer                     "
      "@D[@G+5pts @D]", /* Neg 29 */
      "Punching Bag- -15% to dodge rolls                                    "
      "@D[@G+3pts @D]", /* Neg 30 */
      "Pushover    - -20% block chance                                      "
      "@D[@G+3pts @D]", /* Neg 31 */
      "Poor D. Perc- -20% chance to hit with physical, -10% with ki         "
      "@D[@G+4pts @D]", /* Neg 32 */
      "Thin Skin   - +20% physical and +10% ki damage received              "
      "@D[@G+4pts @D]", /* Neg 33 */
      "Fireprone   - +50% Fire Dmg taken, +10% ki, always burned            "
      "@D[@G+5pts @D]", /* Neg 34 */
      "Energy Int. - +2% ki damage received for every 5 character levels,   "
      "@D[@G+6pts @D]\n                  @rif you have ki charged you have 10% "
      "chance to lose   \n                  it and to take 1/4th damage equal "
      "to it.@D                    ", /* Neg 35 */
      "Coward      - Can't Attack Enemy With 150% Your Powerlevel           "
      "@D[@G+6pts @D]", /* Neg 36 */
      "Arrogant    - Cannot Suppress                                        "
      "@D[@G+1pt  @D]", /* Neg 37 */
      "Unfocused   - Charge concentration randomly breaks                   "
      "@D[@G+3pts @D]", /* Neg 38 */
      "Slacker     - Physical activity drains more stamina                  "
      "@D[@G+3pts @D]", /* Neg 39 */
      "Slow Learner- Character auto-trains skills slower                    "
      "@D[@G+3pts @D]", /* Neg 40 */
      "Masochistic - Defense Skills Cap At 75                               "
      "@D[@G+5pts @D]", /* Neg 41 */
      "Mute        - Can't use IC speech related commands                   "
      "@D[@G+4pts @D]", /* Neg 42 */
      "Wimp        - Strength is capped at 45                               "
      "@D[@G+6pts @D]", /* Neg 43 */
      "Dull        - Intelligence is capped at 45                           "
      "@D[@G+6pts @D]", /* Neg 44 */
      "Foolish     - Wisdom is capped at 45                                 "
      "@D[@G+6pts @D]", /* Neg 45 */
      "Clumsy      - Agility is capped at 45                                "
      "@D[@G+3pts @D]", /* Neg 46 */
      "Slow        - Speed is capped at 45                                  "
      "@D[@G+6pts @D]", /* Neg 47 */
      "Frail       - Constitution capped at 45                              "
      "@D[@G+4pts @D]", /* Neg 48 */
      "Sadistic    - Half Experience Gained For Quick Kills                 "
      "@D[@G+3pts @D]", /* Neg 49 */
      "Loner       - Can't Group, +5% Train gains, +10% to physical gains   "
      "@D[@G+2pts @D]", /* Neg 50 */
      "Bad Memory  - -5 Skill Slots                                         "
      "@D[@G+6pts @D]" /* Neg 51 */
  };

  if (type == 0) {
    send_to_char(ch,
                 "\r\n@YBonus Trait SELECTION "
                 "menu:\r\n@D---------------------------------------\r\n@n");
  ...
  send_to_char(ch,
               "@CX@D)@c Exit Traits Section and complete your character@n\n");
  send_to_char(ch,
               "@D---------------------------------------\n[@WCurrent Points "
               "Pool@W: @y%d@D] [@WPTS From Neg@W: @y%d@D]@w\n",
...
void exchange_ccpoints(struct char_data *ch, int value) {
  ...
  if (GET_BONUS(ch, value) > 0 &&
      GET_CCPOINTS(ch) - list_bonus_cost[value] < 0) {
    display_bonus_menu(ch, type);
    send_to_char(ch, "@RYou must unselect some bonus traits first.\r\n");
    return;
  } else if (GET_BONUS(ch, value) > 0 &&
             GET_CCPOINTS(ch) - list_bonus_cost[value] >= 0) {
  ...
  if (type == 0) {
    if (value > 25) {
      display_bonus_menu(ch, type);
      send_to_char(
          ch, "@RYou are not in the negatives menu, enter B to switch.\r\n");
      return;
    ...
    if (value < 26) {
      display_bonus_menu(ch, type);
      send_to_char(ch,
                   "@RYou are not in the bonuses menu, enter B to switch.\r\n");
      return;
    }
    ...
    if (list_bonus_cost[value] + count > 10) {
      display_bonus_menu(ch, type);
      send_to_char(
          ch, "@RYou can not have more than +10 points from negatives.\r\n");
      return;
    } else if (!opp_bonus(ch, value, type)) {
      return;
    } else {
...
void nanny(struct descriptor_data *d, char *arg) {
  ...
  struct {
    ...
    void (*func)(struct descriptor_data *, char *);
      ...
      {CON_OEDIT, oedit_parse},       {CON_IEDIT, oedit_parse},
      {CON_ZEDIT, zedit_parse},       {CON_SEDIT, sedit_parse},
      {CON_MEDIT, medit_parse},       {CON_REDIT, redit_parse},
      {CON_CEDIT, cedit_parse},       {CON_AEDIT, aedit_parse},
      {CON_TRIGEDIT, trigedit_parse}, {CON_ASSEDIT, assedit_parse},
      {CON_GEDIT, gedit_parse},       {CON_LEVELUP, levelup_parse},
      {CON_HEDIT, hedit_parse},       {CON_HSEDIT, hsedit_parse},
      {CON_POBJ, pobj_edit_parse},    {-1, NULL}};
  ...
  case CON_GET_NAME: /* wait for input of name */
    ...
    if (d->writenew > 0 &&
        (player_i = load_char(tmp_name, d->character)) > -1) {
      userRead(d);
        ...
        if (PLR_FLAGGED(d->character, PLR_DELETED)) {
          ...
          */
          ...
          if (!Valid_Name(tmp_name)) {
            write_to_output(
                d, "@YInvalid name@n, please try @Canother.@n\r\nName: ");
            return;
          }
          CREATE(d->character, struct char_data, 1);
          clear_char(d->character);
          ...
          strcpy(d->character->name,
                 CAP(tmp_name)); /* strcpy: OK (size checked above) */
          GET_PFILEPOS(d->character) = player_i;
          SET_BIT_AR(PRF_FLAGS(d->character), PRF_COLOR);
          display_races(d);
          d->character->rp = d->rpp;
          ...
          if (isbanned(d->host) == BAN_SELECT &&
            ...
            STATE(d) = CON_CLOSE;
            return;
          } else if (GET_LEVEL(d->character) < circle_restrict &&
                     circle_restrict < 101) {
            userRead(d);
            write_to_output(
                d,
                "The game is temporarily restricted to at least %d level.\r\n",
                circle_restrict);
            STATE(d) = CON_UMENU;
            mudlog(NRM, ADMLVL_GOD, TRUE,
                   "Request for character load denied for %s [%s] (wizlock)",
                   GET_NAME(d->character), d->host);
            return;
          }
  ...
  case CON_NAME_CNFRM: /* wait for conf. of new name    */
    if (UPPER(*arg) == 'Y') {
      if (isbanned(d->host) >= BAN_NEW) {
        ...
        write_to_output(
            d, "Sorry, new characters are not allowed from your site!\r\n");
        STATE(d) = CON_CLOSE;
        return;
      }
      if (circle_restrict) {
        write_to_output(
            d, "Sorry, new players can't be created at the moment.\r\n");
        mudlog(NRM, ADMLVL_GOD, TRUE,
      ...
      d->idle_tics = 0;
      write_to_output(
          d, "@MNew character.@n\r\nGive me a @gpassword@n for @C%s@n: ",
  ...
  case CON_GET_USER:
    ...
    if (!*arg) {
      write_to_output(d, "Enter your desired username or the username you have "
                         "already made.\nUsername?\r\n");
      return;
      ...
      return;
    } else if (strstr(arg, "1") || strstr(arg, "2") || strstr(arg, "3") ||
               strstr(arg, "4") || strstr(arg, "5") || strstr(arg, "6") ||
               strstr(arg, "7") || strstr(arg, "8") || strstr(arg, "9") ||
               strstr(arg, "0")) {
      write_to_output(d, "No numbers. Username?\r\n");
      return;
    } else if (strstr(arg, ".") || strstr(arg, ",") || strstr(arg, "!") ||
      ...
      write_to_output(d,
                      "Name must at least be 3 characters long, username?\r\n");
      return;
    } else if (strlen(arg) > 10) {
      write_to_output(d,
                      "Name must be at most 10 characters long, username?\r\n");
      return;
    } else {
      if (!readUserIndex(arg)) {
        if (circle_restrict == 101) {
          write_to_output(d, "The mud is locked for mortals at this time.\r\n");
          write_to_output(d, "Please check: www.advent-truth.com/forum for "
                             "further details.\r\n");
        ...
        if (circle_restrict == 101) {
          ...
          if (d->level == 0) {
            write_to_output(d,
                            "The mud is locked for mortals at this time.\r\n");
  ...
  case CON_USER_CONF:
    if (!*arg) {
      ...
      write_to_output(d, "User Account, %s, created.\r\nEnter Email:\n",
                      d->user);
      write_to_output(d, "Remember your email must be valid and matching the "
                         "example given.\n");
  ...
  case CON_GET_EMAIL:
    if (readUserIndex(d->user) && !*arg) {
      ...
      userRead(d);
      STATE(d) = CON_UMENU;
    } else if (!strstr(arg, ".com") && !strstr(arg, ".net") &&
               !strstr(arg, ".org")) {
      write_to_output(d, "Improper email format missing '.com' or '.net' or "
                         "'.org'. Email?\r\n");
      return;
      ...
      for (k = descriptor_list; k; k = k->next) {
        ...
        if (!strcasecmp(k->user, d->user)) {
          if (STATE(k) == CON_PLAYING) {
            ...
            write_to_output(k, "Your account has been usurped by someone who "
                               "knows its password!@n");
          } else {
            STATE(k) = CON_CLOSE;
            write_to_output(k, "Your account has been usurped by someone who "
                               "knows its password!@n");
  ...
  case CON_UMENU:
    ...
    if (!strcasecmp(arg, "Q")) {
      ...
      write_to_output(d, "Are you sure you want to delete your user file and "
                         "all its characters? Yes or no:\n");
      STATE(d) = CON_DELCNF1;
      ...
      if (d->total == 3 && d->rpp >= 15) {
        ...
        userRead(d);
        write_to_output(
            d, "You need at least 15 RPP to purchase a new character slot.\n");
        return;
      ...
      case 1:
        if (!strcasecmp(d->tmp1, "Empty")) {
          ...
          if (lockRead(d->tmp1) && d->level <= 0) {
            write_to_output(d, "That character has been locked out for rule "
                               "violations. Play another character.\n");
            return;
          } else {
  ...
  case CON_QSEX: /* query sex of new user         */
    ...
    if (IS_HUMAN(d->character) || IS_SAIYAN(d->character) ||
        IS_KONATSU(d->character) || IS_MUTANT(d->character) ||
        IS_ANDROID(d->character) || IS_KAI(d->character) ||
        IS_HALFBREED(d->character) || IS_TRUFFLE(d->character) ||
        (IS_HOSHIJIN(d->character) && IS_FEMALE(d->character))) {
  ...
  case CON_RACE_HELP:
    ...
    if (IS_HALFBREED(d->character)) {
      write_to_output(d,
                      "@YWhat race do you prefer to by identified with?\r\n");
      write_to_output(
          d, "@cThis controls how others first view you and whether you start "
      ...
      write_to_output(
          d, "@YWhat do you want to be identified as at first glance?\r\n");
      write_to_output(d, "@D---------------------------------------@n\r\n");
      write_to_output(
  ...
  case CON_HAIRL: /* query hair length */
    if (IS_HUMAN(d->character) || IS_SAIYAN(d->character) ||
      ...
      if (d->character->hairl == HAIRL_BALD) {
        ...
        write_to_output(d, "@D---------------------------------------@n\r\n");
        write_to_output(d,
                        "@B1@W)@C White  @B2@W)@C Black  @B3@W)@C Green\r\n");
        write_to_output(d,
                        "@B4@W)@C Orange @B5@W)@C Yellow @B6@W)@C Red@n\r\n");
        write_to_output(d, "@B7@W)@C Grey   @B8@W)@C Blue   @B9@W)@C Aqua\r\n");
        write_to_output(d,
                        "@BA@W)@C Pink   @BB@W)@C Purple @BC@W)@C Tan@n\r\n");
        write_to_output(d, "@w\r\nMake a selection:@n\r\n");
        STATE(d) = CON_SKIN;
      } else {
        ...
        write_to_output(d, "@D---------------------------------------@n\r\n");
        write_to_output(d,
                        "@B1@W)@C Black  @B2@W)@C Brown  @B3@W)@C Blonde\r\n");
        write_to_output(
            d, "@B4@W)@C Grey   @B5@W)@C Red    @B6@W)@C Orange@n\r\n");
        write_to_output(d, "@B7@W)@C Green  @B8@W)@C Blue   @B9@W)@C Pink\r\n");
        write_to_output(
            d, "@BA@W)@C Purple @BB@W)@C Silver @BC@W)@C Crimson@n\r\n");
      ...
      if (IS_MAJIN(d->character) || IS_NAMEK(d->character) ||
          IS_ARLIAN(d->character)) {
        switch (*arg) {
        case '1':
          d->character->hairl = HAIRL_BALD;
          break;
        ...
        if (IS_ARLIAN(d->character) && IS_FEMALE(d->character)) {
          ...
          write_to_output(d, "@D---------------------------------------@n\r\n");
          write_to_output(
              d, "@B1@W)@C Black  @B2@W)@C Brown  @B3@W)@C Blonde\r\n");
          write_to_output(
              d, "@B4@W)@C Grey   @B5@W)@C Red    @B6@W)@C Orange@n\r\n");
          write_to_output(d,
                          "@B7@W)@C Green  @B8@W)@C Blue   @B9@W)@C Pink\r\n");
          write_to_output(
              d, "@BA@W)@C Purple @BB@W)@C Silver @BC@W)@C Crimson@n\r\n");
          write_to_output(d, "@BD@W)@C White@n\r\n");
          write_to_output(d, "@w\r\nMake a selection:@n\r\n");
          ...
          write_to_output(d,
                          "@B4@W)@C Orange @B5@W)@C Yellow @B6@W)@C Red@n\r\n");
          write_to_output(d,
                          "@B7@W)@C Grey   @B8@W)@C Blue   @B9@W)@C Aqua\r\n");
          write_to_output(d,
                          "@BA@W)@C Pink   @BB@W)@C Purple @BC@W)@C Tan@n\r\n");
          write_to_output(d, "@w\r\nMake a selection:@n\r\n");
          STATE(d) = CON_SKIN;
  ...
  case CON_HAIRC: /* query hair color */
    ...
    if (IS_ARLIAN(d->character)) {
      ...
      write_to_output(d, "@D---------------------------------------@n\r\n");
      write_to_output(
          d, "@B1@W)@C Plain     @B2@W)@C Mohawk    @B3@W)@C Spiky\r\n");
      write_to_output(
          d, "@B4@W)@C Curly     @B5@W)@C Uneven    @B6@W)@C Ponytail@n\r\n");
      write_to_output(
          d, "@B7@W)@C Afro      @B8@W)@C Fade      @B9@W)@C Crew Cut\r\n");
  ...
  case CON_EYE:
    ...
    write_to_output(
        d, "@YWhat do you want to be your most distinguishing feature:\r\n");
    write_to_output(d, "@D---------------------------------------@n\r\n");
  ...
  case CON_HW:
    ...
    case '1':
      if (!IS_TRUFFLE(d->character) && !IS_ICER(d->character)) {
        char_stat_set(d->character, "height", rand_number(100, 120));
        char_stat_set(d->character, "weight", rand_number(25, 30));
      } else if (IS_ICER(d->character)) {
        char_stat_set(d->character, "height", rand_number(100, 120));
        char_stat_set(d->character, "weight", rand_number(25, 30));
      } else {
        char_stat_set(d->character, "height", rand_number(20, 35));
        char_stat_set(d->character, "weight", rand_number(5, 8));
      }
      break;
    case '2':
      if (!IS_TRUFFLE(d->character) && !IS_ICER(d->character)) {
        char_stat_set(d->character, "height", rand_number(120, 140));
        char_stat_set(d->character, "weight", rand_number(30, 35));
      } else if (IS_ICER(d->character)) {
        char_stat_set(d->character, "height", rand_number(120, 140));
        char_stat_set(d->character, "weight", rand_number(30, 35));
      } else {
        char_stat_set(d->character, "height", rand_number(35, 40));
        char_stat_set(d->character, "weight", rand_number(8, 10));
      }
      break;
    case '3':
      if (!IS_TRUFFLE(d->character) && !IS_ICER(d->character)) {
        char_stat_set(d->character, "height", rand_number(140, 160));
        char_stat_set(d->character, "weight", rand_number(35, 45));
      } else if (IS_ICER(d->character)) {
        char_stat_set(d->character, "height", rand_number(140, 160));
        char_stat_set(d->character, "weight", rand_number(35, 45));
      } else {
        char_stat_set(d->character, "height", rand_number(40, 50));
        char_stat_set(d->character, "weight", rand_number(10, 12));
      }
      break;
    case '4':
      if (!IS_TRUFFLE(d->character) && !IS_ICER(d->character)) {
        char_stat_set(d->character, "height", rand_number(160, 180));
        char_stat_set(d->character, "weight", rand_number(45, 60));
      } else if (IS_ICER(d->character)) {
        write_to_output(d, "That is not an acceptable option.\r\n");
        return;
      } else {
        char_stat_set(d->character, "height", rand_number(50, 60));
        char_stat_set(d->character, "weight", rand_number(12, 15));
      }
      break;
    case '5':
      if (!IS_TRUFFLE(d->character) && !IS_ICER(d->character)) {
        char_stat_set(d->character, "height", rand_number(180, 200));
        char_stat_set(d->character, "weight", rand_number(60, 80));
      } else if (IS_ICER(d->character)) {
        write_to_output(d, "That is not an acceptable option.\r\n");
        return;
      } else {
        char_stat_set(d->character, "height", rand_number(60, 70));
        char_stat_set(d->character, "weight", rand_number(15, 18));
      }
      break;
    case '6':
      if (!IS_TRUFFLE(d->character) && !IS_ICER(d->character)) {
        char_stat_set(d->character, "height", rand_number(200, 220));
        char_stat_set(d->character, "weight", rand_number(80, 100));
      } else {
        write_to_output(d, "That is not an acceptable option.\r\n");
        return;
      }
  ...
  case CON_Q1:
    char_stat_set(d->character, "powerlevel", rand_number(30, 50));
    char_stat_set(d->character, "stamina", rand_number(30, 50));
    char_stat_set(d->character, "ki", rand_number(30, 50));
    if (IS_SAIYAN(d->character)) {
      char_stat_set(d->character, "strength", rand_number(12, 18));
      char_stat_set(d->character, "constitution", rand_number(12, 18));
      char_stat_set(d->character, "wisdom", rand_number(8, 16));
      char_stat_set(d->character, "intelligence", rand_number(8, 14));
      char_stat_set(d->character, "speed", rand_number(8, 18));
      char_stat_set(d->character, "agility", rand_number(8, 16));
    } else if (IS_HALFBREED(d->character)) {
      char_stat_set(d->character, "strength", rand_number(10, 18));
      char_stat_set(d->character, "constitution", rand_number(10, 18));
      char_stat_set(d->character, "wisdom", rand_number(8, 18));
      char_stat_set(d->character, "intelligence", rand_number(8, 18));
      char_stat_set(d->character, "speed", rand_number(8, 18));
      char_stat_set(d->character, "agility", rand_number(8, 18));
    } else if (IS_HUMAN(d->character)) {
      char_stat_set(d->character, "strength", rand_number(8, 18));
      char_stat_set(d->character, "constitution", rand_number(8, 18));
      char_stat_set(d->character, "wisdom", rand_number(10, 18));
      char_stat_set(d->character, "intelligence", rand_number(12, 18));
      char_stat_set(d->character, "speed", rand_number(8, 18));
      char_stat_set(d->character, "agility", rand_number(8, 18));
    } else if (IS_HOSHIJIN(d->character)) {
      char_stat_set(d->character, "strength", rand_number(10, 18));
      char_stat_set(d->character, "constitution", rand_number(9, 18));
      char_stat_set(d->character, "wisdom", rand_number(9, 18));
      char_stat_set(d->character, "intelligence", rand_number(9, 18));
      char_stat_set(d->character, "speed", rand_number(10, 18));
      char_stat_set(d->character, "agility", rand_number(9, 18));
    } else if (IS_NAMEK(d->character)) {
      char_stat_set(d->character, "strength", rand_number(9, 18));
      char_stat_set(d->character, "constitution", rand_number(9, 18));
      char_stat_set(d->character, "wisdom", rand_number(12, 18));
      char_stat_set(d->character, "intelligence", rand_number(8, 18));
      char_stat_set(d->character, "speed", rand_number(8, 18));
      char_stat_set(d->character, "agility", rand_number(8, 18));
    } else if (IS_ARLIAN(d->character)) {
      char_stat_set(d->character, "strength", rand_number(15, 20));
      char_stat_set(d->character, "constitution", rand_number(15, 20));
      char_stat_set(d->character, "wisdom", rand_number(8, 16));
      char_stat_set(d->character, "intelligence", rand_number(8, 16));
      char_stat_set(d->character, "speed", rand_number(8, 18));
      char_stat_set(d->character, "agility", rand_number(8, 18));
    } else if (IS_ANDROID(d->character)) {
      char_stat_set(d->character, "strength", rand_number(12, 18));
      ...
      char_stat_set(d->character, "strength", rand_number(14, 18));
      char_stat_set(d->character, "constitution", rand_number(8, 18));
      char_stat_set(d->character, "wisdom", rand_number(8, 18));
      char_stat_set(d->character, "intelligence", rand_number(8, 18));
      char_stat_set(d->character, "speed", rand_number(8, 18));
      char_stat_set(d->character, "agility", rand_number(8, 14));
    } else if (IS_MAJIN(d->character)) {
      char_stat_set(d->character, "strength", rand_number(11, 18));
      char_stat_set(d->character, "constitution", rand_number(14, 18));
      char_stat_set(d->character, "wisdom", rand_number(8, 14));
      char_stat_set(d->character, "intelligence", rand_number(8, 14));
      char_stat_set(d->character, "speed", rand_number(8, 18));
      char_stat_set(d->character, "agility", rand_number(8, 17));
    } else if (IS_TRUFFLE(d->character)) {
      char_stat_set(d->character, "strength", rand_number(8, 14));
      char_stat_set(d->character, "constitution", rand_number(8, 14));
      char_stat_set(d->character, "wisdom", rand_number(8, 18));
      char_stat_set(d->character, "intelligence", rand_number(14, 18));
      char_stat_set(d->character, "speed", rand_number(8, 18));
      char_stat_set(d->character, "agility", rand_number(8, 18));
    } else if (IS_KAI(d->character)) {
      char_stat_set(d->character, "strength", rand_number(9, 18));
      char_stat_set(d->character, "constitution", rand_number(8, 18));
      char_stat_set(d->character, "wisdom", rand_number(14, 18));
      char_stat_set(d->character, "intelligence", rand_number(10, 18));
      char_stat_set(d->character, "speed", rand_number(8, 18));
      char_stat_set(d->character, "agility", rand_number(8, 18));
    } else if (IS_ICER(d->character)) {
      char_stat_set(d->character, "strength", rand_number(10, 18));
      ...
      char_stat_set(d->character, "wisdom", rand_number(8, 18));
      char_stat_set(d->character, "intelligence", rand_number(8, 18));
      char_stat_set(d->character, "speed", rand_number(8, 15));
      char_stat_set(d->character, "agility", rand_number(8, 18));
      ...
      char_stat_set(d->character, "wisdom", rand_number(9, 18));
      char_stat_set(d->character, "intelligence", rand_number(9, 18));
      char_stat_set(d->character, "speed", rand_number(9, 18));
      char_stat_set(d->character, "agility", rand_number(9, 18));
    } else if (IS_KANASSAN(d->character)) {
      char_stat_set(d->character, "strength", rand_number(8, 16));
      char_stat_set(d->character, "constitution", rand_number(8, 16));
      char_stat_set(d->character, "wisdom", rand_number(12, 18));
      char_stat_set(d->character, "intelligence", rand_number(12, 18));
      char_stat_set(d->character, "speed", rand_number(8, 18));
      char_stat_set(d->character, "agility", rand_number(8, 18));
      ...
      char_stat_set(d->character, "strength", rand_number(10, 14));
      char_stat_set(d->character, "constitution", rand_number(10, 14));
      char_stat_set(d->character, "wisdom", rand_number(10, 16));
      char_stat_set(d->character, "intelligence", rand_number(10, 14));
      char_stat_set(d->character, "speed", rand_number(12, 18));
      char_stat_set(d->character, "agility", rand_number(14, 18));
    }

    switch (*arg) {
    case '1':
      char_stat_mod(d->character, "powerlevel",
                    roll_stats(d->character, 5, 25));
      char_stat_mod(d->character, "stamina", roll_stats(d->character, 8, 50));
      char_stat_mod(d->character, "ki", roll_stats(d->character, 6, 50));
      break;
    case '2':
      char_stat_mod(d->character, "powerlevel",
                    roll_stats(d->character, 5, 55));
      char_stat_mod(d->character, "stamina", roll_stats(d->character, 8, 40));
      char_stat_mod(d->character, "ki", roll_stats(d->character, 6, 40));
      break;
    case '3':
      char_stat_mod(d->character, "powerlevel",
                    roll_stats(d->character, 5, 125));
      char_stat_mod(d->character, "stamina", roll_stats(d->character, 8, 50));
      char_stat_mod(d->character, "ki", roll_stats(d->character, 6, 40));
      break;
    case '4':
      char_stat_mod(d->character, "powerlevel",
                    roll_stats(d->character, 5, 65));
      char_stat_mod(d->character, "stamina", roll_stats(d->character, 8, 65));
      char_stat_mod(d->character, "ki", roll_stats(d->character, 6, 65));
      SET_BIT_AR(PLR_FLAGS(d->character), PLR_SKILLP);
      break;
    case '5':
      char_stat_mod(d->character, "powerlevel",
                    roll_stats(d->character, 5, 75));
      char_stat_mod(d->character, "stamina", roll_stats(d->character, 8, 100));
      char_stat_mod(d->character, "ki", roll_stats(d->character, 6, 75));
      break;
    default:
      write_to_output(d, "That is not an acceptable option.\r\n");
      return;
    ...
    write_to_output(
        d, "@wYou are faced with the strongest opponent you have ever\r\nfaced "
           "in your life. You both have beat each other to the\r\nlimits of "
           "both your strengths. A situation has presented \r\nan opportunity "
           "to win the fight, what do you do?\r\n");
    write_to_output(d, "@D---------------------------------------@n\r\n");
    write_to_output(
        d, "@B1@W)@C Kill my opponent in a very brutal fashion!@n\r\n");
    write_to_output(
        d, "@B2@W)@C Disable my opponent, but spare their life.@n\r\n");
    write_to_output(d,
                    "@B3@W)@C Kill my opponent, I have no other choice.@n\r\n");
    write_to_output(
        d, "@B4@W)@C Try to evade my opponent till I can get away.@n\r\n");
    write_to_output(
        d,
        "@B5@W)@C Take their head clean off and bathe in their blood!@n\r\n");
    write_to_output(d, "@w\r\nMake a selection:@n\r\n");
    STATE(d) = CON_Q2;
    break;
  ...
  case CON_Q2:
    switch (*arg) {
    case '1':
      char_stat_mod(d->character, "alignment", -200);
      break;
    case '2':
      char_stat_mod(d->character, "alignment", 100);
      break;
    case '3':
      char_stat_mod(d->character, "alignment", 10);
      break;
    case '4':
      char_stat_mod(d->character, "alignment", 0);
      break;
    case '5':
      char_stat_mod(d->character, "alignment", -400);
      break;
    default:
      write_to_output(d, "That is not an acceptable option.\r\n");
      return;
    ...
    write_to_output(d, "@YAnswer the following question:\r\n");
    write_to_output(
        d, "@wYou are one day offered a means to gain incredible "
           "strength\r\nby some extraordinary means. The only problem is it "
           "requires\r\nthe lives of innocents to obtain. What do you do?\r\n");
    write_to_output(d, "@D---------------------------------------@n\r\n");
    write_to_output(
        d, "@B1@W)@C Hell yeah I take the opportunity to get stronger!@n\r\n");
    write_to_output(d, "@B2@W)@C I refuse to gain unnatural strength.@n\r\n");
    write_to_output(d, "@B3@W)@C I refuse to gain strength at the cost of the "
                       "innocent.@n\r\n");
    write_to_output(
        d,
        "@B4@W)@C I kill that many innocents before breakfast anyway...@n\r\n");
    write_to_output(d, "@w\r\nMake a selection:@n\r\n");
    STATE(d) = CON_Q3;
    break;
  case CON_Q3:
    ...
    case '1':
      char_stat_mod(d->character, "alignment", -100);
      char_stat_mod(d->character, "powerlevel", 100);
      char_stat_mod(d->character, "stamina", 80);
      char_stat_mod(d->character, "ki", 10);
      break;
    case '2':
      char_stat_mod(d->character, "alignment", 10);
      char_stat_mod(d->character, "powerlevel", 25);
      char_stat_mod(d->character, "stamina", 25);
      char_stat_mod(d->character, "ki", 25);
      break;
    case '3':
      char_stat_mod(d->character, "alignment", 50);
      char_stat_mod(d->character, "powerlevel", 20);
      char_stat_mod(d->character, "stamina", 20);
      char_stat_mod(d->character, "ki", 20);
      break;
    case '4':
      char_stat_mod(d->character, "alignment", -200);
      char_stat_mod(d->character, "powerlevel", 100);
      char_stat_mod(d->character, "stamina", 100);
      char_stat_mod(d->character, "ki", 100);
      break;
    default:
      write_to_output(d, "That is not an acceptable option.\r\n");
      return;
    ...
    write_to_output(d, "@YAnswer the following question:\r\n");
    write_to_output(
        d, "@wOne day you are offered a way to make a lot of money, but in "
           "order\r\nto do so you will need to stop training for a whole month "
           "to\r\nhandle business. What do you do?\r\n");
    write_to_output(d, "@D---------------------------------------@n\r\n");
    write_to_output(d, "@B1@W)@C I take the opportunity, with more money I can "
                       "train better later.@n\r\n");
    write_to_output(d, "@B2@W)@C I refuse to waste my time. What I need is "
                       "some nice hard training.@n\r\n");
    write_to_output(d, "@B3@W)@C Hmm. With more money I can live better, "
                       "certainly that is worth the time.@n\r\n");
    write_to_output(d, "@B4@W)@C I choose to earn a little money while still "
                       "training instead.@n\r\n");
    write_to_output(d, "@w\r\nMake a selection:@n\r\n");
    STATE(d) = CON_Q4;
    break;
  ...
  case CON_Q4:
    switch (*arg) {
    case '1':
      char_stat_mod(d->character, "money", 1000);
      char_stat_mod(d->character, "powerlevel", -rand_number(10, 30));
      char_stat_mod(d->character, "stamina", -rand_number(10, 30));
      char_stat_mod(d->character, "ki", -rand_number(10, 30));
      break;
    case '2':
      char_stat_set(d->character, "money", 0);
      char_stat_mod(d->character, "powerlevel", rand_number(50, 165));
      char_stat_mod(d->character, "stamina", rand_number(50, 165));
      char_stat_mod(d->character, "ki", rand_number(50, 165));
      break;
    case '3':
      char_stat_set(d->character, "money", 2500);
      char_stat_mod(d->character, "powerlevel", -rand_number(15, 25));
      char_stat_mod(d->character, "stamina", -rand_number(15, 25));
      char_stat_mod(d->character, "ki", -rand_number(15, 25));
      break;
    case '4':
      char_stat_set(d->character, "money", 150);
      char_stat_mod(d->character, "powerlevel", rand_number(25, 80));
      char_stat_mod(d->character, "stamina", rand_number(25, 80));
      char_stat_mod(d->character, "ki", rand_number(25, 80));
      break;
    default:
      write_to_output(d, "That is not an acceptable option.\r\n");
      return;
    ...
    write_to_output(d, "@YAnswer the following question:\r\n");
    write_to_output(d, "@wYou are introduced to a new way of training one day, "
                       "what do you do?\r\n");
    write_to_output(d, "@D---------------------------------------@n\r\n");
    ...
    write_to_output(
        d, "@B3@W)@C I will at least try it, for a little while.@n\r\n");
    write_to_output(
        d, "@B4@W)@C No way is superior to eating spinach everyday...@n\r\n");
    write_to_output(d, "@w\r\nMake a selection:@n\r\n");
    STATE(d) = CON_Q5;
    break;
  ...
  case CON_Q5:
    switch (*arg) {
    case '1':
      char_stat_mod(d->character, "powerlevel", rand_number(0, 40));
      break;
    case '2':
      char_stat_mod(d->character, "powerlevel", rand_number(-30, 80));
      break;
    case '3':
      char_stat_mod(d->character, "powerlevel", rand_number(-25, 60));
      break;
    case '4':
      char_stat_mod(d->character, "powerlevel", rand_number(0, 50));
      break;
    default:
      write_to_output(d, "That is not an acceptable option.\r\n");
      return;
    ...
    write_to_output(d, "@wYou have an enemy before you, what is your prefered "
                       "method to attack him?@n\r\n");
    write_to_output(d, "@D---------------------------------------@n\r\n");
    write_to_output(d, "@B1@W)@C I prefer to defend rather than attack.@n\r\n");
    write_to_output(d, "@B2@W)@C I prefer to throw a strong punch at the "
                       "enemy's throat.@n\r\n");
    write_to_output(d, "@B3@W)@C I prefer to send a devestating kick at the "
                       "enemy's neck.@n\r\n");
    write_to_output(
        d, "@B4@W)@C I prefer to smash them with a two-handed slam!@n\r\n");
    write_to_output(
        d, "@B5@W)@C I prefer to throw one of my many energy attacks!@n\r\n");
    write_to_output(d, "@w\r\nMake a selection:@n\r\n");
    STATE(d) = CON_Q6;
    break;
  ...
  case CON_Q6:
    switch (*arg) {
    case '1':
      char_stat_mod(d->character, "powerlevel", rand_number(0, 15));
      char_stat_mod(d->character, "stamina", rand_number(0, 15));
      d->character->choice = 1;
      break;
    case '2':
      char_stat_mod(d->character, "powerlevel", rand_number(0, 30));
    ...
    case '4':
      ...
      break;
    case '5':
      char_stat_mod(d->character, "ki", rand_number(0, 50));
    ...
    write_to_output(d, "\r\n@YAnswer the following question:\r\n");
    write_to_output(d, "@wYou are camped out one night in a field, the sky is "
                       "clear and the\r\nstars visible. Looking at them, what "
                       "thought crosses your mind?@n\r\n");
    write_to_output(d, "@D---------------------------------------@n\r\n");
    write_to_output(
        d, "@B1@W)@C One day, I am going to conquer every one of those.@n\r\n");
    write_to_output(d,
                    "@B2@W)@C I really wish I had brought that special someone "
                    "along as this is a great night for romance@n\r\n");
    write_to_output(d, "@B3@W)@C Those stars hold greater meaning to you than "
                       "just planets, they dictate\r\nyour life or guide you "
                       "and those arround you in one form or another.@n\r\n");
    write_to_output(d,
                    "@B4@W)@C You'd very much like to travel into space to get "
                    "to one of these stars to study all that it holds.@n\r\n");
    write_to_output(d, "@w\r\nMake a selection:@n\r\n");
    STATE(d) = CON_Q7;
    break;
  ...
  case CON_Q7:
    ...
    case '1':
      char_stat_mod(d->character, "alignment", -10);
      char_stat_mod(d->character, "strength", 1);
      break;
    case '2':
      char_stat_mod(d->character, "alignment", +10);
      char_stat_mod(d->character, "speed", 1);
      break;
    case '3':
      char_stat_mod(d->character, "wisdom", 1);
      break;
    case '4':
      char_stat_mod(d->character, "intelligence", 1);
      break;
    default:
      write_to_output(d, "That is not an acceptable option.\r\n");
      return;
    ...
    write_to_output(d, "\r\n@YAnswer the following question:\r\n");
    write_to_output(
        d, "@wOne day, you are on the way home. You happen to walk past "
           "two\r\nof your best friends about to lock horns. Do you@n\r\n");
    write_to_output(d, "@D---------------------------------------@n\r\n");
    write_to_output(
        d, "@B1@W)@C You try to talk them out of it. With a silver tongue, you "
           "can stop the fight before it has began.@n\r\n");
    write_to_output(
        d,
        "@B2@W)@C It's clear that they are quite chuffed about something, you "
        "could stop the fight now but they may just try again later. It's best "
        "to investigate and find out the source of the problem.@n\r\n");
    write_to_output(
        d,
        "@B3@W)@C Pick the one you like the most and help them to win.@n\r\n");
    write_to_output(
        d,
        "@B4@W)@C You try to sneak past them, reckoning no matter what you do "
        "there its a no win situation, so long as they don't see you.@n\r\n");
    write_to_output(d, "@w\r\nMake a selection:@n\r\n");
    STATE(d) = CON_Q8;
    break;
  ...
  case CON_Q8:
    ...
    case '1':
      ...
      break;
    case '2':
      char_stat_mod(d->character, "alignment", +20);
      char_stat_mod(d->character, "wisdom", 1);
      break;
    case '3':
      char_stat_mod(d->character, "alignment", -10);
    ...
    case '4':
      char_stat_mod(d->character, "alignment", -20);
      char_stat_mod(d->character, "agility", 1);
      break;
    default:
      write_to_output(d, "That is not an acceptable option.\r\n");
      return;
    ...
    write_to_output(
        d,
        "@wAs a kid you were confronted with this. On the way from the "
        "bakery,\r\na group of kids corner you in an alley and the leader "
        "demands that\r\nyou surrender your jam donut to him. Do you...@n\r\n");
    write_to_output(d, "@D---------------------------------------@n\r\n");
    write_to_output(d, "@B1@W)@C Throw the donut up in the air, hoping that "
                       "the leader will pay enough attention to it so that you "
                       "can get at least one good shot in.@n\r\n");
    write_to_output(d, "@B2@W)@C Surrender the donut for now but come back "
                       "later with your friends and gain your revenge.@n\r\n");
    write_to_output(
        d,
        "@B3@W)@C Give him the donut, then return to the baker. You tell him a "
        "sob story and convince him to give you a replacement donut.@n\r\n");
    write_to_output(d, "@B4@W)@C It's better just to do as he says and leave "
                       "it to that. After all its only a donut.@n\r\n");
    write_to_output(d, "@w\r\nMake a selection:@n\r\n");
    STATE(d) = CON_Q9;
    break;
  ...
  case CON_Q9:
    ...
    case '1':
      ...
      break;
    case '2':
      char_stat_mod(d->character, "alignment", -30);
      char_stat_mod(d->character, "wisdom", 1);
      break;
    case '3':
      char_stat_mod(d->character, "alignment", -10);
      ...
      break;
    case '4':
      char_stat_mod(d->character, "alignment", -5);
      char_stat_mod(d->character, "intelligence", 1);
      break;
    default:
      write_to_output(d, "That is not an acceptable option.\r\n");
    ...
    if (IS_KAI(d->character) || IS_DEMON(d->character) ||
        IS_MAJIN(d->character)) {
  ...
  case CON_QX:
    ...
    write_to_output(d, "@cCurrent Alignment@D: %s%s@n\r\n",
                    GET_ALIGNMENT(d->character) < -50
                        ? "@R"
                        : (GET_ALIGNMENT(d->character) > 50 ? "@C" : "@G"),
                    disp_align(d->character));
    write_to_output(d, "@YThis is the alignment your character has based on "
                       "your choices.\r\n");
    write_to_output(
        d, "Choose to keep this alignment with no penalty, or choose new\r\n");
    write_to_output(
        d, "alignment and suffer a -5%s PL and -1 stat (random) penalty.\r\n@n",
        "%");
    write_to_output(d, "@D---------------------------------------@n\r\n");
  ...
  case CON_ALIGN:
    ...
    case 'K':
      ...
      break;
    case '1':
      char_stat_set(d->character, "alignment", 1000);
      penalty = TRUE;
      break;
    case '2':
      char_stat_set(d->character, "alignment", 799);
      penalty = TRUE;
      break;
    case '3':
      char_stat_set(d->character, "alignment", 599);
      penalty = TRUE;
      break;
    case '4':
      char_stat_set(d->character, "alignment", 299);
      penalty = TRUE;
      break;
    case '5':
      char_stat_set(d->character, "alignment", 0);
      penalty = TRUE;
      break;
    case '6':
      char_stat_set(d->character, "alignment", -299);
    ...
    default:
      write_to_output(d,
                      "That is not an acceptable option! Choose again...\r\n");
      return;
    }
    if (moveon == TRUE) {
      write_to_output(
          d, "@CWould you like to keep skills gained from your sensei/race "
             "combo (skills, not abilities)\r\nor would you prefer to keep "
             "those skill slots empty? If you choose\r\nto forget then you "
             "will receive 200 PS in exchange.@n\r\n");
      write_to_output(d, "keep or forget: \r\n");
      STATE(d) = CON_SKILLS;
    } else if (penalty == TRUE) {
      loseBasePLPercent(d->character, .2);

      switch (roll) {
      case 1:
        char_stat_mod(d->character, "strength", -1);
      case 2:
        char_stat_mod(d->character, "constitution", -1);
      case 3:
        char_stat_mod(d->character, "wisdom", -1);
      case 4:
        char_stat_mod(d->character, "intelligence", -1);
      ...
      write_to_output(
          d, "@CWould you like to keep skills gained from your sensei/race "
             "combo (skills, not abilities)\r\nor would you prefer to keep "
             "those skill slots empty? If you choose\r\nto forget then you get "
             "200 PS in exchange.@n\r\n");
      write_to_output(d, "keep or forget: \r\n");
      STATE(d) = CON_SKILLS;
    } else {
      return;
  ...
  case CON_SKILLS:
    if (!*arg) {
      ...
      if (!IS_BIO(d->character) && !IS_MUTANT(d->character)) {
        ...
        write_to_output(d, "@CThis menu (and the Negatives menu) are for "
                           "selecting various traits about your character.\n");
        ...
        write_to_output(
            ...
            "@D--------------------------------------------------------@n\n");
        write_to_output(
            d, "@B 1@W) @CExtreme Speed       @c-+30%s to Speed Index @C@n\n",
            "%");
        write_to_output(d,
                        "@B 2@W) @CInc. Cell Regen     @c-LF regen refills "
                        "12%s instead of 5%s@C@n\n",
                        "%", "%");
        write_to_output(d, "@B 3@W) @CExtreme Reflexes    @c-+10 to parry, "
                           "block, and dodge. +10 agility at creation.@C@n\n");
        write_to_output(d, "@B 4@W) @CInfravision         @c-+5 to spot "
                           "hiding, can see in dark @C@n\n");
        write_to_output(
            d,
            "@B 5@W) @CNatural Camo        @c-+10 to hide/sneak rolls@C@n\n");
        write_to_output(d, "@B 6@W) @CLimb Regen          @c-Limbs regen "
                           "almost instantly.@C@n\n");
        write_to_output(d, "@B 7@W) @CPoisonous           @c-Immune to poison, "
                           "poison bite attack.@C@n\n");
        write_to_output(
            d,
            "@B 8@W) @CRubbery Body        @c-10%s of physical dmg to you is "
            "reduced and attacker takes that much loss in stamina.@C@n\n",
            "%");
        write_to_output(d, "@B 9@w) @CInnate Telepathy    @c-Start with "
                           "telepathy at SLVL 50@n\n");
        write_to_output(d,
                        "@B10@w) @CNatural Energy      @c-Get 5%s of your ki "
                        "damage refunded back into your current ki total.@n\n",
                        "%");
        write_to_output(d, "@wChoose: ");
        ...
        write_to_output(
            ...
            "@D--------------------------------------------------------@n\n");
        write_to_output(
            d, "@B1@W) @CHuman   @c- @CHigher PS gains from fighting@n\n");
        write_to_output(
            d, "@B2@W) @CSaiyan  @c- @CSaiyan fight gains (halved)@n\n");
        write_to_output(d, "@B3@W) @CNamek   @c- @CNo food needed@n\n");
        write_to_output(
            d, "@B4@W) @CIcer    @c- @C+20%s damage for Tier 4 attacks@n\n",
            "%");
        write_to_output(
            d, "@B5@W) @CTruffle @c- @CGrant Truffle Auto-train bonus@n\n");
        write_to_output(
            d, "@B6@W) @CArlian  @c- @CGrants Arlian Adrenaline ability@n\n\n");
        write_to_output(d, "@B7@W) @CKai     @c- @CStart with SLVL 30 "
                           "Telepathy and SLVL 30 Focus.\r\n");
        write_to_output(d,
                        "@B8@w) @CKonatsu @c- @C40%s higher chance to multihit "
                        "on physical attacks.\r\n",
                        "%");
        write_to_output(d, "@wChoose: ");
        d->character->genome[0] = 0;
      ...
      if (!IS_BIO(d->character) && !IS_MUTANT(d->character)) {
        char_stat_mod(d->character, "practices", 200);
        ...
        write_to_output(
            d,
            "@B 8@W) @CRubbery Body        @c-10%s less physical dmg to you is "
            "reduced and attacker takes that much loss in stamina.@C@n\n",
            "%");
        write_to_output(d, "@B 9@w) @CInnate Telepathy    @c-Start with "
                           "telepathy at SLVL 50@n\n");
  ...
  case CON_GENOME:
    if (IS_MUTANT(d->character)) {
      ...
      write_to_output(
          ...
          "%");
      write_to_output(d,
                      "@B 2@W) @CInc. Cell Regen     @c-LF regen refills 12%s "
                      "instead of 5%s@C@n\n",
                      "%", "%");
      write_to_output(d, "@B 3@W) @CExtreme Reflexes    @c-+10 to parry, "
                         "block, and dodge. +10 agility at creation.@C@n\n");
      write_to_output(d, "@B 4@W) @CInfravision         @c-+5 to spot hiding, "
                         "can see in dark @C@n\n");
      write_to_output(
          d, "@B 5@W) @CNatural Camo        @c-+10 to hide/sneak rolls@C@n\n");
      write_to_output(d, "@B 6@W) @CLimb Regen          @c-Limbs regen almost "
                         "instantly.@C@n\n");
      write_to_output(d, "@B 7@W) @CPoisonous           @c-Immune to poison, "
                         "poison bite attack.@C@n\n");
      write_to_output(
      ...
      if (d->character->genome[0] > 0 && d->character->genome[1] <= 0) {
        ...
        if (num == 3) {
          char_stat_mod(d->character, "agility", 10);
        }
        write_to_output(d,
                        "@CRolling second mutation... Your second mutation is "
                        "@D[@Y%s@D]@n\r\n",
                        display_genome[num]);
      ...
      write_to_output(
          d, "@B1@W) @CHuman   @c- @CHigher PS gains from fighting@n\n");
      write_to_output(d,
                      "@B2@W) @CSaiyan  @c- @CSaiyan fight gains (halved)@n\n");
      write_to_output(d, "@B3@W) @CNamek   @c- @CNo food needed@n\n");
      write_to_output(
          d, "@B4@W) @CIcer    @c- @C+20%s damage for Tier 4 attacks@n\n", "%");
      write_to_output(
      ...
      write_to_output(
          d, "@B6@W) @CArlian  @c- @CGrants Arlian Adrenaline ability@n\n\n");
      write_to_output(d, "@B7@W) @CKai     @c- @CStart with SLVL 30 Telepathy "
                         "and SLVL 30 Focus.\r\n");
      write_to_output(d,
                      "@B8@w) @CKonatsu @c- @C40%s higher chance to multihit "
                      "on physical attacks.\r\n",
                      "%");
      write_to_output(
          d, "@D----[@gGenome 1@W: @G%s@D]----[@gGenome 2@W: @G%s@D]----",
          display_genome[d->character->genome[0]],
          display_genome[d->character->genome[1]]);
      ...
      if (select > 8 || select < 1) {
        write_to_output(
            d, "@RSelect two genomes to be your primary DNA strains.\n");
        write_to_output(
            d,
            "@D--------------------------------------------------------@n\n");
        write_to_output(
        ...
        write_to_output(d,
                        ...
                        "%");
        write_to_output(
            d, "@RThat is not an acceptable selection. @WTry again:@n\n");
        return;
      } else {
        ...
        } else if (d->character->genome[0] > 0 &&
                   d->character->genome[0] == select) {
          write_to_output(
              d, "You can't choose the same thing for both genomes!\r\n");
          write_to_output(d, "\n@wChoose: ");
          return;
        } else if (d->character->genome[1] == 0) {
  ...
  case CON_BONUS:
    if (!*arg) {
      ...
      write_to_output(d, "\r\n@wTo check the bonuses/negatives you have in "
                         "game use the status command");
      if (GET_CCPOINTS(d->character) > 0) {
        write_to_output(
            d, "\r\n@GYour left over points were spent on Practice Sessions@w");
        char_stat_mod(d->character, "practices",
                      (100 * GET_CCPOINTS(d->character)));
      }
      write_to_output(d, "\r\n*** PRESS RETURN: ");
  ...
  case CON_NEGATIVE:
    if (!*arg) {
      ...
      GET_NEGCOUNT(d->character) = 0;
      if (char_stat_get(d->character, "powerlevel") <= 0) {
        char_stat_set(d->character, "powerlevel", 90);
      }
      if (char_stat_get(d->character, "ki") <= 0) {
        char_stat_set(d->character, "ki", 90);
      }
      if (char_stat_get(d->character, "stamina") <= 0) {
        char_stat_set(d->character, "stamina", 90);
      }
      write_to_output(d, "\r\n@wTo check the bonuses/negatives you have in "
                         "game use the status command");
      if (GET_CCPOINTS(d->character) > 0) {
  ...
  case CON_CLASS_HELP:
    ...
    if (IS_ANDROID(d->character)) {
      ...
      write_to_output(
          d, "\r\n@RAnswer The following questions carefully, they construct "
             "your alignment\r\nand affect your stats.\r\n\r\n");
      write_to_output(d, "@WQuestion (@G1@W out of @g10@W)\r\n");
      write_to_output(d, "@YAnswer the following question:\r\n");
      write_to_output(d, "@wYou go to train one day, but do not know the "
                         "best\r\nway to approach it, What do you do?\r\n");
      write_to_output(d, "@D---------------------------------------@n\r\n");
      write_to_output(
          d, "@B1@W)@C Ask someone with more experience what to do.@n\r\n");
      write_to_output(d,
                      "@B2@W)@C Jump in with some nice classic pushups!@n\r\n");
      write_to_output(d, "@B3@W)@C Search for something magical to increase my "
                         "strength.@n\r\n");
      write_to_output(d, "@B4@W)@C Practice my favorite skills instead of "
                         "working just on my body.@n\r\n");
      write_to_output(d, "@B5@W)@C Spar with a friend so we can both improve "
                         "our abilities.@n\r\n");
      write_to_output(d, "@w\r\nMake a selection:@n\r\n");
      STATE(d) = CON_Q1;
    }
  ...
  case CON_ANDROID:
    ...
    case '1':
      ...
      write_to_output(
          d, "\r\n@RAnswer The following questions carefully, they may "
             "construct your alignment in conflict with your trainer, or your "
             "stats contrary to your liking.\r\n\r\n");
    ...
    case '2':
      ...
      write_to_output(
          d, "\r\n@RAnswer The following questions carefully, they may "
             "construct your alignment in conflict with your trainer, or your "
             "stats contrary to your linking.\r\n\r\n");
      write_to_output(d, "@YAnswer the following question:\r\n");
      write_to_output(d, "@wYou go to train one day, but do not know the "
                         "best\r\nway to approach it, What do you do?\r\n");
      write_to_output(d, "@D---------------------------------------@n\r\n");
    ...
    case '3':
      ...
      write_to_output(d,
                      "\r\n@RAnswer The following questions carefully, they "
                      "may construct your alignment in conflict with your "
                      "trainer or your stats contrary to your liking.\r\n\r\n");
      write_to_output(d, "@YAnswer the following question:\r\n");
      write_to_output(d, "@wYou go to train one day, but do not know the "
                         "best\r\nway to approach it, What do you do?\r\n");
      write_to_output(d, "@D---------------------------------------@n\r\n");
      ...
      if (CONFIG_REROLL_PLAYER_CREATION &&
        ...
        default:
          ...
          write_to_output(d,
                          ...
                          GET_WIS(d->character), GET_CHA(d->character));
          write_to_output(
              d, "Initial statistics, you may reassign individual numbers\r\n");
  ...
  case CON_QSTATS:
    if (parse_stats(d, arg)) {
      ...
      mudlog(CMP, ADMLVL_GOD, TRUE, "New player: %s [%s %s]",
             GET_NAME(d->character), TRUE_RACE(d->character),
             SENSEI_NAME(d->character));
      mudlog(CMP, ADMLVL_GOD, TRUE,
             "Str: %2d Dex: %2d Con: %2d Int: %2d "
             "Wis:  %2d Cha: %2d mod total: %2d",
             GET_STR(d->character), GET_DEX(d->character),
             GET_CON(d->character), GET_INT(d->character),
  ...
  case CON_MENU: { /* get selection from main menu  */
    ...
    case '1':
      ...
      if (GET_ADMLEVEL(d->character) >= 1 &&
          BOARDNEWIMM > GET_BOARD(d->character, 1))
        send_to_char(d->character, "\r\n@GMake sure to check the immortal "
                                   "board, there is a new post there.@n\r\n");
      if (GET_ADMLEVEL(d->character) >= 1 &&
          BOARDNEWCOD > GET_BOARD(d->character, 2))
        send_to_char(d->character, "\r\n@GMake sure to check the request file, "
                                   "it has been updated.@n\r\n");
      if (GET_ADMLEVEL(d->character) >= 1 &&
          BOARDNEWBUI > GET_BOARD(d->character, 4))
        send_to_char(d->character, "\r\n@GMake sure to check the builder "
                                   "board, there is a new post there.@n\r\n");
      if (GET_ADMLEVEL(d->character) >= 1 &&
          BOARDNEWDUO > GET_BOARD(d->character, 3))
        send_to_char(d->character, "\r\n@GMake sure to check punishment board, "
      ...
      if (!IS_ANDROID(d->character)) {
        ...
        sprintf(buf3,
                "@D[@GBlip@D]@Y %s\r\n@RSomeone has suddenly entered your "
                "scouter detection range!@n.",
      ...
      if (!GET_EQ(d->character, WEAR_WIELD1) &&
          PLR_FLAGGED(d->character, PLR_THANDW)) {
    ...
    case '2':
      ...
      write_to_output(d, "Enter the new text you'd like others to see when "
                         "they look at you.\r\n");
      send_editor_help(d);
  ...
  case CON_DELCNF1:
    if (!strcmp(arg, "yes") || !strcmp(arg, "YES")) {
      write_to_output(
          d, "Your user and character files have been deleted. Good bye.\n");
      userDelete(d);
      STATE(d) = CON_CLOSE;
      ...
      write_to_output(d, "Clearly type yes or no. Yes to delete and no to "
                         "return to the menu.\nYes or no:\n");
      return;
    }
    break;
  ...
  case CON_DELCNF2:
    if (!strcmp(arg, "yes") || !strcmp(arg, "YES")) {
      ...
      */
      ...
      if (GET_LEVEL(d->character) > 19 && !RESTRICTED_RACE(d->character) &&
          !CHEAP_RACE(d->character)) {
        int refund = GET_LEVEL(d->character) / 10;
        refund *= 2;
        write_to_output(d,
                        "@D[@g%d RPP refunded to your account for your "
                        "character's levels.@D]@n\r\n",
                        refund);
        d->rpp += refund;
      }
      ...
      if (GET_LEVEL(d->character) <= 40 && CHEAP_RACE(d->character)) {
        write_to_output(d, "@D[@gSince your race doesn't cost RPP to level "
                           "before 40 you are refunded 0 RPP.@D]@n\r\n");
      }
      int refund = get_race(d->character->race)->getRPPRefund();
      if (refund && GET_LEVEL(d->character) > 1) {
        write_to_output(d,
                        "@D[@g%d RPP refunded to your account for your %s "
                        "character.@D]@n\r\n",
                        refund, TRUE_RACE(d->character));
        d->rpp += refund;
      }
      mudlog(NRM, ADMLVL_GOD, TRUE,
             "User %s has deleted character %s (lev %d).", d->user,
