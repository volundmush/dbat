        ...
        case 'Y':
            ...
            else
            {
                ...
                *buf = '\0';
                ...
                send_to_imm("@gHedit@D: @w%s@G has just edited and saved, @Y%s@G.@n", d->character->getName(), buf);
...
ACMD(do_hindex)
{
    ...
    {
        num++;
        if (is_abbrev(argument, help_table[i].keywords) && (GET_ADMLEVEL(ch) >= help_table[i].min_level))
            len +=
