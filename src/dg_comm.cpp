...
void fly_planet(room_vnum roomVnum, char *messg, Character *ch) {
    if (!messg || !*messg)
        return;
...
void send_to_sense(int type, char *messg, Character *ch) {
    if (!messg || !*messg)
        return;
...
void send_to_scouter(char *messg, Character *ch, int num, int type) {
    if (!messg || !*messg)
        return;
