...
{
  switch (pos) {
    case WEAR_WIELD1:   return CAN_WEAR(obj, ITEM_WEAR_WIELD);
    case WEAR_WIELD2:   return CAN_WEAR(obj, ITEM_WEAR_TAKE);
    case WEAR_FINGER_R:
    ...
    if (is_abbrev(arg, "mob")) {
      ...
      switch (type) {
        ...
        case OBJ_TRIGGER:
          ...
          break;
        case MOB_TRIGGER:
          c = get_char_room_vis((struct char_data *)go, name, nullptr);
          break;
      }
      if (c)
...
void init_lookup_table(void)
{
  ...
  for (i = 0; i < BUCKET_COUNT; i++) {
    ...
    lookup_table[i].c    = nullptr;
    lookup_table[i].next = nullptr;
  }
}
...
{
  int bucket = (int) (uid & (BUCKET_COUNT - 1));
  struct lookup_table_t *lt = &lookup_table[bucket], *flt = nullptr;
