  ...
  return;*/
    ...
    if (!strcasecmp("hold", arg2) || !strcasecmp("choke", arg2) || !strcasecmp("grab", arg2) ||
        ...
        if (((!IS_NPC(vict) && IS_ICER(vict) && rand_number(1, 30) >= 28) || AFF_FLAGGED(vict, AFF_ZANZOKEN)) &&
            ...
            if (!AFF_FLAGGED(ch, AFF_ZANZOKEN) || (AFF_FLAGGED(ch, AFF_ZANZOKEN) && GET_SPEEDI(ch) + rand_number(1, 5) <
                ...
                act("@C$N@c disappears, avoiding @C$n's@c grapple attempt before reappearing!@n", false, ch, nullptr,
                    vict, TO_NOTVICT);
                for(auto c : {ch, vict}) c->setAffectFlag(AFF_ZANZOKEN, false);
                ch->decCurST(cost);
        ...
        if ((GET_HIT(ch) * 0.02) * GET_STR(ch) < (GET_HIT(vict) * 0.01) * GET_STR(vict)) {
            ...
            vict->setPlayerFlag(PLR_THANDW, false);
...
ACMD(do_infuse) {
    ...
    ch->setAffectFlag(AFF_INFUSE, true);
...
ACMD(do_candy) {
    ...
    vict->setMobFlag(MOB_HUSK, false);
    die(vict, ch);
...
ACMD(do_implant) {
    ...
    if (!(strcmp(arg, "rarm"))) {
        if (GET_LIMBCOND(vict, 0) >= 1) {
            ...
            vict->setPlayerFlag(PLR_CRARM, true);
            obj_from_char(limb);
            extract_obj(limb);
            return;
        }
        ...
        if (GET_LIMBCOND(vict, 1) >= 1) {
            ...
            vict->setPlayerFlag(PLR_CLARM, true);
            obj_from_char(limb);
            extract_obj(limb);
            return;
        }
        ...
        if (GET_LIMBCOND(vict, 2) >= 1) {
            ...
            vict->setPlayerFlag(PLR_CRLEG, true);
            obj_from_char(limb);
            extract_obj(limb);
            return;
        }
        ...
        if (GET_LIMBCOND(vict, 3) >= 1) {
            ...
            vict->setPlayerFlag(PLR_CLLEG, true);
            obj_from_char(limb);
            extract_obj(limb);
            return;
        }
...
ACMD(do_pose) {
    ...
    reveal_hiding(ch, 0);
        ...
        ch->setPlayerFlag(PLR_POSE, true);
...
ACMD(do_fury) {
    ...
    ch->setPlayerFlag(PLR_FURY, true);
}
        ...
        if (!(vict = get_char_vis(ch, arg2, nullptr, FIND_CHAR_ROOM))) {
            ...
            if (axion_dice(0) > GET_SKILL(ch, SKILL_TELEPATHY)) {
                ...
                if (rand_number(1, 15) >= 14 && !AFF_FLAGGED(ch, AFF_SHOCKED)) {
                    ...
                    ch->setAffectFlag(AFF_SHOCKED, true);
                } else {
        ...
        if (GET_SPEEDI(ch) + rand_number(1, 5) < GET_SPEEDI(ch) + rand_number(1, 5)) {
            ...
            if (!IS_NPC(vict) && !IS_NPC(ch)) {
                send_to_imm("[PK] %s killed %s at room [%d]\r\n", GET_NAME(ch), GET_NAME(vict),
                            vict->getRoomVnum());
                vict->setPlayerFlag(PLR_ABSORBED, true);
            }
...
ACMD(do_regenerate) {
    ...
    if (!IS_NPC(ch)) {
        ...
        if (!PLR_FLAGGED(ch, PLR_TAIL) && IS_BIO(ch)) {
            ch->setPlayerFlag(PLR_TAIL, true);
    ...
    if (AFF_FLAGGED(ch, AFF_CURSE)) {
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_MIGHT)) {
                ...
                ch->setAffectFlag(AFF_MIGHT, true);
                ch->decCurKI(ch->getMaxKI() / 20);
                int duration = roll_aff_duration(GET_INT(ch), 2);
        ...
        if (!can_kill(ch, vict, nullptr, 0)) {
            ...
            if (AFF_FLAGGED(vict, AFF_SLEEP)) {
                ...
                act("$n focuses ki while moving $s hands in a lulling pattern, putting $N to sleep!", true, ch, nullptr,
                    vict, TO_NOTVICT);
                GET_POS(vict) = POS_SLEEPING;
                vict->setAffectFlag(AFF_FLYING, false);
                GET_ALT(vict) = 0;
                return;
            }
        }
...
ACMD(do_disguise) {
    ...
    if (PLR_FLAGGED(ch, PLR_DISGUISED)) {
        ...
        ch->setPlayerFlag(PLR_DISGUISED, true);
...
ACMD(do_eyec) {
    ...
    if (PLR_FLAGGED(ch, PLR_EYEC)) {
        ch->setPlayerFlag(PLR_EYEC, false);
        ...
        ch->setPlayerFlag(PLR_EYEC, true);
...
ACMD(do_heal) {
    ...
    if (vict != ch) {
        ...
        if (AFF_FLAGGED(vict, AFF_BURNED)) {
            ...
            vict->setAffectFlag(AFF_BURNED, false);
        }
    ...
    if (vict == ch) {
        ...
        vict->setAffectFlag(AFF_BLIND, false);
        GET_LIMBCOND(vict, 0) = 100;
        GET_LIMBCOND(vict, 1) = 100;
        GET_LIMBCOND(vict, 2) = 100;
        GET_LIMBCOND(vict, 3) = 100;
...
ACMD(do_barrier) {
    ...
    if (size > prob) {
        ...
        ch->setAffectFlag(AFF_SANCTUARY, true);
        GET_COOLDOWN(ch) = 20;
        return;
    }
}
...
bool placeShadowDragon(int &location) {
    ...
    while (real_room(num) == NOWHERE || !(ROOM_FLAGGED(real_room(num), ROOM_EARTH) ||
                                          ROOM_FLAGGED(real_room(num), ROOM_VEGETA) ||
                                          ROOM_FLAGGED(real_room(num), ROOM_FRIGID) ||
                                          ROOM_FLAGGED(real_room(num), ROOM_AETHER) ||
                                          ROOM_FLAGGED(real_room(num), ROOM_NAMEK) ||
...
void summonShadowDragons() {
    placeShadowDragon(SHADOW_DRAGON1);
    placeShadowDragon(SHADOW_DRAGON2);
    placeShadowDragon(SHADOW_DRAGON3);
    placeShadowDragon(SHADOW_DRAGON4);
    placeShadowDragon(SHADOW_DRAGON5);
    placeShadowDragon(SHADOW_DRAGON6);
    placeShadowDragon(SHADOW_DRAGON7);
    ...
    if (!strcasecmp(arg, "expand")) {
        ...
        if (MINDLINK(ch) == nullptr) {
            ...
            if (GET_INT(MINDLINK(ch)) < axion_dice(-10) && !AFF_FLAGGED(MINDLINK(ch), AFF_SHOCKED)) {
                ...
                MINDLINK(ch)->setAffectFlag(AFF_SHOCKED, false);
            }
...
ACMD(do_spar) {
    ...
    if (ch->togglePlayerFlag(PLR_SPAR)) {
...
void base_update(uint64_t heartPulse, double deltaTime) {
    ...
    for (d = descriptor_list; d; d = d->next) {
        ...
        d->character->setPlayerFlag(PLR_TRANSMISSION, false);

        if (!FIGHTING(d->character) && AFF_FLAGGED(d->character, AFF_POSITION)) {
            d->character->setAffectFlag(AFF_POSITION, false);
        }
        if (SITS(d->character)) {
        ...
        if (PLR_FLAGGED(d->character, PLR_SELFD) && !PLR_FLAGGED(d->character, PLR_SELFD2)) {
            if (rand_number(4, 100) < GET_SKILL(d->character, SKILL_SELFD)) {
                ...
                d->character->setPlayerFlag(PLR_SELFD2, true);
            }
        }
        ...
        if (!strcasecmp(arg, "zenni")) { /* MOOLA! */
            if (prob > perc) {
                if (GET_GOLD(vict) > 0) {
                    ...
                    if (!IS_NPC(vict)) {
                        vict->setPlayerFlag(PLR_STOLEN, true);
...
perform_group(struct char_data *ch, struct char_data *vict, int highlvl, int lowlvl, int64_t highpl, int64_t lowpl) {
  ...
  */
    ...
    vict->setAffectFlag(AFF_GROUP, true);
    if (ch != vict)
    ...
    if (!(vict = get_char_vis(ch, buf, nullptr, FIND_CHAR_ROOM)))
        ...
        if (!AFF_FLAGGED(vict, AFF_GROUP)) {
            if (!AFF_FLAGGED(ch, AFF_GROUP)) {
                ...
                ch->setAffectFlag(AFF_GROUP, true);
            }
            ...
            vict->setAffectFlag(AFF_GROUP, false);
        }
    }
}
...
ACMD(do_ungroup) {
    ...
    if (!*buf) {
        ...
        for (f = ch->followers; f; f = next_fol) {
            next_fol = f->next;
            if (AFF_FLAGGED(f->follower, AFF_GROUP)) {
                f->follower->setAffectFlag(AFF_GROUP, false);
    ...
    tch->setAffectFlag(AFF_GROUP, false);
                    ...
                    case 382:
                        if (AFF_FLAGGED(ch, AFF_BURNED)) {
                            ...
                            act("@C$n@W gently applies a burn salve to $s burns.@n", true, ch, mag_item, nullptr,
                                TO_ROOM);
                            ch->setAffectFlag(AFF_BURNED, false);
                            extract_obj(mag_item);
                        } else {
...
ACMD(do_display) {
    ...
    if (!strcasecmp(argument, "transforms")) {
        ch->togglePrefFlag(PRF_FORM);
        ch->togglePrefFlag(PRF_TECH);
        return;
    }

    if (!strcasecmp(argument, "on") || !strcasecmp(argument, "all")) {
        for(auto f : allPrefs) ch->setPrefFlag(f, true);
    } else if (!strcasecmp(argument, "off") || !strcasecmp(argument, "none")) {
        for(auto f : allPrefs) ch->setPrefFlag(f, false);
    } else {
    ...
    REMOVE_BIT_AR(PRF_FLAGS(ch), PRF_DISPKI);*/
        ...
        for (i = 0; i < strlen(argument); i++) {
            switch (LOWER(argument[i])) {
                case 'p':
                    ch->togglePrefFlag(PRF_DISPHP);
                    break;
                case 's':
                    ch->togglePrefFlag(PRF_DISPMOVE);
                    break;
                case 'k':
                    ch->togglePrefFlag(PRF_DISPKI);
                    break;
                case 't':
                    ch->togglePrefFlag(PRF_DISPTNL);
                    break;
                case 'h':
                    ch->togglePrefFlag(PRF_DISTIME);
                    break;
                case 'g':
                    ch->togglePrefFlag(PRF_DISGOLD);
                    break;
                case 'l':
                    ch->togglePrefFlag(PRF_DISPRAC);
                    break;
                case 'c':
                    ch->togglePrefFlag(PRF_DISPERC);
                    break;
                case 'm':
                    ch->togglePrefFlag(PRF_DISHUTH);
                    break;
                case 'o':
                    ch->togglePrefFlag(PRF_FORM);
                    break;
    ...
    if (num == 47) {
        ...
        if (!AFF_FLAGGED(vict, AFF_SPIRIT) && !PLR_FLAGGED(vict, PLR_BANDAGED)) {
            ...
            if ((GET_SKILL(ch, SKILL_FIRST_AID) + 1) > dc) {
                ...
                vict->setPlayerFlag(PLR_BANDAGED, true);
                extract_obj(aid_obj);
            } else {
                if (vict != ch) {
...
ACMD(do_aura) {
    ...
    if (!strcasecmp(arg, "light")) {
        ...
        if (PLR_FLAGGED(ch, PLR_AURALIGHT)) {
            ...
            ch->setPlayerFlag(PLR_AURALIGHT, false);
            ...
            ch->setPlayerFlag(PLR_AURALIGHT, true);
...
void genRace(char_data* ch, std::string suggestedRace) {
    ...
    ch->set(CharAppearance::hair_color, -1);
...
void genHairLength(char_data* ch, std::string suggestedHair){
    if(is_abbrev(suggestedHair.c_str(), "bald") || is_abbrev(suggestedHair.c_str(), "tiny") 
        ...
        ch->set(CharAppearance::hair_length, HAIRL_BALD);
        ch->set(CharAppearance::hair_color, HAIRC_NONE);
        ch->set(CharAppearance::hair_style, HAIRS_NONE);
        send_to_char(ch,"\r\nSet.\r\n");
        return;
    }
    if(is_abbrev(suggestedHair.c_str(), "short") || suggestedHair == "2") {
        ch->set(CharAppearance::hair_length, HAIRL_SHORT);
        send_to_char(ch,"\r\nSet.\r\n");
        return;
    }
    if(is_abbrev(suggestedHair.c_str(), "medium") || suggestedHair == "3") {
        ch->set(CharAppearance::hair_length, HAIRL_MEDIUM);
        send_to_char(ch,"\r\nSet.\r\n");
        return;
    }
    if(is_abbrev(suggestedHair.c_str(), "long") || suggestedHair == "4") {
        ch->set(CharAppearance::hair_length, HAIRL_LONG);
        send_to_char(ch,"\r\nSet.\r\n");
        return;
    }
    if(is_abbrev(suggestedHair.c_str(), "really long") || suggestedHair == "5") {
        ch->set(CharAppearance::hair_length, HAIRL_RLONG);
        send_to_char(ch,"\r\nSet.\r\n");
        return;
    }
...
void genHairColour(char_data* ch, std::string suggestedHair){
    ...
    if(is_abbrev(suggestedHair.c_str(), "black") || suggestedHair == "1") {
        ch->set(CharAppearance::hair_color, HAIRC_BLACK);
        send_to_char(ch,"\r\nSet.\r\n");
        return;
    }
    if(is_abbrev(suggestedHair.c_str(), "brown") || suggestedHair == "2") {
        ch->set(CharAppearance::hair_color, HAIRC_BLONDE);
        send_to_char(ch,"\r\nSet.\r\n");
        return;
    }
...
ACMD(do_gen) {
    ...
    send_to_char(ch, "[Eyes] Eyes: %s\r\n", ch->get(CharAppearance::eye_color) == 100 ? "@RUnset@n" : eye_colour[ch->get(CharAppearance::eye_color)]);
    send_to_char(ch, "[Skin] Skin: %s\r\n", ch->get(CharAppearance::skin_color) == 100 ? "@RUnset@n" : skin_tone[ch->get(CharAppearance::skin_color)]);
