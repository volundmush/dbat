...
ACMD(do_rpp) {
  ...
  if (!*arg) { /* Display menu */
    ...
    send_to_char(
        ...
        "@b  "
        "------------------------------------------------------------------\n");
    send_to_char(ch, "  @C1@D)@R Disabled            @D[@G -- RPP @D]  "
                     "@C2@D)@R Disabled              @D[@G -- RPP @D]@n\n");
    send_to_char(ch, "  @C3@D)@c Custom Equipment    @D[@G 20 RPP @D]  "
                     "@C4@D)@c Alignment Change      @D[@G 20 RPP @D]\n");
    send_to_char(ch, "  @C5@D)@c 7,500 zenni         @D[@G  1 RPP @D]  "
                     "@C6@D)@c +2 To A Stat          @D[@G  2 RPP @D]\n");
    send_to_char(ch, "  @C7@D)@c +750 PS             @D[@G  4 RPP @D]  "
                     "@C8@D)@R Disabled               @D[@G -- RPP @D]\n");
    send_to_char(ch,
                 "  @C9@D)@c 50%s TNL Exp         @D[@G%3d RPP @D] @C10@D)@c "
                 "Aura Change           @D[@G  2 RPP @D]\n",
                 "%", tnlcost);
    send_to_char(ch, " @C11@D)@c Reach Softcap       @D[@G  5 RPP @D] "
                     "@C12@D)@c RPP Store             @D[@G ?? RPP @D]\n");
    send_to_char(ch, " @C13@D)@c Extra Feature       @D[@G  1 RPP @D] "
                     "@C14@D)@c Restring Equipment    @D[@G  1 RPP @D]\n");
    send_to_char(ch, " @C15@D)@c Extra Skillslot     @D[@G  3 RPP @D] "
                     "@C16@D)@R Disabled              @D[@G -- RPP @D]@n\n");
    send_to_char(ch, "@b  "
                     "---------------------------------------------------------"
                     "---------@n\n");
    send_to_char(ch, "@D                           [@YYour RPP@D:@G %3d@D]@n\n",
                 GET_RP(ch));
    // send_to_char(ch, "@D                           [@YRPP Bank@D:@G
    // %3d@D]@n\n", GET_RBANK(ch));
    send_to_char(ch, "\nSyntax: rpp (num)\n");
    return;
  }
  ...
  if (selection <= 0 || selection > max_choice) {
    send_to_char(ch, "You must choose a number from the menu. Enter the "
                     "command again with no arguments for the menu.\r\n");
    return;
  }
  ...
  if (selection > 2) {
    ...
    if (selection == 3) { /* Custom Equipment Construction */
      if (GET_RP(ch) < 20) {
        send_to_char(ch, "You need at least 20 RPP to initiate a custom "
                         "equipment build.\r\n");
        return;
      } else {
        ...
        ch->desc->obj_long =
            strdup("@wA @cgeneric @Darmor @Wvest@w is lying here@n");
        ch->desc->obj_type = 1;
    ...
    if (selection == 4) { /* Simple Align Change*/
      ...
      if (GET_RP(ch) < pay) {
        ...
        if (!*arg2) {
          send_to_char(ch, "What do you want to change your alignment to? "
                           "(evil, sorta-evil, neutral, sorta-good, good)");
          return;
        }
        if (!strcasecmp(arg2, "evil")) {
          ...
          send_to_char(
              ch,
              "That is not an acceptable option for changing alignment.\r\n");
          return;
        }
      ...
      if (GET_RP(ch) < pay) {
        ...
        if (!strcasecmp(arg2, "str")) {
          if (GET_BONUS(ch, BONUS_WIMP) > 0 &&
              char_stat_get(ch, "strength") >= 70) {
            send_to_char(ch, "You can't because that stat maxes at 70 due to a "
                             "trait negative.\r\n");
            return;
          ...
          if (GET_BONUS(ch, BONUS_FRAIL) > 0 &&
              char_stat_get(ch, "constitution") >= 70) {
            send_to_char(ch, "You can't because that stat maxes at 70 due to a "
                             "trait negative.\r\n");
            return;
          ...
          if (GET_BONUS(ch, BONUS_DULL) > 0 &&
              char_stat_get(ch, "intelligence") >= 70) {
            send_to_char(ch, "You can't because that stat maxes at 70 due to a "
                             "trait negative.\r\n");
            return;
    ...
    if (selection == 9) { /*Simple Exp Reward*/
      ...
      if (GET_RP(ch) < pay) {
        ...
        send_to_char(ch, "You can not buy experience anymore at your level. I "
                         "think you know why.\r\n");
        return;
      } else if (level_exp(ch, GET_LEVEL(ch) + 1) - GET_EXP(ch) < 0) {
        send_to_char(ch,
                     "You can not buy experience anymore UNTIL you level.\r\n");
        return;
      } else {
        char_stat_mod(ch, "experience", level_exp(ch, GET_LEVEL(ch) + 1) * .52);
        send_to_char(ch,
                     "You gained 50%s of the entire experience needed for your "
                     "next level.\r\n",
                     "%");
      } /* Can pay for it */
    ...
    if (selection == 10) { /* Simple Aura Change*/
      ...
      if (GET_RP(ch) < pay) {
        ...
        if (!*arg2) {
          send_to_char(ch, "Change your aura to what? (white, blue, red, "
                           "green, pink, purple, yellow, black, orange)");
          return;
        }
    ...
    if (selection == 11) { /* Simple Soft-cap Reward*/
      ...
      if (GET_RP(ch) < pay) {
        ...
        if (is_soft_cap(ch, 0) && is_soft_cap(ch, 1) && is_soft_cap(ch, 2)) {
          send_to_char(
              ch, "You are already above your softcap for this level.\r\n");
          return;
        }
        bring_to_cap(ch);
      } /* Can pay for it */
    ...
    if (selection == 14) { /* Restring equipment reward */
      ...
      if (GET_RP(ch) < pay) {
        ...
        send_to_char(
            ch,
            "You can not restring a custom piece. Why? Cause I say so. :P\r\n");
        return;
      } else {
        STATE(ch->desc) = CON_POBJ;
    ...
    if (selection == 15) { /* Skillslot Reward */
      ...
      if (GET_RP(ch) < pay) {
        send_to_char(
            ch,
            "You do not have enough RPP in your bank for that selection.\r\n");
        return;
      ...
      if (GET_RP(ch) < pay) {
        ...
        if (found == FALSE) {
          send_to_char(ch, "You have reduced the Dragon Ball wait by a whole "
                           "real life day!\r\n");
          send_to_all("%s has just reduced the Dragon Ball wait by a whole "
                      "real life day!\r\n",
                      GET_NAME(ch));
          dballtime -= 86400;
          if (dballtime <= 0) {
          ...
          send_to_char(ch, "Sorry, it seems there there are several powers "
                           "interfering with the Dragon Balls.\r\n");
          return;
        } else {
          send_to_char(ch, "Sorry, but there is already a set of Dragon Balls "
                           "in existence.\r\n");
          return;
        }
      } /* Can pay for it */
  ...
  if (selection <= 2) {
    ...
    if (selection == 1) {
      ...
      if (GET_RP(ch) < pay) {
        send_to_char(ch,
                     "Nice try but you don't have enough RPP for that.\r\n");
        return;
      } else {
    ...
    if (selection == 1) {
      ...
      send_to_imm("RPP Request: %s paid for Custom Skill, uhoh spaggettios",
                  GET_NAME(ch));
      BOARDNEWCOD = time(0);
      save_mud_time(&time_info);
    }
    ...
    save_char(ch);
    send_to_char(ch,
                 "@R%d@W RPP paid for your selection. An immortal will address "
                 "the request soon enough. Be patient.@n\r\n",
                 pay);
...
ACMD(do_commune) {
  ...
  if ((getCurKI(ch)) < cost) {
    send_to_char(
        ch, "You do not have enough ki to commune with the Eldritch Star.\r\n");
    return;
  }
  if (prob < perc) {
    decCurKI(ch, cost);
    reveal_hiding(ch, 0);
    act("@cYou close your eyes and try to commune with the Eldritch Star. You "
        "are unable to concentrate though.@n",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@W$n closes $s eyes for a moment. Then $e reopens them and frowns.@n",
        TRUE, ch, 0, 0, TO_ROOM);
    ...
    decCurST(ch, cost);
    reveal_hiding(ch, 0);
    act("@cYou close your eyes and commune with the Eldritch Star spiritually. "
        "You feel your stamina replenish some.@n",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@W$n closes $s eyes for a moment. Then $e reopens them and smiles.@n",
        TRUE, ch, 0, 0, TO_ROOM);
    WAIT_STATE(ch, PULSE_2SEC);
    return;
  }
...
ACMD(do_grapple) {
  ...
  if (PLR_FLAGGED(ch, PLR_THANDW)) {
    send_to_char(ch,
                 "Your are too busy wielding your weapon with two hands!\r\n");
    return;
  }
  ...
  if (ABSORBBY(ch)) {
    send_to_char(
        ch, "You are currently being absorbed by someone! Try 'escape'!\r\n");
    return;
  }

  if (GRAPPLING(ch) != NULL) {
    act("@RYou stop grappling with @r$N@R!@n", TRUE, ch, 0, GRAPPLING(ch),
        TO_CHAR);
    act("@r$n@R stops grappling with @rYOU!!@n", TRUE, ch, 0, GRAPPLING(ch),
        TO_VICT);
  ...
  if (GRAPPLED(ch) != NULL) {
    send_to_char(ch, "You are currently a victim of grappling! Try 'escape' to "
                     "break free!\r\n");
    return;
  }
  ...
  if (!strcasecmp("hold", arg2) || !strcasecmp("choke", arg2) ||
      !strcasecmp("grab", arg2) || !strcasecmp("wrap", arg2)) {
    pass = TRUE;
    int perc = GET_SKILL(ch, SKILL_GRAPPLE), prob = axion_dice(0),
        cost = GET_MAX_MOVE(ch) / 100;
    ...
    if (((!IS_NPC(vict) && IS_ICER(vict) && rand_number(1, 30) >= 28) ||
      ...
      if (!char_condition_has(ch, "zanzoken") ||
        ...
        reveal_hiding(ch, 0);
        act("@C$N@c disappears, avoiding your grapple attempt before "
            "reappearing!@n",
            FALSE, ch, 0, vict, TO_CHAR);
        act("@cYou disappear, avoiding @C$n's@c grapple attempt before "
            "reappearing!@n",
        ...
        reveal_hiding(ch, 0);
        act("@C$N@c disappears, trying to avoid your grapple but your zanzoken "
            "is faster!@n",
            FALSE, ch, 0, vict, TO_CHAR);
        act("@cYou zanzoken to avoid the grapple attempt but @C$n's@c zanzoken "
            "is faster!@n",
            FALSE, ch, 0, vict, TO_VICT);
    ...
    if ((GET_HIT(ch) * 0.02) * GET_STR(ch) <
        (GET_HIT(vict) * 0.01) * GET_STR(vict)) {
      reveal_hiding(ch, 0);
      act("@RYou try to grapple with @r$N@R, but $E manages to overpower "
          "you!@n",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@r$n@R tries to grapple with YOU, but you manage to overpower $m!@n",
          TRUE, ch, 0, vict, TO_VICT);
      act("@r$n@R tries to grapple with @r$N@R, but $E manages to overpower "
          "@r$n@R!@n",
      ...
      WAIT_STATE(ch, PULSE_4SEC);
      return;
    } else if ((GET_HIT(ch) * 0.01) * GET_STR(ch) <
                   (GET_HIT(vict) * 0.01) * GET_STR(vict) &&
               rand_number(1, 4) == 1) {
      reveal_hiding(ch, 0);
      act("@RYou try to grapple with @r$N@R, but $E manages to overpower "
      ...
      reveal_hiding(ch, 0);
      act("@RYou try to grapple with @r$N@R, but $E manages to avoid it!@n",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@r$n@R tries to grapple with YOU, but you manage to avoid it!@n",
          TRUE, ch, 0, vict, TO_VICT);
      act("@r$n@R tries to grapple with @r$N@R, but $E manages to avoid it!@n",
          TRUE, ch, 0, vict, TO_NOTVICT);
      decCurST(ch, cost);
      improve_skill(ch, SKILL_GRAPPLE, 1);
      WAIT_STATE(ch, PULSE_4SEC);
      ...
      reveal_hiding(ch, 0);
      act("@RYou rush at @r$N@R and manage to get $M in a hold from behind!@n",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@r$n@R rushes at YOU and manages to get you in a hold from "
          "behind!@n",
          TRUE, ch, 0, vict, TO_VICT);
      act("@r$n@R rushes at @r$N@R and manages to get $M in a hold from "
          "behind!@n",
          TRUE, ch, 0, vict, TO_NOTVICT);
      ...
      reveal_hiding(ch, 0);
      act("@RYou rush at @r$N@R and manage to grab $S throat with both "
          "hands!@n",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@r$n@R rushes at YOU and manages to grab your throat with both "
          "hands!@n",
          TRUE, ch, 0, vict, TO_VICT);
      act("@r$n@R rushes at @r$N@R and manages to grab $S throat with both "
          "hands!@n",
          TRUE, ch, 0, vict, TO_NOTVICT);
      ...
      if (!IS_MAJIN(ch)) {
        send_to_char(
            ch,
            "Your body is not flexible enough to wrap around a target!\r\n");
        return;
      }

      act("@MMoving quickly you stretch your body out and wrap it around the "
          "length of @c$N's@M body! You tighten your body until you begin "
          "crushing @c$N@M!",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@C$n@M quickly stretches out $s body and wraps it around @RYOU@M! "
          "You feel $s body begin to crush your own!@n",
          TRUE, ch, 0, vict, TO_VICT);
      act("@C$n@M quickly stretches out $s body and wraps it around @c$N@M! It "
      ...
      reveal_hiding(ch, 0);
      act("@RYou rush at @r$N@R and manage to lock your arm onto $S!@n", TRUE,
          ch, 0, vict, TO_CHAR);
      act("@r$n@R rushes at YOU and manages to lock $s arm onto your's!@n",
          TRUE, ch, 0, vict, TO_VICT);
    ...
    send_to_char(ch,
                 "Syntax: grapple (target) (hold | choke | grab | wrap)\r\n");
    return;
  }
}
...
ACMD(do_trip) {
  ...
  if (vict != NULL) {
      ...
      if (!char_condition_has(ch, "zanzoken") ||
          (char_condition_has(ch, "zanzoken") &&
           GET_SPEEDI(ch) + rand_number(1, 5) <
               GET_SPEEDI(vict) + rand_number(1, 5))) {
        reveal_hiding(ch, 0);
        act("@C$N@c disappears, avoiding your trip before reappearing!@n",
            FALSE, ch, 0, vict, TO_CHAR);
        act("@cYou disappear, avoiding @C$n's@c trip before reappearing!@n",
            FALSE, ch, 0, vict, TO_VICT);
        ...
        reveal_hiding(ch, 0);
        act("@C$N@c disappears, trying to avoid your trip but your zanzoken is "
            "faster!@n",
            FALSE, ch, 0, vict, TO_CHAR);
        act("@cYou zanzoken to avoid the trip but @C$n's@c zanzoken is "
            "faster!@n",
            FALSE, ch, 0, vict, TO_VICT);
    ...
    if (perc < prob) { /* Fail! */
      reveal_hiding(ch, 0);
      act("@mYou move to trip $N@m, but you screw up and $E keeps $S "
          "footing!@n",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@m$n@m moves to trip YOU, but $e screws up and you manage to keep "
          "your footing!@n",
          TRUE, ch, 0, vict, TO_VICT);
      act("@m$n@m moves to trip $N@m, but $e screws up and $N@m manages to "
          "keep $S footing!@n",
          TRUE, ch, 0, vict, TO_NOTVICT);
      ...
      reveal_hiding(ch, 0);
      act("@mYou move to trip $N@m, and manage to knock $M off $S feet!@n",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@m$n@m moves to trip YOU, and manages to knock you off your feet!@n",
          TRUE, ch, 0, vict, TO_VICT);
      act("@m$n@m moves to trip $N@m, and manages to knock $N@m off $S feet!@n",
          TRUE, ch, 0, vict, TO_NOTVICT);
      improve_skill(ch, SKILL_TRIP, 0);
      decCurST(ch, cost);
...
ACMD(do_train) {
  ...
  weight = getCurCarriedWeight(ch);

  int strcap = 5000, spdcap = 5000, intcap = 5000, wiscap = 5000, concap = 5000,
      aglcap = 5000;
  ...
  if (weight < (GET_LEVEL(ch) * 100) && GET_LEVEL(ch) <= 19) {
   send_to_char(ch, "With so little weight on you like that it would be a joke
  to try and train.\r\n"); return; } else if (weight < (GET_LEVEL(ch) * 110) &&
  GET_LEVEL(ch) <= 45) { send_to_char(ch, "With so little weight on you like
  that it would be a joke to try and train.\r\n"); return; } else if (weight <
  (GET_LEVEL(ch) * 125) && GET_LEVEL(ch) > 45) { send_to_char(ch, "With so
  little weight on you like that it would be a joke to try and train.\r\n");
   return;
  }
 */
  ...
  if (char_stat_get(ch, stat_name_key) >= 70 && GET_BONUS(ch, nega_trait) > 0) {
    send_to_char(
        ch, "You're not able to withstand increasing your %s beyond 70.\r\n",
        stat_name);
    return;
  }
  ...
  case 4:
    if ((getCurST(ch)) < cost) {
      send_to_char(ch, "You do not have enough stamina with the current weight "
                       "worn and gravity!\r\n");
      return;
    }
  ...
  case 6:
    if ((getCurKI(ch)) < cost) {
      send_to_char(ch, "You do not have enough ki with the current weight worn "
                       "and gravity!\r\n");
      return;
    }
  ...
  case 1:
    switch (msg_case) {
    case 1:
      act("@WYou throw a flurry of punches into the air at an invisible "
          "opponent.@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n throws a flurry of punches into the air.@n", TRUE, ch, 0, 0,
          TO_ROOM);
      break;
    case 2:
      act("@WYou leap into the air and throw a wild kick at an invisible "
          "opponent@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n leaps into the air and throws a wild kick at nothing.@n", TRUE,
          ch, 0, 0, TO_ROOM);
      break;
    case 3:
      act("@WYou leap high into the air and unleash a flurry of punches and "
          "kicks at an invisible opponent@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n leaps high into the air and unleashes a flurry of punches and "
          "kicks at nothing.@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    }
  ...
  case 2:
    switch (msg_case) {
    case 1:
      act("@WYou dash quickly around the surrounding area as fast as you "
          "can!@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n dashes quickly around the surrounding area as fast as $e "
          "can!@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    case 2:
      act("@WYou dodge to the side as fast as you can!@n", TRUE, ch, 0, 0,
          TO_CHAR);
      act("@W$n dodges to the side as fast as $e can!@n", TRUE, ch, 0, 0,
          TO_ROOM);
      break;
    case 3:
      act("@WYou dash backwards as fast as you can!@n", TRUE, ch, 0, 0,
          TO_CHAR);
      act("@W$n dashes backwards as fast as $e can!@n", TRUE, ch, 0, 0,
          TO_ROOM);
      break;
    }
    break;
  case 3:
    switch (msg_case) {
    case 1:
      act("@WYou leap into the air and then slam into the ground with your "
          "feet outstretched!@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n leaps into the air and then slams into the ground with $s feet "
          "outstretched!?@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    case 2:
      act("@WYou leap into the air and then slam into the ground with your "
          "fists!@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n leaps into the air and then slams into the ground with $s "
          "fists!?@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    case 3:
  ...
  case 4:
    switch (msg_case) {
    case 1:
      act("@WYou do a series of backflips through the air, landing gracefully "
          "on one foot a moment later.@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n does a series of backflips through the air, landing gracefully "
          "on one foot a moment later.@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    case 2:
      act("@WYou flip forward and launch off your hands into the air. You land "
          "gracefully on one foot a moment later.@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n flips forward and launches off $s hands into the air. Then $e "
          "lands gracefully on one foot a moment later.@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    case 3:
      act("@WYou flip to the side off one hand and then land on your feet.@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n flips to the side off one hand and then lands on $s feet.@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    }
    break;
  case 5:
    switch (msg_case) {
    case 1:
      act("@WConcentrating you fly high into the air as fast as you can before "
          "settling slowly back to the ground.@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n flies high into the air as fast as $e can before settling "
          "slowly back to the ground.@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    case 2:
      act("@WYou focus your ki at your outstretched hand and send a mild "
          "shockwave in that direction!@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n focuses $s ki at $s outstretched hand and sends a mild "
          "shockwave in that direction!@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    case 3:
      act("@WYou concentrate on your ki and force torrents of it to rush out "
          "from your body randomly!@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n seems to concentrate before torrents of ki randomly blasts out "
          "from $s body!@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    }
  ...
  case 6:
    switch (msg_case) {
    case 1:
      act("@WYou close your eyes and wage a mental battle against an imaginary "
          "opponent.@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n closes $s eyes for a moment and an expression of intensity "
          "forms on it.@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    case 2:
      act("@WYou look around and contemplate battle tactics for an imaginary "
          "scenario.@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@W$n looks around and appears to be imagining things that aren't "
          "there.@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    case 3:
      act("@WYou invent a battle plan for a battle that doesn't exist!@n", TRUE,
          ch, 0, 0, TO_CHAR);
      act("@W$n seems to have thought of something.@n", TRUE, ch, 0, 0,
          TO_ROOM);
      break;
    }
    break;
...
ACMD(do_rip) {
  ...
  if (ch != vict && GET_POS(ch) > POS_SLEEPING) {
    if ((getCurST(ch)) < GET_MAX_MOVE(ch) / 20) {
      ...
      decCurST(ch, getMaxST(ch) / 20);
      if (GET_HIT(ch) > GET_HIT(vict) * 2) {
        reveal_hiding(ch, 0);
        act("@rYou rush at @R$N@r and grab $S tail! With a powerful tug you "
            "pull it off!@n",
            TRUE, ch, 0, vict, TO_CHAR);
        act("@R$n@r rushes at YOU and grabs your tail! With a powerful tug $e "
            "pulls it off!@n",
            TRUE, ch, 0, vict, TO_VICT);
        act("@R$n@R rushes at @R$N@r and grab $S tail! With a powerful tug $e "
            "pulls it off!@n",
            TRUE, ch, 0, vict, TO_NOTVICT);
        char_lose_tail(vict);
        return;
        ...
        reveal_hiding(ch, 0);
        act("@rYou rush at @R$N@r and grab $S tail! You are too weak to pull "
            "it off though!@n",
            TRUE, ch, 0, vict, TO_CHAR);
        act("@R$n@r rushes at YOU and grabs your tail! $e is too weak to pull "
            "it off though!@n",
            TRUE, ch, 0, vict, TO_VICT);
        act("@R$n@R rushes at @R$N@r and grab $S tail! $e is too weak to pull "
            "it off though!@n",
            TRUE, ch, 0, vict, TO_NOTVICT);
        return;
      }
      ...
      decCurST(ch, getMaxST(ch) / 20);
      reveal_hiding(ch, 0);
      act("@rYou rush at @R$N@r and try to grab $S tail, but fail!@n", TRUE, ch,
          0, vict, TO_CHAR);
      act("@R$n@r rushes at YOU and tries to grab your tail, but fails!@n",
          TRUE, ch, 0, vict, TO_VICT);
      act("@R$n@R rushes at @R$N@r and tries to grab $S tail, but fails!@n",
          TRUE, ch, 0, vict, TO_NOTVICT);
      return;
    }
  } else if (ch == vict) {
    ...
    act("@R$n@r grabs $s own tail and yanks it off!@n", TRUE, ch, 0, 0,
        TO_ROOM);
    char_lose_tail(vict);
  } else {
    if ((getCurST(ch)) < GET_MAX_MOVE(ch) / 20) {
    ...
    decCurST(ch, getMaxKI(ch) / 20);
    reveal_hiding(ch, 0);
    act("@rYou reach and grab @R$N's@r tail! With a powerful tug you pull it "
        "off!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@RYou feel your tail pulled off!@n", TRUE, ch, 0, vict, TO_VICT);
    act("@R$n@R reaches and grabs @R$N's@r tail! With a powerful tug $e pulls "
        "it off!@n",
        TRUE, ch, 0, vict, TO_NOTVICT);
    char_lose_tail(vict);
    return;
...
ACMD(do_paralyze) {
  ...
  if ((getCurKI(ch)) < GET_HIT(vict) / 10 + (GET_MAX_MANA(ch) / 20)) {
    send_to_char(ch, "You realize you can't paralyze them. You don't have "
                     "enough ki to restrain them!\r\n");
    return;
  }
  ...
  if (GET_BONUS(vict, BONUS_INSOMNIAC)) {
    ...
    act("@RYou focus ki and point both your arms at @r$N@R. However $N seems "
        "to shake off your paralysis attack!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@r$n @Rfocuses ki and points both $s arms at YOU! Your insomnia makes "
        "you immune to $s feeble paralysis attempt.@n",
        TRUE, ch, 0, vict, TO_VICT);
    act("@r$n @Rfocuses ki and points both $s arms at @r$N@R. However $N seems "
        "to shake off $s paralysis attack!@n",
        TRUE, ch, 0, vict, TO_NOTVICT);
    return;
  } else if (prob < perc) {
    reveal_hiding(ch, 0);
    act("@RYou focus ki and point both your arms at @r$N@R. However $E manages "
        "to avoid your attempt to paralyze $M!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@r$n @Rfocuses ki and points both $s arms at YOU! You manage to avoid "
        "$s technique though...@n",
        TRUE, ch, 0, vict, TO_VICT);
    act("@r$n @Rfocuses ki and points both $s arms at @r$N@R. However $E "
        "manages to avoid @r$n's@R attempted technique...@n",
        TRUE, ch, 0, vict, TO_NOTVICT);
    decCurKI(ch, GET_HIT(vict) / 6 + (GET_MAX_MANA(ch) / 20));
    improve_skill(ch, SKILL_PARALYZE, 0);
  } else {
    reveal_hiding(ch, 0);
    act("@RYou focus ki and point both your arms at @r$N@R. Your ki flows into "
        "$S body and partially paralyzes $M!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@r$n @Rfocuses ki and points both $s arms at YOU! You are caught in "
        "$s paralysis technique and now can barely move!@n",
        TRUE, ch, 0, vict, TO_VICT);
    act("@r$n @Rfocuses ki and points both $s arms at @r$N@R. @r$n's@R ki "
...
ACMD(do_taisha) {
  ...
  if (room_flagged(room, ROOM_AURA)) {
    send_to_char(
        ch, "This area already has an aura of regeneration around it.\r\n");
    return;
  }

  if (room_gravity_get(room) > 0) {
    send_to_char(ch, "This area's gravity is too hostile to an aura.\r\n");
    return;
  }

  if ((getCurKI(ch)) < GET_MAX_MANA(ch) / 3) {
  ...
  decCurKI(ch, getMaxKI(ch) / 3);
  if (prob < perc) {
    reveal_hiding(ch, 0);
    act("@WYou hold up your hands while channeling ki. Your technique fails to "
        "produce an aura though....@n",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@g$n@W holds up $s hands while channeling ki. $s technique fails to "
        "produce an aura though....",
        TRUE, ch, 0, 0, TO_ROOM);
    improve_skill(ch, SKILL_TAISHA, 1);
    return;
  } else {
    reveal_hiding(ch, 0);
    act("@WYou hold up your hands while channeling ki. Suddenly a @wburst@W of "
        "calming @Cblue@W light covers the surrounding area!@n",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@g$n holds up $s hands while channeling ki. Suddenly a @wburst@W of "
        "calming @Cblue@W light covers the surrounding area!@n",
        TRUE, ch, 0, 0, TO_ROOM);
    improve_skill(ch, SKILL_TAISHA, 1);
...
ACMD(do_kura) {
  ...
  if ((getCurKI(ch)) >= GET_MAX_MANA(ch)) {
  ...
  if ((getCurST(ch)) < cost) {
    send_to_char(ch,
                 "You do not have enough stamina for that high a number.\r\n");
    return;
  }
  ...
  if (skill <= axion_dice(0)) {
    decCurST(ch, cost);
    reveal_hiding(ch, 0);
    act("You crouch down and scream as your eyes turn red. You attempt to tap "
        "into your dark energies but you fail!",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@c$n@w crouches down and screams as $s eyes turn red and $e attempts "
        "to tap into dark energies but fails!",
        TRUE, ch, 0, 0, TO_ROOM);
    ...
    reveal_hiding(ch, 0);
    act("You crouch down and scream as your eyes turn red. You attempt to tap "
        "into your dark energies and succeed as a rush of energy explodes "
        "around you!",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@c$n@w crouches down and screams as $s eyes turn red. Suddenly $e "
        "manages to tap into dark energies and a rush of energy explodes "
        "around $m!",
        TRUE, ch, 0, 0, TO_ROOM);
    improve_skill(ch, SKILL_KURA, 0);
    WAIT_STATE(ch, PULSE_2SEC);
...
ACMD(do_candy) {
  ...
  if ((getCurKI(ch)) < ch_max / 15) {
    send_to_char(ch, "You do not have enough ki.\r\n");
    return;
  }
  if (rand_number(1, 6) == 6) {
    decCurKI(ch, getMaxKI(ch) / 15);
    reveal_hiding(ch, 0);
    act("@cYou aim your forelock at @R$N@c and fire a beam of energy but it is "
        "dodged!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@C$n@c aims $s forelock at @R$N@c and fires a beam of energy but the "
        "beam is dodged!@n",
        TRUE, ch, 0, vict, TO_NOTVICT);
    if (!FIGHTING(ch)) {
      set_fighting(ch, vict);
    ...
    decCurKI(ch, getMaxKI(ch) / 15);
    reveal_hiding(ch, 0);
    act("@cYou aim your forelock at @R$N@c and fire a beam of energy that "
        "envelopes $S entire body and changes $M into candy!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@C$n@c aims $s forelock at @R$N@c and fires a beam of energy that "
        "envelopes $S entire body and changes $M into candy!@n ",
        TRUE, ch, 0, vict, TO_NOTVICT);
    if (vict_max >= ch_max * 1.5) {
      ...
      obj_to_char(obj, ch);
               ...
               (((vict_max >= ch_max * .5) && GET_LEVEL(ch) < 100) ||
                ((vict_max >= ch_max * .1) && GET_LEVEL(ch) >= 100))) {
      send_to_char(ch, "You grab the candy as it falls.\r\n");
...
ACMD(do_future) {
  ...
  if ((getCurKI(ch)) < GET_MAX_MANA(ch) / 40) {
    send_to_char(ch, "You do not have enough ki.\r\n");
    return;
  }
  ...
  if (GET_PRACTICES(ch, GET_CLASS(ch)) < 100) {
    send_to_char(
        ch,
        "You do not have enough PS to activate or pass on this ability.\r\n");
    return;
  }

  if (vict != ch) {
    if (char_stat_get(vict, "speed") + 5 > 70 &&
        GET_BONUS(vict, BONUS_SLOW) > 0) {
      send_to_char(
          ch, "They can't handle having their speed increased beyond 70.\r\n");
      return;
    }
    if (char_stat_get(vict, "intelligence") + 2 > 70 &&
        GET_BONUS(vict, BONUS_DULL) > 0) {
      send_to_char(ch, "They can't handle having their intelligence increased "
                       "beyond 70.\r\n");
      return;
    }
    ...
    char_stat_mod(ch, "practices", -100);
    reveal_hiding(ch, 0);
    act("@CYou focus your energy into your fingers before stabbing your claws "
        "into $N and bestowing the power of Future Sight upon $M. Shortly "
        "after $E passes out.@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@C$n focuses $s energy into $s fingers before stabbing $s claws into "
        "YOUR neck and bestowing the power of Future Sight upon you! Soon "
        "after you pass out!@n",
        TRUE, ch, 0, vict, TO_VICT);
    act("@C$n focuses $s energy into $s fingers before stabbing $s claws into "
    ...
    if (char_stat_get(ch, "speed") + 5 > 70 && GET_BONUS(ch, BONUS_SLOW) > 0) {
      send_to_char(
          ch, "You can't handle having your speed increased beyond 70.\r\n");
      return;
    }
    if (char_stat_get(ch, "intelligence") + 2 > 70 &&
        GET_BONUS(ch, BONUS_DULL) > 0) {
      send_to_char(
          ch,
          "You can't handle having your intelligence increased beyond 70.\r\n");
      return;
    }
    decCurKI(ch, getMaxKI(ch) / 40);
    char_stat_mod(ch, "practices", -100);
    reveal_hiding(ch, 0);
    act("@CYou focus your energy into your mind and awaken your latent Future "
        "Sight powers!@n",
        TRUE, ch, 0, vict, TO_CHAR);
...
ACMD(do_drag) {
  ...
  if (sect == SECT_WATER_NOSWIM || sect == SECT_WATER_SWIM) {
  ...
  if (GET_POS(vict) != POS_SLEEPING) {
    reveal_hiding(ch, 0);
    act("@wYou try to grab and pull @C$N@W with you, but $E resists!@n", TRUE,
        ch, 0, vict, TO_CHAR);
    act("@C$n@W tries to grab and pull you! However you resist!@n", TRUE, ch, 0,
        vict, TO_VICT);
    act("@C$n@W tries to grab and pull @c$N@W but $E resists!@n", TRUE, ch, 0,
        vict, TO_NOTVICT);
    ...
    reveal_hiding(ch, 0);
    act("@wYou try to grab and pull @C$N@W with you, but $E is too heavy!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@C$n@W tries to grab and pull @c$N@W but $E is too heavy!@n", TRUE, ch,
        0, vict, TO_ROOM);
    return;
  } else {
    reveal_hiding(ch, 0);
...
ACMD(do_suppress) {
  ...
  if (char_condition_has(ch, "powering_up")) {
  ...
  if (!strcasecmp(arg, "release")) {
    if (GET_SUPPRESS(ch)) {
      reveal_hiding(ch, 0);
      act("@GYou stop suppressing your current powerlevel!@n", TRUE, ch, 0, 0,
          TO_CHAR);
      act("@G$n smiles as a rush of power erupts around $s body briefly.@n",
          TRUE, ch, 0, 0, TO_ROOM);
...
ACMD(do_hass) {
  ...
  if ((getCurST(ch)) < GET_MAX_MOVE(ch) / 30) {
  ...
  if (perc < prob) {
    reveal_hiding(ch, 0);
    act("@WYou try to move your arms at incredible speeds but screw up and "
        "waste some of your stamina.@n",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@C$n@W tries to move $s arms at incredible speeds but screws up and "
        "wastes some of $s stamina.@n",
        TRUE, ch, 0, 0, TO_ROOM);
    ...
    reveal_hiding(ch, 0);
    act("@WYou concentrate and start to move your arms at incredible speeds.@n",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@C$n@W concentrates and starts to move $s arms at incredible "
        "speeds.@n",
        TRUE, ch, 0, 0, TO_ROOM);
...
ACMD(do_implant) {
  ...
  limb = char_inventory_search_vnum(ch, 66, FALSE,
                                    SEARCH_GENUINE | SEARCH_WORKING);
  ...
  if (!(strcmp(arg, "rarm"))) {
    if (GET_LIMBCOND(vict, 1) >= 1) {
      ...
      if (vict != ch) {
        reveal_hiding(ch, 0);
        act("@WYou place the $p@W up to @c$N@W's body. It automaticly adjusts "
            "itself, becoming a new right arm!@n",
            TRUE, ch, limb, vict, TO_CHAR);
        act("@C$n@W places a $p@W up to your body. It automaticly adjusts "
            "itself, becoming a new right arm!@n",
            TRUE, ch, limb, vict, TO_VICT);
        act("@C$n@W places a $p@W up to @c$N@W's body. It automaticly adjusts "
            "itself, becoming a new right arm!@n",
            TRUE, ch, limb, vict, TO_NOTVICT);
      }
      if (vict == ch) {
        reveal_hiding(ch, 0);
        act("@WYou place the $p@W up to your body. It automaticly adjusts "
            "itself, becoming a new right arm!@n",
            TRUE, ch, limb, 0, TO_CHAR);
        act("@C$n@W places the $p@W up to $s body. It automaticly adjusts "
            "itself, becoming a new right arm!@n",
            TRUE, ch, limb, 0, TO_ROOM);
      }
    ...
    if (GET_LIMBCOND(vict, 2) >= 1) {
      ...
      if (vict && vict != ch) {
        reveal_hiding(ch, 0);
        act("@WYou place the $p@W up to @c$N@W's body. It automaticly adjusts "
            "itself, becoming a new left arm!@n",
            TRUE, ch, limb, vict, TO_CHAR);
        act("@C$n@W places a $p@W up to your body. It automaticly adjusts "
            "itself, becoming a new left arm!@n",
            TRUE, ch, limb, vict, TO_VICT);
    ...
    if (GET_LIMBCOND(vict, 3) >= 1) {
      ...
      if (vict && vict != ch) {
        reveal_hiding(ch, 0);
        act("@WYou place the $p@W up to @c$N@W's body. It automaticly adjusts "
            "itself, becoming a new right leg!@n",
            TRUE, ch, limb, vict, TO_CHAR);
        act("@C$n@W places a $p@W up to your body. It automaticly adjusts "
            "itself, becoming a new right leg!@n",
            TRUE, ch, limb, vict, TO_VICT);
    ...
    if (GET_LIMBCOND(vict, 4) >= 1) {
      ...
      if (vict && vict != ch) {
        reveal_hiding(ch, 0);
        act("@WYou place the $p@W up to @c$N@W's body. It automaticly adjusts "
            "itself, becoming a new left leg!@n",
            TRUE, ch, limb, vict, TO_CHAR);
        act("@C$n@W places a $p@W up to your body. It automaticly adjusts "
            "itself, becoming a new left leg!@n",
            TRUE, ch, limb, vict, TO_VICT);
...
ACMD(do_pose) {
  ...
  if ((getCurST(ch)) < GET_MAX_MOVE(ch) / 40) {
    send_to_char(ch, "You do not have enough stamina to pull off such an "
                     "exciting pose!\r\n");
    return;
  }
  ...
  if (char_condition_has(ch, "special_pose")) {
    send_to_char(
        ch,
        "You are already feeling good and confident from a previous pose.\r\n");
    return;
  }
  ...
  if (char_stat_get(ch, "strength") + 8 > 70 && GET_BONUS(ch, BONUS_WIMP) > 0) {
    send_to_char(
        ch, "You can't handle having your strength increased beyond 70.\r\n");
    return;
  }
  if (char_stat_get(ch, "wisdom") + 8 > 70 &&
      GET_BONUS(ch, BONUS_FOOLISH) > 0) {
    send_to_char(
        ch, "You can't handle having your wisdom increased beyond 70.\r\n");
    return;
  }

  if (prob < perc) {
    reveal_hiding(ch, 0);
    act("@WYou attempt to strike an awe inspiring pose, but end up falling on "
        "your face!@n",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@C$n@W attempts to strike an awe inspiring pose, but ends up falling "
        "on $s face!@n",
        TRUE, ch, 0, 0, TO_ROOM);
    ...
    case 1:
      act("@WYou turn around with your back to everyone. You bend forward "
          "dramatically and put your head between your legs!@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@C$n@W turns around with $s back to you. $e bends forward "
          "dramatically and puts $s head between $s legs. Strange...@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    case 2:
      act("@WYou turn to the side while flexing your muscles and extend your "
          "arms up at an angle dramatically!@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@C$n@W turns to the side while flexing $s muscles and extending $s "
          "arms up at an angle dramatically!@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    case 3:
      act("@WYou extend one leg outward while you bend forward, balancing on a "
          "single leg!@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@C$n@W extends one leg outward while $e bends forward, balancing on "
          "a single leg!@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    case 4:
      act("@WYou drop down to one knee while angling your arms up to either "
          "side and slanting your hands down like wings!@n",
          TRUE, ch, 0, 0, TO_CHAR);
      act("@C$n@W drops down to one knee while angling $s arms up to either "
          "side and slanting $s hands down like wings!@n",
          TRUE, ch, 0, 0, TO_ROOM);
      break;
    }
    send_to_char(
...
ACMD(do_fury) {
  ...
  if (char_condition_has(ch, "halfbreed_fury")) {
    send_to_char(ch, "You are already furious, your next attack will "
                     "devestate, hurry use it!\r\n");
    return;
  }

  if (GET_FURY(ch) < 100) {
    send_to_char(ch, "You do not have enough anger to release your fury upon "
                     "your foes!\r\n");
    return;
  }
  ...
  if (!*arg) {
    ...
    send_to_char(ch, "Syntax: fury (attack) <--- this will not use up your LF "
                     "to restore PL.\n        fury <--- fury by itself will do "
                     "both LF to PL restore and attack boost.\r\n");
    return;
  }

  reveal_hiding(ch, 0);
  act("You release your fury! Your very next attack is guaranteed to rip your "
      "foes a new one!",
      TRUE, ch, 0, 0, TO_CHAR);
  act("$n screams furiously as a look of anger appears on $s face!", TRUE, ch,
      0, 0, TO_ROOM);
  char_condition_apply(ch, "halfbreed_fury", "fury", "released");
}
...
ACMD(do_think) {
  ...
  if (!*argument) {
    ...
    send_to_imm("@GTELEPATHY: @C%s@G telepaths @c%s, @W'@w%s@W'@n",
                ...
                GET_ADMLEVEL(tch) > 0 ? GET_NAME(tch) : GET_USER(tch),
                argument);
    return;
  }
}
...
ACMD(do_telepathy) {
  ...
  else if ((getCurKI(ch)) < GET_MAX_MANA(ch) / 40) {
    send_to_char(
        ch, "You do not have enough ki to focus your mental abilities.\r\n");
    return;
  }
  if (!(strcmp(arg, "far"))) {
    if (IS_NPC(ch)) {
      ...
      send_to_char(ch, "Their mental power oustrips your's by unfathomable "
                       "measurements!\r\n");
      return;
    } else if (AFF_FLAGGED(ch, AFF_SHOCKED)) {
      send_to_char(ch, "Your mind has been shocked by telepathic feedback! You "
                       "are not able to use telepathy right now.\r\n");
      return;
    } else if (IS_ANDROID(vict)) {
      send_to_char(ch,
                   "You can't touch the mind of such an artificial being.\r\n");
      return;
    } else if (GET_SKILL(vict, SKILL_TELEPATHY) + GET_INT(vict) >
               GET_SKILL(ch, SKILL_TELEPATHY) + GET_INT(ch)) {
      send_to_char(ch, "They throw off your attempt with their own telepathic "
                       "abilities!\r\n");
      return;
      ...
      if (GET_INT(vict) > GET_INT(ch)) {
        send_to_char(
            ch,
            "You feel like someone was using your mind for something...\r\n");
      }
      decCurKI(ch, getMaxKI(ch) / 40);
      return;
    }
  ...
  if (!(strcmp(arg, "link"))) {
    ...
    if (MINDLINK(ch)) {
      act("@CYou remove the link your mind had with @w$N.@n", TRUE, ch, 0,
          MINDLINK(ch), TO_CHAR);
      ...
      send_to_char(
          ch, "Someone else is already telepathically linked with them.\r\n");
      return;
    } else if (GET_SKILL(ch, SKILL_TELEPATHY) <
               axion_dice(GET_INT(vict) * 0.1)) {
      act("@R$n@r tried to link $s mind with yours, but you manage to force a "
          "break in the link!@n",
          FALSE, ch, 0, vict, TO_VICT);
      act("@R$N@r manages to sense the intrusion and with $S intelligence push "
          "you out!@n",
          FALSE, ch, 0, vict, TO_CHAR);
      return;
    } else {
      act("@CYou link your mind with @w$N.@n", TRUE, ch, 0, vict, TO_CHAR);
      act("@w$n@C links $s mind with yours. You can speak your thoughts to $m "
          "with 'think'.@n",
          TRUE, ch, 0, vict, TO_VICT);
    ...
    if (!(vict = get_char_vis(ch, arg2, NULL, FIND_CHAR_ROOM))) {
      ...
      if (axion_dice(0) > GET_SKILL(ch, SKILL_TELEPATHY)) {
        decCurKI(ch, getMaxKI(ch) / 40);
        act("@wYou attempt to read $N's@w mind, but fail to see it clearly.@n",
            TRUE, ch, 0, vict, TO_CHAR);
        ...
        return;
                 ...
                 rand_number(1, 2) == 2) {
        decCurKI(ch, getMaxKI(ch) / 40);
        act("@wYou fail to read @c$N's@w mind and they seemed to have noticed "
            "the attempt!@n",
            TRUE, ch, 0, vict, TO_CHAR);
        act("@C$n@w attempts to read your mind, but you resist and force $m "
            "out!@n",
            TRUE, ch, 0, vict, TO_VICT);
        improve_skill(ch, SKILL_TELEPATHY, 0);
        return;
    ...
    if (vict == ch) {
      ...
      if (!MINDLINK(ch)) {
        send_to_char(ch, "@WYou tell @c%s@W telepathically, @w'@C%s@w'@n\r\n",
                     GET_NAME(vict), arg2);
        send_to_char(vict,
                     "@c%s@W talks to you telepathically, @w'@C%s@w'@n\r\n",
                     GET_NAME(ch), arg2);
        send_to_imm("@GTELEPATHY: @C%s@G telepaths @c%s, @W'@w%s@W'@n",
                    GET_ADMLEVEL(ch) > 0 ? GET_NAME(ch) : GET_USER(ch),
                    GET_ADMLEVEL(vict) > 0 ? GET_NAME(vict) : GET_USER(vict),
                    arg2);
      } else {
        send_to_char(ch, "@WYou tell @c%s@W telepathically, @w'@C%s@w'@n\r\n",
...
ACMD(do_potential) {
  ...
  if (IS_NPC(vict)) {
    send_to_char(
        ch, "Why would you waste your time releasing their potential?\r\n");
    return;
  }
  if (vict == ch) {
  ...
  /* Rillao: transloc, add new transes here */
    ...
    gainBasePL(vict, (getBasePL(vict) / 100) * boost);
    if (IS_HALFBREED(vict)) {
      gainBaseKI(vict, (getBaseKI(vict) / 100) * boost);
      gainBaseST(vict, (getBaseST(vict) / 100) * boost);
    }
    reveal_hiding(ch, 0);
    act("You place your hand on top of $N's head. After a moment of "
        "concentrating you release their hidden potential.",
        TRUE, ch, 0, vict, TO_CHAR);
    act("$n places $s hand on top of your head. After a moment you feel a rush "
        "of power as your hidden potential is released!",
        TRUE, ch, 0, vict, TO_VICT);
    act("$n places $s hand on $N's head. After a moment a rush of power "
        "explodes off of $N's body!",
        TRUE, ch, 0, vict, TO_NOTVICT);
...
ACMD(do_majinize) {
  ...
  if (PLR_FLAGGED(vict, PLR_PR)) {
    send_to_char(
        ch, "You can't majinize them their potential has been released!\r\n");
    return;
  }
  ...
  if (char_condition_has(vict, "majinized") > 0 && char_condition_number_get(vict, "majinized", "lord") != GET_IDNUM(ch)) {
    ...
    send_to_char(ch, "Their alignment is so opposed to your's that they resist "
                     "your attempts to enslave them!\r\n");
    return;
  } else if (GET_MAX_HIT(vict) > GET_MAX_HIT(ch) * 4) {
    send_to_char(ch, "Their powerlevel is so much higher than yours they "
                     "resist your attempts to enslave them!\r\n");
    return;
  }
  /* Rillao: transloc, add new transes here */
    ...
    act("You remove $N's majinization, freeing them from your influence, but "
        "also weakening them.",
        TRUE, ch, 0, vict, TO_CHAR);
    act("$n removes your majinization, freeing you from their influence, and "
        "weakening you!",
        TRUE, ch, 0, vict, TO_VICT);
    ...
    reveal_hiding(ch, 0);
    act("You focus your power into $N, influencing their mind and increasing "
        "their strength! After the struggle ends in $S mind a glowing purple M "
        "forms on $S forehead.",
        TRUE, ch, 0, vict, TO_CHAR);
    act("$n focuses power into you, influencing your mind and increasing your "
        "strength! After the struggle in your mind ends a glowing purple M "
        "forms on your forehead.",
        TRUE, ch, 0, vict, TO_VICT);
    act("$n focuses power into $N, influencing their mind and increasing their "
...
ACMD(do_spit) {
  ...
  if (FIGHTING(vict)) {
    send_to_char(vict, "You can't manage to spit on them, they are moving "
                       "around too much!\r\n");
    return;
  }

  cost = ((GET_MAX_MANA(ch) / (GET_SKILL(ch, SKILL_SPIT) / 4)) +
          GET_MAX_MANA(ch) / 100);

  if ((getCurKI(ch)) < cost) {
    send_to_char(ch,
                 "You do not have enough ki to petrifiy with your spit!\r\n");
    return;
  }
  ...
  if (GET_SKILL(ch, SKILL_SPIT) < axion_dice(0)) {
    decCurKI(ch, cost);
    reveal_hiding(ch, 0);
    act("@WGathering spit you concentrate ki into a wicked loogie and let it "
        "loose, but it falls short of hitting @c$N@W!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@C$n@W seems to focus ki before hawking a loogie at you! Fortunatly "
        "the loogie falls short.@n",
        TRUE, ch, 0, vict, TO_VICT);
    act("@C$n@W seems to focus ki before hawking a loogie at @c$N@W! "
        "Fortunatly for @c$N@W the loogie falls short.@n",
        TRUE, ch, 0, vict, TO_NOTVICT);
    ...
    decCurKI(ch, cost);
    reveal_hiding(ch, 0);
    act("@C$N@c disappears, avoiding your spit before reappearing!@n", FALSE,
        ch, 0, vict, TO_CHAR);
    act("@cYou disappear, avoiding @C$n's@c @rstone spit@c before "
        "reappearing!@n",
        FALSE, ch, 0, vict, TO_VICT);
    ...
    reveal_hiding(ch, 0);
    act("@WGathering spit you concentrate ki into a wicked loogie and let it "
        "loose, and it smacks into @c$N@W turning $M into stone!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@C$n@W seems to focus ki before hawking a loogie at you! It manages "
        "to hit and you instantly turn to stone!@n",
        TRUE, ch, 0, vict, TO_VICT);
    act("@C$n@W seems to focus ki before hawking a loogie at @c$N@W! It "
        "manages to hit and $E instantly turns to stone!@n",
        TRUE, ch, 0, vict, TO_NOTVICT);
    improve_skill(ch, SKILL_SPIT, 1);
    WAIT_STATE(ch, PULSE_2SEC);
    return;
...
ACMD(do_recharge) {
  ...
  if (GET_COOLDOWN(ch) > 0) {
    send_to_char(ch, "You must wait a short period before your nanites can "
                     "convert your ki.\r\n");
    return;
  }
  ...
  if (!PLR_FLAGGED(ch, PLR_REPAIR)) {
    ...
    if ((getCurKI(ch)) < cost) {
      send_to_char(ch,
                   "You do not have enough ki to recharge your stamina.\r\n");
      return;
      ...
      reveal_hiding(ch, 0);
      GET_COOLDOWN(ch) = 10;
      act("You focus your ki into your energy reserves, recharging them some.",
          TRUE, ch, 0, 0, TO_CHAR);
      act("$n stops and glows green briefly.", TRUE, ch, 0, 0, TO_ROOM);
      decCurKI(ch, cost);
      if ((getCurST(ch)) + (cost * 2) < GET_MAX_MOVE(ch)) {
        incCurST(ch, cost * 2);
      } else {
...
ACMD(do_srepair) {
  ...
  if (GET_COOLDOWN(ch) > 0) {
    send_to_char(
        ch,
        "You must wait a short period before your nanites can repair you.\r\n");
    return;
  }
  ...
  if (!IS_NPC(ch) && !PLR_FLAGGED(ch, PLR_REPAIR)) {
    ...
    if ((getCurST(ch)) < cost) {
      send_to_char(ch,
                   "You do not have enough stamina to repair yourself.\r\n");
      return;
    } else if (GET_HIT(ch) >= getMaxPL(ch)) {
      send_to_char(ch, "You are already at full functionality and do not "
                       "require repairs.\r\n");
      return;
    } else {
      reveal_hiding(ch, 0);
      GET_COOLDOWN(ch) = 10;
      act("You repair some of your outer casings and internal systems, with "
          "the small nano-robots contained in your body.",
          TRUE, ch, 0, 0, TO_CHAR);
      act("$n stops a moment as small glowing particles move across $s body.",
          TRUE, ch, 0, 0, TO_ROOM);
      ...
      if (repaired == TRUE) {
        send_to_char(ch, "@GYour nano-robots also repair all of your equipment "
                         "a little bit.@n\r\n");
      }
      decCurST(ch, cost);
      heal = cost * 2;
      ...
      if (!IS_NPC(ch) && rand_number(1, 3) == 2 &&
          (getCurKI(ch)) < GET_MAX_MANA(ch)) {
        send_to_char(ch, "@GThe repairs have managed to relink power reserves "
                         "and boost your current energy level.@n\r\n");
        incCurKI(ch, cost);
      }
      WAIT_STATE(ch, PULSE_2SEC);
    }
...
ACMD(do_upgrade) {
  ...
  if (!strcasecmp("augment", arg)) {
    ...
    if (!(obj = get_obj_in_list_vis(ch, "Augmentation", NULL, inv_for_char(ch)))) {
      ...
      case 90:
        gain =
            GET_MAX_HIT(ch) * 0.005; /* R: Original was GET_LEVEL(ch) * 2000; */
        break;
      case 91:
      case 92:
      case 93:
      ...
      case 99:
        gain =
            GET_MAX_HIT(ch) * 0.005; /* R: Original was GET_LEVEL(ch) * 4000; */
        break;
      case 100:
        gain = GET_MAX_HIT(ch) *
               0.005; /* R: Original was GET_LEVEL(ch) * 100000 */
        if (gain > 10000000) {
          gain = 10000000;
        }
        break;
      ...
      if (!strcasecmp("powerlevel", arg2)) {
        obj_from_char(obj);
        extract_obj(obj);
        act("@WYou install the circuits and upgrade your maximum powerlevel.@n",
            TRUE, ch, 0, 0, TO_CHAR);
        act("@C$n@W installs some circuits and upgrades $s systems.@n", TRUE,
            ch, 0, 0, TO_ROOM);
        ...
        obj_from_char(obj);
        extract_obj(obj);
        act("@WYou install the circuits and upgrade your maximum ki.@n", TRUE,
            ch, 0, 0, TO_CHAR);
        act("@C$n@W installs some circuits and upgrades $s systems.@n", TRUE,
            ch, 0, 0, TO_ROOM);
        ...
        obj_from_char(obj);
        extract_obj(obj);
        act("@WYou install the circuits and upgrade your maximum stamina.@n",
            TRUE, ch, 0, 0, TO_CHAR);
        act("@C$n@W installs some circuits and upgrades $s systems.@n", TRUE,
            ch, 0, 0, TO_ROOM);
        ...
        send_to_char(
            ch, "What do you want to augment? Powerlevel, ki, or stamina?\r\n");
        return;
      }
    }
  }
  ...
  if (PLR_FLAGGED(ch, PLR_ABSORB)) {
    send_to_char(ch, "You are an absorb model and can only upgrade with "
                     "augmentation kits.\r\n");
    return;
  }
  ...
  if (is_soft_cap(ch, 0)) {
    send_to_char(
        ch, "@mYou are unable to spend anymore UGP right now (Softcap)@n\r\n");
    return;
  }

  if (!*arg2 && (!strcasecmp("powerlevel", arg) || !strcasecmp("ki", arg) ||
                 !strcasecmp("stamina", arg))) {
    send_to_char(ch, "How many times do you want to increase %s?", arg);
    return;
  }

  if (atoi(arg2) <= 0 &&
      (!strcasecmp("powerlevel", arg) || !strcasecmp("ki", arg) ||
       !strcasecmp("stamina", arg))) {
  ...
  if (!strcasecmp("powerlevel", arg)) {
    ...
    if (cost > GET_UP(ch)) {
      ...
      return;
    } else if (is_soft_cap(ch, bonus)) {
      send_to_char(ch, "@mYou can't spend that much UGP on it as it will go "
                       "over your softcap.@n\r\n");
      return;
    } else {
      char_stat_mod(ch, "upgrades", -cost);
      send_to_char(ch, "You upgrade your system and gain %s %s!",
                   add_commas(bonus), arg);
...
ACMD(do_absorb) {
  ...
  if (!IS_ANDROID(ch) && !IS_BIO(ch)) {
    send_to_char(
        ch,
        "You shouldn't have this skill, you are incapable of absorbing.\r\n");
    send_to_imm("ERROR: Absorb skill on %s when they are not a bio or android.",
                GET_NAME(ch));
    return;
  }
  ...
  if (GRAPPLED(ch)) {
    send_to_char(ch,
                 "You are currently being grappled with! Try 'escape'!\r\n");
    return;
  }
  if (GRAPPLING(ch)) {
  ...
  if (ABSORBING(ch)) {
    act("@WYou stop absorbing from @c$N@W!@n", TRUE, ch, 0, ABSORBING(ch),
        TO_CHAR);
  ...
  if (IS_ANDROID(ch)) {
    ...
    if (GET_MAX_HIT(vict) * 20 < GET_MAX_HIT(ch)) {
      send_to_char(ch,
                   "They are too weak for you to bother absorbing from.\r\n");
      return;
    }
    if ((getCurST(vict)) < (GET_MAX_MOVE(vict) / 20) &&
        (getCurKI(vict)) < (GET_MAX_MANA(vict) / 20)) {
      send_to_char(
          ch, "They have nothing to absorb right now, they are drained...\r\n");
      return;
    }
    reveal_hiding(ch, 0);
    if (init_skill(ch, SKILL_ABSORB) < axion_dice(0)) {
      act("@WYou rush at @c$N@W and try to absorb from them, but $E manages to "
          "avoid you!@n",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@C$n@W rushes at you and tries to grab you, but you manage to avoid "
          "$m!@n",
          TRUE, ch, 0, vict, TO_VICT);
      act("@C$n@w rushes at @c$N@W and tries to grab $M, but @c$N@W manages to "
          "avoid @c$n@W!@n",
          TRUE, ch, 0, vict, TO_NOTVICT);
      ...
      act("@WYou rush at @c$N@W and try to absorb from them, and manage to "
          "grab on!@n",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@C$n@W rushes at you and tries to grab you, and manages to grab "
          "on!@n",
          TRUE, ch, 0, vict, TO_VICT);
      act("@C$n@w rushes at @c$N@W and tries to grab $M, and manages to grab "
          "on!@n",
          TRUE, ch, 0, vict, TO_NOTVICT);
      improve_skill(ch, SKILL_ABSORB, 1);
    ...
    if (GET_MAX_HIT(vict) >= (getBasePL(ch)) * 3) {
      send_to_char(
          ch,
          "You are too weak to absorb them into your cellular structure!\r\n");
      return;
    }
    reveal_hiding(ch, 0);
    if (GET_SKILL(ch, SKILL_ABSORB) < axion_dice(0)) {
      ...
      act("@WYou rush at @c$N@W and your tail engulfs $M! You quickly suck $S "
          "squirming body into your tail, absorbing $m!@n",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@C$n@W rushes at you and $s tail engulfs you! $e quickly sucks your "
          "squirming body into $s tail, absorbing you!@n",
          TRUE, ch, 0, vict, TO_VICT);
      act("@C$n@w rushes at @c$N@W and $s tail engulfs $M! You quickly suck $S "
          "squirming body into your tail, absorbing @c$N@W!@n",
          TRUE, ch, 0, vict, TO_NOTVICT);
      GET_ABSORBS(ch) -= 1;

      int64_t stam = (getBaseST(vict)) / 5;
      ...
      send_to_char(ch,
                   "@D[@gABSORB@D] @rPL@W: @D(@y%s@D) @cKi@W: @D(@y%s@D) "
                   "@gSt@W: @D(@y%s@D)@n\r\n",
                   add_commas(pl), add_commas(ki), add_commas(stam));
      improve_skill(ch, SKILL_ABSORB, 1);
      die(vict, NULL);
    ...
    if (!IS_NPC(vict)) {
      send_to_char(ch, "You can't absorb their bio extract, you need to "
                       "swallow them with your tail!\r\n");
      return;
    }
    if (is_soft_cap(ch, 0)) {
      send_to_char(
          ch,
          "You can not handle any more bio extract at your current level.\r\n");
      return;
    }
    ...
    /* Rillao: transloc, add new transes here */
      ...
      act("@WYou rush at @c$N@W and stab them with your tail! You quickly suck "
          "out all the bio extract you need and leave the empty husk behind!",
          TRUE, ch, 0, vict, TO_CHAR);
      act("@C$n@w rushes at @c$N@W and stabs $M with $s tail! $e quickly sucks "
          "out all the bio extract and leaves the empty husk of @c$N@W "
          "behind!@n",
          TRUE, ch, 0, vict, TO_NOTVICT);
      int64_t stam = (getBaseST(vict)) / 2000;
      int64_t ki = (getBaseKI(vict)) / 2000;
      int64_t pl = (getBasePL(vict)) / 2000;
    ...
    if (!IS_BIO(ch) && !IS_ANDROID(ch)) {
      send_to_char(ch, "You have the absorb skill but are incapable of "
                       "absorbing. This error has been reported.\r\n");
      send_to_imm("ERROR: Absorb attempted by %s even though they are not bio "
                  "or android.",
                  GET_NAME(ch));
    } else {
...
ACMD(do_escape) {
  ...
  int skill = ABSORBBY(ch) ? GET_SKILL(ABSORBBY(ch), SKILL_ABSORB)
                           : GET_SKILL(GRAPPLED(ch), SKILL_GRAPPLE);
  ...
  if (ABSORBBY(ch)) {
    ...
    if (escaped) {
      ...
      act("@c$N@W struggles to break loose of @C$n's@W hold!@n", TRUE,
          ABSORBBY(ch), 0, ch, TO_NOTVICT);
      act("@WYou struggle to break loose of @C$n's@W hold!@n", TRUE,
          ABSORBBY(ch), 0, ch, TO_VICT);
      act("@c$N@W struggles to break loose of your hold!@n", TRUE, ABSORBBY(ch),
          0, ch, TO_CHAR);
      if (rand_number(1, 3) == 3) {
  ...
  if (GRAPPLED(ch)) {
    ...
    if (escaped) {
      if (GRAPTYPE(GRAPPLED(ch)) == 4) {
        act("@c$N@M flexes with all $S might and causes your body to explode "
            "outward into gooey chunks!@n",
            TRUE, GRAPPLED(ch), 0, ch, TO_CHAR);
        act("@MYou flex with all your might and cause @C$n's@M body to explode "
            "outward into gooey chunks!@n",
            TRUE, GRAPPLED(ch), 0, ch, TO_VICT);
        act("@c$N@M flexes with all $S might and causes @C$n's@M body to "
            "explode outward into gooey chunks!@n",
            TRUE, GRAPPLED(ch), 0, ch, TO_NOTVICT);

        act("@MYou reform your body mere moments later.@n", TRUE, GRAPPLED(ch),
            0, 0, TO_CHAR);
        act("@C$n@M reforms $s body mere moments later.", TRUE, GRAPPLED(ch), 0,
            0, TO_ROOM);
      } else {
        act("@c$N@W manages to break loose of @C$n's@W hold!@n", TRUE,
        ...
        act("@c$N@W manages to break loose of your hold!@n", TRUE, GRAPPLED(ch),
            0, ch, TO_CHAR);
      }
      if (FIGHTING(ch) == NULL) {
      ...
      act("@WYou struggle to break loose of @C$n's@W hold!@n", TRUE,
          GRAPPLED(ch), 0, ch, TO_VICT);
      act("@c$N@W struggles to break loose of your hold!@n", TRUE, GRAPPLED(ch),
          0, ch, TO_CHAR);
      if (rand_number(1, 3) == 3) {
        int64_t dmg = GET_MAX_HIT(ch) * 0.025;
...
ACMD(do_regenerate) {
  ...
  if (!*arg) {
    send_to_char(ch,
                 "Regenerate how much PL?\r\nMax percent you can regen: "
  ...
  if (GET_HIT(ch) >= getMaxPL(ch)) {
    send_to_char(ch,
                 "You do not need to regenerate, you are at full health.\r\n");
    return;
  }

  if (GET_SUPPRESS(ch) > 0 &&
      GET_HIT(ch) >= ((getMaxPL(ch) / 100) * GET_SUPPRESS(ch))) {
    send_to_char(ch,
                 "You do not need to regenerate, you are at full health.\r\n");
    return;
  }
  ...
  if (num <= 0) {
    send_to_char(
        ch, "What is the point of that?\r\nSyntax: regenerate (1 - 100)\r\n");
    return;
  }

  if (num > 100) {
    send_to_char(ch,
                 "You can't regenerate that much!\r\nMax you can regen: %d\r\n",
                 skill);
    return;
  }
  ...
  int64_t life = ((getCurLF(ch)) - amt * 0.8),
          energy = ((getCurKI(ch)) - amt * 0.2);

  if ((life <= 0 || energy <= 0) && !IS_NPC(ch)) {
    send_to_char(
        ch, "Your life force or ki are too low to regenerate that much.\r\n");
  ...
  if (GET_HIT(ch) >= getMaxPL(ch)) {
    act("You concentrate your ki and regenerate your body completely.", TRUE,
        ch, 0, 0, TO_CHAR);
    act("$n concentrates and regenerates $s body completely.", TRUE, ch, 0, 0,
        TO_ROOM);
  } else if (amt < GET_MAX_HIT(ch) / 10) {
    act("You concentrate your ki and regenerate your body a little.", TRUE, ch,
        0, 0, TO_CHAR);
    act("$n concentrates and regenerates $s body a little.", TRUE, ch, 0, 0,
        TO_ROOM);
  } else if (amt < GET_MAX_HIT(ch) / 5) {
    act("You concentrate your ki and regenerate your body some.", TRUE, ch, 0,
        0, TO_CHAR);
    act("$n concentrates and regenerates $s body some.", TRUE, ch, 0, 0,
        TO_ROOM);
  } else if (amt < GET_MAX_HIT(ch) / 2) {
    act("You concentrate your ki and regenerate your body a great deal.", TRUE,
        ch, 0, 0, TO_CHAR);
    act("$n concentrates and regenerates $s body a great deal.", TRUE, ch, 0, 0,
        TO_ROOM);
  } else if (GET_HIT(ch) < GET_MAX_HIT(ch)) {
    act("You concentrate your ki and regenerate you nearly completely.", TRUE,
        ch, 0, 0, TO_CHAR);
    act("$n concentrates and regenerates $s body nearly completely.", TRUE, ch,
        0, 0, TO_ROOM);
  }
...
ACMD(do_kaioken) {
  ...
  if (x == GET_KAIOKEN(ch)) {
    send_to_char(
        ch,
        "You are already at that kaioken level! To release, try kaioken 0\r\n");
    return;
  }

  if (!IS_NPC(ch)) {
    if ((IS_TRANSFORMED(ch) || (IS_HOSHIJIN(ch) && GET_PHASE(ch) > 0)) &&
        x > 5) {
      send_to_char(ch, "You can not manage a kaioken level higher than 5 when "
                       "transformed.\r\n");
      return;
    }
  }
  ...
  if ((getCurKI(ch)) < cost) {
    send_to_char(ch, "You do not have enough ki to focus into your body for "
                     "that level.\r\n");
    return;
  }
  ...
  if (init_skill(ch, SKILL_KAIOKEN) < roll) {
    send_to_char(
        ch, "You try to focus your ki into your body but mess up somehow.\r\n");
    act("$n tries to use kaioken but messes up somehow.", TRUE, ch, 0, 0,
        TO_ROOM);
    WAIT_STATE(ch, PULSE_1SEC);
    return;
  }

  if (getMaxHealth(ch) < kaioken_levels[x]) {
    act("@rA blazing red aura bursts up around your body, flashing intensely "
        "before your body gives out and you release the kaioken because of the "
        "pressure!@n",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@rA blazing red aura bursts up around @R$n's @rbody, flashing "
        "intensely before $s body gives out and $e releases the kaioken "
        "because of the pressure!@n",
        TRUE, ch, 0, 0, TO_ROOM);
    return;
  }

  apply_kaioken(ch, x, true);
...
ACMD(do_plant) {
  ...
  if ((ADM_FLAGGED(vict, ADM_NOSTEAL) || GET_MOB_SPEC(vict) == shop_keeper) &&
      GET_ADMLEVEL(ch) < 5)
    roll = -10; /* Failure */
  ...
  if (roll <= detect && roll <= fail) {
    ...
    act("@C$n@w tries to plant $p@w on @c$N@w!@n", TRUE, ch, obj, vict,
        TO_NOTVICT);
    act("@wYou try and fail to plant $p@w on @c$N@w, and $E notices!@n", TRUE,
        ch, obj, vict, TO_CHAR);
    WAIT_STATE(ch, PULSE_2SEC);
    return;
  } else if (roll <= fail) {
    act("@wYou try and fail to plant $p@w on @c$N@w! However no one seemed to "
        "notice.@n",
        TRUE, ch, obj, vict, TO_CHAR);
    WAIT_STATE(ch, PULSE_2SEC);
    return;
  } else if (GET_OBJ_WEIGHT(obj) + (getCurCarriedWeight(vict)) >
             CAN_CARRY_W(vict)) {
    reveal_hiding(ch, 0);
    act("@C$n@w tries to plant $p@w on you!@n", TRUE, ch, obj, vict, TO_VICT);
    act("@C$n@w tries to plant $p@w on @c$N@w!@n", TRUE, ch, obj, vict,
        TO_NOTVICT);
    act("@wYou try and fail to plant $p@w on @c$N@w because $E can't carry the "
        "weight. It seems $E noticed the attempt!@n",
        TRUE, ch, obj, vict, TO_CHAR);
    WAIT_STATE(ch, PULSE_2SEC);
  } else if (roll <= detect) {
    act("@cYou feel like the weight of your inventory has changed.@n", TRUE, ch,
        obj, vict, TO_VICT);
    act("@c$N@w looks around after feeling $S pockets.@n", TRUE, ch, obj, vict,
        TO_NOTVICT);
    act("@wYou plant $p@w on @c$N@w! @c$N @wseems to notice the change in "
        "weight in their inventory.@n",
        TRUE, ch, obj, vict, TO_CHAR);
    obj_from_char(obj);
    ...
    act("@wYou plant $p@w on @c$N@w! No one noticed, whew....@n", TRUE, ch, obj,
        vict, TO_CHAR);
    obj_from_char(obj);
    obj_to_char(obj, vict);
    WAIT_STATE(ch, PULSE_2SEC);
...
ACMD(do_forgery) {
  ...
  if (OBJ_FLAGGED(obj2, ITEM_FORGED)) {
    send_to_char(
        ch, "%s is forgery, there is no reason to make a fake of a fake!\r\n",
  ...
  if (OBJ_FLAGGED(obj2, ITEM_BROKEN)) {
    send_to_char(
        ch, "%s is broken, there is no reason to make a fake of this mess!\r\n",
        obj2->short_description);
    WAIT_STATE(ch, PULSE_2SEC);
    return;
  }
  ...
  if (GET_OBJ_VNUM(obj2) >= 60000 || GET_OBJ_VNUM(obj2) == 0) {
    send_to_char(
        ch, "You can not make a forgery of that! It's far too squishy....");
    return;
  }
  ...
  if (OBJ_FLAGGED(obj2, ITEM_PROTECTED)) {
    send_to_char(ch,
                 "You don't know where to begin with this work of ART.\r\n");
    return;
  }

  reveal_hiding(ch, 0);
  act("@c$n@w looks at $p, begins to work on forging a fake copy of it.@n",
      TRUE, ch, obj2, 0, TO_ROOM);
  improve_skill(ch, SKILL_FORGERY, 1);
  if (GET_SKILL(ch, SKILL_FORGERY) < axion_dice(0)) {
    if (rand_number(1, 10) >= 9) { /* Uh oh */
      send_to_char(ch,
                   "In the middle of creating a forgery of %s you screw up. "
                   "The fabrication unit built into the forgery kit melts and "
                   "bonds with the original. You clumsy mistake with the Estex "
                   "Titanium drill has broken both.\r\n",
                   obj2->short_description);
      extract_obj(obj4);
      extract_obj(obj2);
      return;
    ...
    send_to_char(ch,
                 "You start to make a forgery of %s but screw up and waste "
                 "your forgery kit..\r\n",
                 obj2->short_description);
    act("@c$n@w tried to duplicate $p but screws up somehow.@n", TRUE, ch, obj2,
        0, TO_ROOM);
    obj_from_char(obj4);
    extract_obj(obj4);
    WAIT_STATE(ch, PULSE_2SEC);
  ...
  GET_OBJ_WEIGHT(obj3) =
      rand_number(GET_OBJ_WEIGHT(obj3) / 2, GET_OBJ_WEIGHT(obj3));
...
ACMD(do_appraise) {
  ...
  reveal_hiding(ch, 0);
  act("@c$n@w looks at $p, turning it over in $s hands.@n", TRUE, ch, obj, 0,
      TO_ROOM);
  ...
  send_to_char(ch, "%s is worth: %s\r\nMin Lvl: %d\r\n", obj->short_description,
               add_commas(GET_OBJ_COST(obj)), displevel);
  ...
  } else if (OBJ_FLAGGED(obj, ITEM_SLOT1) &&
  ...
  } else if (OBJ_FLAGGED(obj, ITEM_SLOT2) &&
             !OBJ_FLAGGED(obj, ITEM_SLOTS_FILLED)) {
    send_to_char(ch, "Token Slots  : @m2/2@n\n");
  }
  send_to_char(ch, "Bonuses:");
  act("@c$n@w looks at $p and nods, a satisfied look on $s face.@n", TRUE, ch,
      obj, 0, TO_ROOM);
  int percent = FALSE;
  for (i = 0; i < MAX_OBJ_AFFECT; i++) {
    if (obj->affected[i].modifier) {
      if (obj->affected[i].location == APPLY_REGEN ||
          obj->affected[i].location == APPLY_TRAIN ||
          obj->affected[i].location == APPLY_LIFEMAX) {
        percent = TRUE;
      }
      sprinttype(obj->affected[i].location, apply_types, buf, sizeof(buf));
      send_to_char(ch, "%s %+d%s to %s", found++ ? "," : "",
                   obj->affected[i].modifier, percent == TRUE ? "%" : "", buf);
      percent = FALSE;
...
ACMD(do_disguise) {
  ...
  if (PLR_FLAGGED(ch, PLR_DISGUISED)) {
    ...
    act("@C$n @wpulls off $s disguise and reveals $mself!", TRUE, ch, 0, 0,
        TO_ROOM);
    return;
  }
  ...
  if (!GET_EQ(ch, WEAR_HEAD)) {
    send_to_char(
        ch,
        "You can't disguise your identity without anything on your head.\r\n");
    return;
  }

  if ((getCurST(ch)) < (getCurST(ch)) / 50) {
  ...
  if (skill > roll) {
    send_to_char(ch, "You managed to disguise yourself with some skilled "
                     "manipulation of your headwear.\r\n");
    act("@C$n @wmanages to disguise $mself with some skilled manipulation of "
        "$s headwear.",
        TRUE, ch, 0, 0, TO_ROOM);
    SET_BIT_AR(PLR_FLAGS(ch), PLR_DISGUISED);
    return;
  } else {
    send_to_char(ch, "You finish attempting to disguise yourself, but realize "
                     "you failed and need to try again.\r\n");
    act("@C$n @wattempts and fails to disguise $mself properly and must try "
        "again.",
        TRUE, ch, 0, 0, TO_ROOM);
    decCurST(ch, getMaxST(ch) / 50);
    return;
  }
}
...
ACMD(do_situp) {
  ...
  if ((gravity != 10 && IS_BARDOCK(ch)) || !IS_BARDOCK(ch)) {
    cost += gravity * ((gravity * 2) + GET_LEVEL(ch));
  } else if (IS_BARDOCK(ch)) {
    cost += gravity * GET_LEVEL(ch);
  }
  ...
  if (gravity == 300) {
    ...
    cost += 15000000;
  } else if (gravity == 5000) {
    cost += 25000000;
  } else if (gravity == 10000) {
    cost += 50000000;
  }
  ...
  if (PLR_FLAGGED(ch, PLR_SPAR)) {
    send_to_char(ch, "You shouldn't be sparring if you want to work out, it "
                     "could be dangerous.\r\n");
    return;
  }
  if (FIGHTING(ch)) {
  ...
  if (char_condition_has(ch, "flying")) {
    ...
    if (gravity == 0) {
      ...
      act("@gYou do a situp, feeling the strain of gravity.@n", TRUE, ch, 0, 0,
          TO_CHAR);
      act("@g$n does a situp, while sweating.@n", TRUE, ch, 0, 0, TO_ROOM);
    } else if (gravity == 20) {
      bonus = rand_number(8, 14);
      act("@gYou do a situp, and feel gravity's pull.@n", TRUE, ch, 0, 0,
          TO_CHAR);
      act("@g$n does a situp, while sweating.@n", TRUE, ch, 0, 0, TO_ROOM);
    } else if (gravity == 30) {
      ...
      act("@gYou do a situp, and really strain against the gravity.@n", TRUE,
          ch, 0, 0, TO_CHAR);
      act("@g$n does a situp, while sweating.@n", TRUE, ch, 0, 0, TO_ROOM);
    } else if (gravity == 200) {
      ...
      act("@gYou do a situp, and it was a really hard one to finish.@n", TRUE,
          ch, 0, 0, TO_CHAR);
      act("@g$n does a situp, while sweating profusely.@n", TRUE, ch, 0, 0,
          TO_ROOM);
    } else if (gravity == 5000) {
    ...
    if (cost >= GET_MAX_MOVE(ch) / 2) {
      ...
      send_to_char(ch, "This gravity is so easy to you, you could do it in "
                       "your sleep...\r\n");
      bonus /= 8;
    } else if (cost < GET_MAX_MOVE(ch) / 100) {
      send_to_char(
          ch, "This gravity is the opposite of a challenge for you...\r\n");
      bonus /= 5;
    } else if (cost < GET_MAX_MOVE(ch) / 50) {
      send_to_char(ch,
                   "This gravity is definitely not a challenge for you...\r\n");
      bonus /= 4;
    ...
    if (room_flagged(room, ROOM_HBTC)) {
      send_to_char(ch, "@rThis place feels like it operates on a different "
                       "time frame, it feels great...@n\r\n");
      bonus *= 10;
    ...
    if (bonus <= 0 && !room_flagged(room, ROOM_HBTC)) {
      bonus = 1;
    }
    if (bonus <= 0 && room_flagged(room, ROOM_HBTC)) {
      bonus = 15;
...
ACMD(do_meditate) {
  ...
  if (!strcasecmp(arg, "expand")) {
    ...
    if (GET_PRACTICES(ch, GET_CLASS(ch)) < cost) {
      send_to_char(ch, "You do not have enough practice sessions to expand "
                       "your mind and ability to remember skills.\r\n");
      send_to_char(ch, "%s needed.\r\n", add_commas(cost));
    } else if (GET_SLOTS(ch) >= 60 && GET_BONUS(ch, BONUS_GMEMORY) == 0) {
      send_to_char(ch,
                   "You can not have any more slots through this process.\r\n");
    } else if (GET_SLOTS(ch) >= 65 && GET_BONUS(ch, BONUS_GMEMORY) == 1) {
      send_to_char(ch,
                   "You can not have any more slots through this process.\r\n");
    } else {
      send_to_char(ch,
                   "During your meditation you manage to expand your mind and "
                   "get the feeling you could learn some new skills.\r\n");
      char_stat_mod(ch, "skill_slots", 1);
    ...
    if (MINDLINK(ch) == NULL) {
      ...
      return;
    } else if ((getCurKI(ch)) < GET_MAX_MANA(MINDLINK(ch)) * 0.05) {
      send_to_char(ch, "You do not have enough ki to manage a break.\r\n");
      return;
    } else if (GET_INT(ch) + rand_number(-5, 10) >=
               GET_INT(MINDLINK(ch)) +
                   (GET_SKILL(MINDLINK(ch), SKILL_TELEPATHY) * 0.1)) {
      act("@rYou manage to break the mind link between you and @R$N@r!@n",
          FALSE, ch, 0, MINDLINK(ch), TO_CHAR);
      act("$n closes their eyes for a few seconds.", FALSE, ch, 0, MINDLINK(ch),
          TO_ROOM);
      send_to_char(MINDLINK(ch),
                   "@rYour mind linked target manages to push you out!@n\r\n");
      if (GET_INT(MINDLINK(ch)) < axion_dice(-10) &&
      ...
      act("@rYou struggle to free your mind of @R$N's@r link, but fail!@n",
          FALSE, ch, 0, MINDLINK(ch), TO_CHAR);
      act("$n closes their eyes for a few seconds, and appears to struggle "
          "quite a bit.",
          FALSE, ch, 0, MINDLINK(ch), TO_ROOM);
      send_to_char(MINDLINK(ch), "@rYour mind linked target struggles to free "
                                 "their mind, but fails!@n\r\n");
  ...
  if ((getCurKI(ch)) < cost) {
    ...
    /* Meditate Message */
    act("@cYou close your eyes and concentrate, lifting $p@c with your ki.@n",
        TRUE, ch, obj, 0, TO_CHAR);
    act("@c$n closes $s eyes and lifts $p@c with $s ki.@n", TRUE, ch, obj, 0,
        TO_ROOM);
    ...
    if (cost >= GET_MAX_MANA(ch) / 2) {
      send_to_char(
          ch,
          "The object weight and gravity are a great challenge for you!\r\n");
      bonus *= 10;
    } else if (cost >= GET_MAX_MANA(ch) / 4) {
      send_to_char(ch, "The object weight and gravity are an awesome challenge "
                       "for you!\r\n");
      bonus *= 8;
    } else if (cost >= GET_MAX_MANA(ch) / 10) {
      send_to_char(
          ch,
          "The object weight and gravity are a good challenge for you!\r\n");
      bonus *= 6;
    } else if (cost < GET_MAX_MANA(ch) / 1000) {
      send_to_char(ch, "The object weight and gravity are so easy to you, you "
                       "could do it in your sleep....\r\n");
      bonus /= 8;
    } else if (cost < GET_MAX_MANA(ch) / 100) {
      send_to_char(ch, "The object weight and gravity are the opposite of a "
                       "challenge for you....\r\n");
      bonus /= 5;
    } else if (cost < GET_MAX_MANA(ch) / 50) {
      send_to_char(ch, "The object weight and gravity are definitely not a "
                       "challenge for you....\r\n");
      bonus /= 4;
    } else if (cost < GET_MAX_MANA(ch) / 30) {
      send_to_char(ch, "The object weight and gravity are barely a challenge "
                       "for you....\r\n");
      bonus /= 3;
    } else if (cost < GET_MAX_MANA(ch) / 20) {
      send_to_char(ch, "The object weight and gravity are hardly a challenge "
                       "for you....\r\n");
      bonus /= 2;
    } else {
      send_to_char(ch, "This gravity is just perfect for you....\r\n");
    ...
    if (bonus != 1 && IS_DEMON(ch) && rand_number(1, 100) >= 80) {
      send_to_char(
          ch,
          "Your spirit magnifies the strength of your body! @D[@G+%s@D]@n\r\n",
          add_commas(bonus / 2));
      gainBasePL(ch, bonus / 2);
    }
...
ACMD(do_pushup) {
  ...
  if (char_condition_has(ch, "flying")) {
    ...
    if (gravity == 0) {
      ...
      bonus = rand_number(3, 7);
      act("@gYou do a pushup, feeling the strain of gravity.@n", TRUE, ch, 0, 0,
          TO_CHAR);
      act("@g$n does a pushup, while sweating.@n", TRUE, ch, 0, 0, TO_ROOM);
    } else if (gravity == 20) {
      bonus = rand_number(8, 14);
      act("@gYou do a pushup, and feel gravity's pull.@n", TRUE, ch, 0, 0,
          TO_CHAR);
      act("@g$n does a pushup, while sweating.@n", TRUE, ch, 0, 0, TO_ROOM);
    } else if (gravity == 30) {
      bonus = rand_number(14, 20);
      ...
      bonus = rand_number(180, 250);
      act("@gYou do a pushup, and really strain against the gravity.@n", TRUE,
          ch, 0, 0, TO_CHAR);
      act("@g$n does a pushup, while sweating.@n", TRUE, ch, 0, 0, TO_ROOM);
    } else if (gravity == 200) {
      bonus = rand_number(400, 600);
      ...
      bonus = rand_number(9000, 10000);
      act("@gYou do a pushup, and it was a really hard one to finish.@n", TRUE,
          ch, 0, 0, TO_CHAR);
      act("@g$n does a pushup, while sweating profusely.@n", TRUE, ch, 0, 0,
          TO_ROOM);
    } else if (gravity == 5000) {
      bonus = rand_number(15000, 20000);
      act("@gYou do a pushup, and it was a really hard one to finish.@n", TRUE,
    ...
    if (cost >= getMaxPL(ch) / 2) {
      ...
      send_to_char(
          ch, "This gravity is definitely not a challenge for you....\r\n");
      bonus /= 4;
...
static void tick_player_flag_cleanup(struct char_data *ch) {
  ...
  if (!FIGHTING(ch) && char_condition_has(ch, "advantageous_position"))
    char_condition_remove(ch, "advantageous_position", "not_fighting");
...
ACMD(do_scouter) {
  ...
  if (!obj) {
    ...
    if (!strcasecmp("scan", arg)) {
      for (i = descriptor_list; i; i = i->next) {
        if (STATE(i) != CON_PLAYING) {
          ...
          int dir =
              find_first_step(char_room_get(ch), char_room_get(i->character));
          int same = FALSE;
          ...
          if (OBJ_FLAGGED(obj, ITEM_BSCOUTER) &&
              GET_HIT(i->character) >= 150000) {
            send_to_char(
                ch,
                "@D<@GPowerlevel Detected@D:@w ?????????@D> @w---> @C%s@n\r\n",
                same == TRUE ? pathway : blah);
          } else if (OBJ_FLAGGED(obj, ITEM_MSCOUTER) &&
                     GET_HIT(i->character) >= 5000000) {
            send_to_char(
                ch,
                "@D<@GPowerlevel Detected@D:@w ?????????@D> @w---> @C%s@n\r\n",
                same == TRUE ? pathway : blah);
          } else if (OBJ_FLAGGED(obj, ITEM_ASCOUTER) &&
                     GET_HIT(i->character) >= 15000000) {
            send_to_char(
                ch,
                "@D<@GPowerlevel Detected@D:@w ?????????@D> @w---> @C%s@n\r\n",
                same == TRUE ? pathway : blah);
            ...
            send_to_char(
                ch, "@D<@GPowerlevel Detected@D: [@Y%s@D]@w ---> @C%s@n\r\n",
                add_commas(GET_HIT(i->character)),
                same == TRUE ? pathway : blah);
          }
          ++count;
          free(blah);
    ...
    if (IS_ANDROID(vict)) {
      ...
      send_to_char(ch, "@D|@1                                  @n@D|@n\r\n");
      send_to_char(ch, "@D|@1@RP@r@1o@Rw@r@1e@1@Rr L@r@1e@Rv@r@1e@1@Rl@1@D:    "
                       "             @RERROR@n@D|@n\r\n");
      send_to_char(ch, "@D|@1@CC@c@1ha@1@Cr@c@1ge@1@Cd Ki @1@D:                "
                       " @RERROR@n@D|@n\r\n");
      send_to_char(ch, "@D|@1@YS@y@1ta@1@Ym@y@1in@1@Ya    @1@D:                "
                       " @RERROR@n@D|@n\r\n");
      send_to_char(ch, "@D|@1                                  @n@D|@n\r\n");
      ...
      if (OBJ_FLAGGED(obj, ITEM_BSCOUTER) && GET_HIT(vict) >= 150000) {
        ...
        int64_t stam = (getCurST(vict)), mstam = GET_MAX_MOVE(vict);
        if (stam <= 0)
          stam = 1;
        if (mstam <= 0)
          mstam = 1;
        ...
        send_to_char(ch, "@D|@1                                  @n@D|@n\r\n");
        send_to_char(ch,
                     "@D|@1@RReading target...                 @n@D|@n\r\n");
        send_to_char(ch, "@D|@1                                  @n@D|@n\r\n");
        send_to_char(ch,
                     "@D|@1@RP@r@1o@Rw@r@1e@1@Rr L@r@1e@Rv@r@1e@1@Rl@1@D: "
                     "@Y%21s@n@D|@n\r\n",
                     add_commas(GET_HIT(vict)));
        if (!IS_NPC(vict)) {
          send_to_char(
              ch, "@D|@1@CC@c@1ha@1@Cr@c@1ge@1@Cd Ki @1@D: @Y%21s@n@D|@n\r\n",
              add_commas(GET_CHARGE(vict)));
        } else if (IS_NPC(vict)) {
          send_to_char(
              ch, "@D|@1@CC@c@1ha@1@Cr@c@1ge@1@Cd Ki @1@D: @Y%21s@n@D|@n\r\n",
              add_commas(vict->mobcharge *
                         rand_number(GET_LEVEL(ch) * 50, GET_LEVEL(ch) * 200)));
        }
        if (percent < 10)
          send_to_char(
              ch, "@D|@1@YS@y@1ta@1@Ym@y@1in@1@Ya    @1@D: @Y%21s@n@D|@n\r\n",
              "Exhausted");
        else if (percent < 25)
          send_to_char(
              ch, "@D|@1@YS@y@1ta@1@Ym@y@1in@1@Ya    @1@D: @Y%21s@n@D|@n\r\n",
...
ACMD(do_steal) {
  ...
  if (!GET_SKILL(ch, SKILL_SLEIGHT_OF_HAND) &&
    ...
    send_to_char(ch, "You learn the very veeeery basics of theft. Which is "
                     "don't get caught.\r\n");
    SET_SKILL(ch, SKILL_SLEIGHT_OF_HAND, 1);
  } else if (!GET_SKILL(ch, SKILL_SLEIGHT_OF_HAND) &&
             slot_count(ch) + 1 > GET_SLOTS(ch)) {
    send_to_char(ch, "You can't learn any more skills and thus can not steal "
                     "right now!\r\n");
    return;
  }
  ...
  if (!*arg) {
    send_to_char(
        ch, "An important basic of theft is actually having a victim!\r\n");
    return;
  }
  ...
  if (!(vict = get_char_vis(ch, arg2, NULL, FIND_CHAR_ROOM))) {
    ...
    send_to_char(
        ch, "You are trapped inside the newbie shield until level 9 and can't "
            "piss off those bigger and better than you. Awww...\r\n");
    return;
  ...
  if ((getCurST(ch)) < (GET_MAX_MOVE(ch) / 40) + IS_CARRYING_W(ch)) {
    send_to_char(ch, "You do not have enough stamina.\r\n");
    return;
  }
  ...
  if (axion_dice(0) > 100 && GET_POS(vict) != POS_SLEEPING) { /* Unlucky! */
    reveal_hiding(ch, 0);
    act("@r$N@R just happens to glance in your direction! What terrible "
        "luck!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@RYou just happen to glance behind you and spot @r$n@R trying to "
        "STEAL from you!@n",
        TRUE, ch, 0, vict, TO_VICT);
    act("@r$N@R just happens to glance in @r$n's@R direction and catches $m "
        "trying to STEAL!@n",
        TRUE, ch, 0, vict, TO_NOTVICT);
    prob = -1000;
  }
  ...
  if (prob + 20 < perc &&
      GET_POS(vict) != POS_SLEEPING) { /* Critical failure! */
    reveal_hiding(ch, 0);
    act("@rYou are caught trying to stick your hand in @R$N's@r possessions!@n",
        TRUE, ch, 0, vict, TO_CHAR);
    act("@rYou catch @R$n@r trying to rummage through your possessions!@n",
        TRUE, ch, 0, vict, TO_VICT);
    act("@R$n@R is caught by @R$N@r as $e sticks $s hand in @R$N's@r "
        "possessions!@n",
        TRUE, ch, 0, vict, TO_NOTVICT);
    WAIT_STATE(ch, PULSE_3SEC);
    if (IS_NPC(vict)) {
    ...
    if (!strcasecmp(arg, "zenni")) { /* MOOLA! */
      if (prob > perc) {
        if (GET_GOLD(vict) > 0) {
          ...
          if (gold + GET_GOLD(ch) > GOLD_CARRY(ch)) {
            send_to_char(
                ch, "You can't hold that much more zenni on your person!\r\n");
            return;
          }
          ...
          if (!IS_NPC(vict)) {
            ...
            mudlog(NRM, MAX(ADMLVL_GRGOD, GET_INVIS_LEV(ch)), TRUE,
                   "THEFT: %s has stolen %s zenni@n from %s", GET_NAME(ch),
                   add_commas(gold), GET_NAME(vict));
          }
          if (gold > 1)
          ...
          if (axion_dice(0) > prob) {
            send_to_char(ch, "You think that your movements might have been a "
                             "bit obvious.\r\n");
            reveal_hiding(ch, 0);
            act("@R$n@r just stole zenni from @R$N@r!@n", TRUE, ch, 0, vict,
                TO_ROOM);
        ...
        reveal_hiding(ch, 0);
        act("@rYou are caught trying to steal zenni from @R$N@r!@n", TRUE, ch,
            0, vict, TO_CHAR);
        act("@rYou catch @R$n's@r hand trying to snatch your zenni!@n", TRUE,
            ch, 0, vict, TO_VICT);
        act("@R$N@r catches @R$n's@r hand trying to snatch $S zenni!@n", TRUE,
            ch, 0, vict, TO_NOTVICT);
        WAIT_STATE(ch, PULSE_3SEC);
        if (IS_NPC(vict)) {
          set_fighting(vict, ch);
      ...
      if (!(obj = get_obj_in_list_vis(ch, arg, NULL, inv_for_char(vict)))) {
        ...
        if (!obj) {
          ...
          send_to_char(ch, "Steal worn equipment from them while they are "
                           "awake? That's a stupid idea...\r\n");
          return;
          ...
          send_to_char(ch, "You can't steal that!\r\n");
          return;
        } else if (GET_OBJ_WEIGHT(obj) + (getCurCarriedWeight(ch)) >
                   CAN_CARRY_W(ch)) {
          ...
          if (axion_dice(0) > prob) {
            send_to_char(ch, "You think that your movements might have been a "
                             "bit obvious.\r\n");
            reveal_hiding(ch, 0);
            act("@R$n@r just stole $p@r from @R$N@r!@n", TRUE, ch, obj, vict,
                TO_ROOM);
          ...
          act("@rYou feel your body being shifted while you sleep and wake up "
              "to find @R$n@r trying to steal $p@r from you!@n",
              TRUE, ch, obj, vict, TO_VICT);
          act("@R$N@r catches @R$n's@r trying to $p@r from $M during $S "
              "sleep!@n",
        ...
        if (!give_otrigger(obj, vict, ch) || !receive_mtrigger(ch, vict, obj)) {
          ...
          if (!IS_NPC(vict)) {
            SET_BIT_AR(PLR_FLAGS(vict), PLR_STOLEN);
            mudlog(NRM, MAX(ADMLVL_GRGOD, GET_INVIS_LEV(ch)), TRUE,
                   "THEFT: %s has stolen %s@n from %s", GET_NAME(ch),
          ...
          act("@rYou are caught trying to steal $p@r from @R$N@r!@n", TRUE, ch,
              obj, vict, TO_CHAR);
          act("@rYou catch @R$n@r trying to steal $p@r from you!@n", TRUE, ch,
              obj, vict, TO_VICT);
          act("@R$N@r catches @R$n's@r trying to $p@r!@n", TRUE, ch, obj, vict,
              TO_NOTVICT);
          WAIT_STATE(ch, PULSE_3SEC);
          if (IS_NPC(vict)) {
            char_position_set(vict, POS_STANDING);
...
static int perform_group(struct char_data *ch, struct char_data *vict,
  ...
  if (GET_BONUS(vict, BONUS_LONER) > 0) {
    act("$n is the loner type and refuses to be in your group.", TRUE, vict, 0,
        ch, TO_VICT);
    return (0);
  }
  ...
  if (GET_LEVEL(vict) + 12 < highlvl) {
   act("$n isn't experienced enough to be in your group with its current
  members.", TRUE, vict, 0, ch, TO_VICT); return (0);
  }

  if (GET_LEVEL(vict) > lowlvl + 12) {
   act("$n is too experienced to be in your group with its current members.",
  TRUE, vict, 0, ch, TO_VICT); return (0);
  }

  if (highlvl >= 100) {
   if ((getMaxPL(vict))) > highpl * 1.5) {
    act("$n is too powerful right now to be in a level 100 group with you.",
  TRUE, vict, 0, ch, TO_VICT); return (0);
   }

   if ((getMaxPL(vict))) < lowpl * 0.5) {
    act("$n is too weak right now to be in a level 100 group with you.", TRUE,
  vict, 0, ch, TO_VICT); return (0);
   }
  }
  */
    ...
    if (char_condition_has(k, "group")) {
      ...
      if (GET_HIT(k) > GET_MAX_HIT(k) / 10) {
        snprintf(buf, sizeof(buf),
                 "@gL@D: @w$N @W- @D[@RPL@Y: @c%s @CKi@Y: @c%s @GST@Y: @c%s@D] "
                 "[@w%2d %s %s@D]@n",
                 add_commas(GET_HIT(k)), add_commas((getCurKI(k))),
                 add_commas((getCurST(k))), GET_LEVEL(k), CLASS_ABBR(k),
                 RACE_ABBR(k));
      }
      if (GET_HIT(k) <= (GET_MAX_HIT(k) - (getCurCarriedWeight(k))) / 10) {
        snprintf(buf, sizeof(buf),
                 "@gL@D: @w$N @W- @D[@RPL@Y: @r%s @CKi@Y: @c%s @GST@Y: @c%s@D] "
                 "[@w%2d %s %s@D]@n",
                 add_commas(GET_HIT(k)), add_commas((getCurKI(k))),
                 add_commas((getCurST(k))), GET_LEVEL(k), CLASS_ABBR(k),
                 RACE_ABBR(k));
...
ACMD(do_group) {
  ...
  if (!(vict = get_char_vis(ch, buf, NULL, FIND_CHAR_ROOM)))
    ...
    if (!char_condition_has(vict, "group")) {
      ...
      if (ch != vict)
        act("$N is no longer a member of your group.", FALSE, ch, 0, vict,
            TO_CHAR);
...
ACMD(do_report) {
  ...
  snprintf(buf, sizeof(buf),
           "$n reports: %" I64T "/%" I64T "H, %" I64T "/%" I64T "M, %" I64T
           "/%" I64T "V\r\n",
...
ACMD(do_split) {
  ...
  if (is_number(buf)) {
    ...
    send_to_char(ch,
                 "How much zenni do you wish to split with your group?\r\n");
    return;
  }
}
    ...
    case SCMD_WIMPY:
      ...
      if ((value_lev = atoi(arg)) != 0) {
        if (value_lev < 0)
          ...
          send_to_char(
              ch,
              "You can't set your wimp level above half your powerlevel.\r\n");
        else {
          send_to_char(
              ch, "Okay, you'll wimp out if you drop below %d powerlevel.\r\n",
              value_lev);
...
ACMD(do_display) {
  ...
  if (!*argument) {
    send_to_char(ch, "Usage: prompt { P | K | T | S | F | H | G | L | C | M | "
                     "all/on | none/off }\r\n");
    return;
  }
  ...
  if (!strcasecmp(argument, "on") || !strcasecmp(argument, "all")) {
    ...
    for (i = 0; i < strlen(argument); i++) {
      ...
      case 'f':
        ...
        break;
      default:
        send_to_char(ch, "Usage: prompt { P | K | T | S | F | H | G | L | "
                         "all/on | none/off }\r\n");
        return;
      }
    }
...
ACMD(do_gen_tog) {
  ...
  const char *tog_messages[][2] = {
      ...
      {"", ""},
      {"You are no longer hidden from view on the who list and public "
       "channels.\r\n",
  ...
  case SCMD_HIDE:
    if ((GET_CHARGE(ch) > 0 && GET_PREFERENCE(ch) != PREFERENCE_KI) ||
        ...
        char_condition_has(ch, "powering_up") || char_condition_has(ch, "flying")) {
    ...
    } else if (!GET_SKILL(ch, SKILL_HIDE) &&
               slot_count(ch) + 1 > GET_SLOTS(ch)) {
      send_to_char(
          ch,
          "@RYou need more skill slots in order to learn this skill.@n\r\n");
      return;
    }
...
ACMD(do_compare) {
  ...
  if (obj1 == obj2) {
    ...
    case ITEM_ARMOR:
      ...
      break;
    case ITEM_WEAPON:
      value1 = (1 + GET_OBJ_VAL(obj1, VAL_WEAPON_DAMSIZE)) *
               GET_OBJ_VAL(obj1, VAL_WEAPON_DAMDICE);
      value2 = (1 + GET_OBJ_VAL(obj2, VAL_WEAPON_DAMSIZE)) *
               GET_OBJ_VAL(obj2, VAL_WEAPON_DAMDICE);
      break;
    }
  }
...
ACMD(do_break) {
  ...
  if (!(cmbrk = generic_find(arg, FIND_OBJ_INV | FIND_OBJ_EQUIP, ch, &dummy,
                             &obj))) {
...
ACMD(do_fix) {
  ...
  if (!strcasecmp("self", arg)) {
    if (!IS_ANDROID(ch)) {
      send_to_char(ch,
                   "Only androids can fix their bodies with repair kits.\r\n");
      return;
    } else {
      self = TRUE;
    }
  ...
  if (self == FALSE) {
    if (!(cmbrk =
              generic_find(arg, FIND_OBJ_INV | FIND_OBJ_EQUIP | FIND_OBJ_ROOM,
                           ch, &dummy, &obj))) {
  ...
  if (self == FALSE) {
    ...
    if (obj->carried_by == NULL && !PLR_FLAGGED(ch, PLR_REPLEARN) &&
        (level_exp(ch, GET_LEVEL(ch) + 1) - GET_EXP(ch) > 0 ||
         GET_LEVEL(ch) >= 100)) {
      int64_t gain = (level_exp(ch, GET_LEVEL(ch) + 1) * 0.0003) *
                     GET_SKILL(ch, SKILL_REPAIR);
      send_to_char(ch,
                   "@mYou've learned a bit from repairing it. @D[@gEXP@W: "
                   "@G+%s@D]@n\r\n",
                   add_commas(gain));
    ...
    if (GET_HIT(ch) >= getMaxPL(ch)) {
      ...
      act("You try to repair your body but screw up..", TRUE, ch, 0, 0,
          TO_CHAR);
      act("$n tries to repair $s body but screws up..", TRUE, ch, 0, 0,
          TO_ROOM);
      extract_obj(obj4);
      improve_skill(ch, SKILL_REPAIR, 1);
      WAIT_STATE(ch, PULSE_5SEC);
      ...
      act("You use the repair kit to fix part of your body...", TRUE, ch, 0, 0,
          TO_CHAR);
...
*/
int is_innate_ready(struct char_data *ch, int spellnum) { return TRUE; }
...
static int spell_in_book(struct obj_data *obj, int spellnum) { return 0; }
...
ACMD(do_clan) {
    ...
    if (!(strcmp(arg1, "create"))) {
      if (!*arg2)
        ...
        mudlog(BRF, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), TRUE,
               "(GC) %s has created a clan named %s.", GET_NAME(ch), arg2);
      }
      return;
  ...
  if (!(strcmp(arg1, "join"))) {
    if (!*arg2)
      ...
      send_to_char(ch, "You are already a member of %s.\r\n", arg2);
    else if (GET_CLAN(ch) != NULL && checkCLAN(ch) == 1 &&
             !strstr(GET_CLAN(ch), "Applying"))
      send_to_char(
          ch, "You are already a member of %s, you need to leave it first.\r\n",
          GET_CLAN(ch));
    else if (clanOpenJoin(arg2)) {
    ...
    if (GET_CLAN(ch) == NULL || !(strcmp(GET_CLAN(ch), "None."))) {
      ...
      if (clanIsModerator(GET_CLAN(ch), ch) == FALSE)
        send_to_char(
            ch, "You must be a moderator to edit the clan's information.\r\n");
      else {
    ...
    if (GET_CLAN(ch) == NULL || !(strcmp(GET_CLAN(ch), "None."))) {
      ...
      if (!clanIsMember(GET_CLAN(ch), ch) &&
          !clanIsModerator(GET_CLAN(ch), ch)) {
        send_to_char(ch, "You are not in a clan.\r\n");
        return;
      } else if (!(char_room_get(ch) &&
                   room_flagged(char_room_get(ch), ROOM_CBANK)) &&
                 clanBANY(GET_CLAN(ch), ch) == FALSE) {
        send_to_char(ch, "You are not in your clan bank and your clan doesn't "
                         "have bank anywhere.\r\n");
        return;
      } else if (!*arg2) {
    ...
    if (GET_CLAN(ch) == NULL || !(strcmp(GET_CLAN(ch), "None."))) {
      ...
      if (!clanIsModerator(GET_CLAN(ch), ch)) {
        ...
        send_to_char(ch,
                     "The name length can't be longer than 20 characters.\r\n");
        return;
      } else if (strstr(arg2, "@")) {
    ...
    if (GET_CLAN(ch) == NULL || !(strcmp(GET_CLAN(ch), "None."))) {
      ...
      if (!clanIsModerator(GET_CLAN(ch), ch)) {
        ...
        send_to_char(
            ch,
            "That person is no where to be found in the entire universe.\r\n");
        return;
      } else if (GET_CLAN(vict) == NULL || !(strcmp(GET_CLAN(vict), "None."))) {
        send_to_char(
            ch, "That person is not even in a clan, let alone your's.\r\n");
        return;
      } else if (!clanIsMember(GET_CLAN(ch), vict)) {
        send_to_char(ch, "You can only rank those in your clan and only if "
                         "below leader.\r\n");
        return;
      } else if (clanIsModerator(GET_CLAN(ch), vict)) {
        send_to_char(
            ch,
            "You can't rank a fellow leader, you require imm assistance.\r\n");
        return;
      } else if (!*arg3) {
        send_to_char(ch, "What rank do you want to set them to?\r\n[ 0 = "
                         "Member, 1 = Midrank, 2 = Highrank]\r\n");
        return;
      }
      int num = atoi(arg3);
      if (num < 0 || num > 2) {
    ...
    if (GET_CLAN(ch) == NULL || !(strcmp(GET_CLAN(ch), "None."))) {
      ...
      if (!clanIsMember(GET_CLAN(ch), ch) &&
        ...
        return;
      } else if (!(clanIsModerator(GET_CLAN(ch), ch) ||
                   GET_ADMLEVEL(ch) >= ADMLVL_IMPL)) {
        send_to_char(
            ch,
            "You do not have the power to withdraw from the clan bank.\r\n");
        return;
      } else if (atoi(arg2) <= 0) {
        send_to_char(ch, "It needs to be a value higher than 0...\r\n");
        return;
    ...
    if (GET_CLAN(ch) == NULL || !(strcmp(GET_CLAN(ch), "None."))) {
      ...
      send_to_char(
          ch, "@W[ @C%-20s @W]@w has @D(@Y%s@D)@w zenni in its clan bank.\r\n",
    ...
    if (!*arg3 || !*name1) {
      ...
      send_to_char(ch, "%s is not a valid clan.\r\n", arg3);
    } else if (!(clanIsModerator(arg3, ch) ||
                 GET_ADMLEVEL(ch) >= ADMLVL_IMPL)) {
      ...
      if ((player_i = load_char(name, vict)) > -1) {
        ...
        if (!clanIsMember(arg3, vict)) {
          send_to_char(ch, "%s isn't even a member of %s.\r\n", GET_NAME(vict),
                       arg3);
        } else if (clanIsModerator(arg3, vict) &&
                   GET_ADMLEVEL(ch) < ADMLVL_IMPL) {
          send_to_char(
              ch, "You do not have the power to kick a leader out of %s.\r\n",
              arg3);
        } else {
      ...
      send_to_char(ch,
                   "You do not have the power to kick a leader out of %s.\r\n",
                   arg3);
    } else {
      send_to_char(ch, "You expel %s from %s.\r\n", GET_NAME(vict), arg3);
    ...
    if (!*arg3 || !*name)
      ...
      send_to_char(ch, "%s is not a valid clan.\r\n", arg3);
    else if ((clanIsModerator(arg3, ch) == FALSE &&
              clanIsMember(arg3, ch) == FALSE &&
              GET_ADMLEVEL(ch) < ADMLVL_IMPL) ||
             (GET_CRANK(ch) < 2 && GET_ADMLEVEL(ch) < ADMLVL_IMPL))
      send_to_char(ch, "Only leaders or highrank can decline people from "
                       "entering a clan.\r\n");
    else if (!(vict = get_char_vis(ch, name, NULL, FIND_CHAR_WORLD)))
    ...
    if (!*arg3 || !*name)
      ...
      send_to_char(
          ch,
          "Only leaders or captains can enroll people into their clan.\r\n");
    else if (!(vict = get_char_vis(ch, name, NULL, FIND_CHAR_WORLD)))
      send_to_char(ch, "%s is not around at the moment.\r\n", name);
    else if (!clanIsApplicant(arg3, vict))
      send_to_char(ch, "%s isn't applying to join %s.\r\n", GET_NAME(vict),
      ...
      send_to_char(ch,
                   "%s needs to have at least earned 5 RPP at some point to "
                   "join a clan.\r\n",
                   GET_NAME(vict));
    else {
    ...
    if (!*arg3 || !*name)
      ...
      send_to_char(
          ch, "Only leaders can make other people in a clan a leader.\r\n");
    else if (!(vict = get_char_vis(ch, name, NULL, FIND_CHAR_WORLD)))
      send_to_char(ch, "%s is not around at the moment.\r\n", name);
    else {
    ...
    if (!*name || !*setting)
      ...
      send_to_char(ch,
                   "Leave access may only be set to free or restricted.\r\n");
    return;
    ...
    if (!*name || !*setting)
      ...
      send_to_char(ch,
                   "Leave access my only be set to free or restricted.\r\n");
    return;
...
ACMD(do_aid) {
  ...
  if (found == FALSE || aid_obj == NULL) {
    if (num == 47) {
      ...
      send_to_char(ch,
                   "You need a TCX-Medical Equipment Construction Kit.\r\n");
    }
    return;
  }
  if (num == 47) {
    ...
    if (!AFF_FLAGGED(vict, AFF_SPIRIT) && !PLR_FLAGGED(vict, PLR_BANDAGED)) {
      ...
      if ((GET_SKILL(ch, SKILL_FIRST_AID) + 1) > dc) {
        ...
        int64_t roll =
            (getMaxPL(vict) / 100) * (GET_WIS(ch) / 4) + getMaxPL(vict) * 0.25;
        if (GET_BONUS(ch, BONUS_HEALER) > 0) {
          roll += roll * .1;
        }
        incCurHealth(vict, roll);
        ...
        if (vict != ch) {
          send_to_char(ch, "You fail to bandage their wounds properly, wasting "
                           "the set of bandages...\r\n");
          act("$N fails to bandage $n's wounds properly, wasting an entire set "
              "of bandages...",
              TRUE, vict, 0, ch, TO_NOTVICT);
          act("$N fails to bandage your wounds properly, wasting an entire set "
              "of bandages...",
              TRUE, vict, 0, ch, TO_CHAR);
        } else {
          act("$N fails to bandage $s wounds properly, wasting an entire set "
              "of bandages...",
              TRUE, vict, 0, ch, TO_NOTVICT);
          act("You fail to bandage your wounds properly, wasting an entire set "
              "of bandages...",
    ...
    if (GET_SKILL(ch, SKILL_FIRST_AID) < 65) {
      send_to_char(ch,
                   "You need at least a skill level of 65 in First Aid.\r\n");
      return;
    } else {
      if (GET_SKILL(ch, SKILL_FIRST_AID) < axion_dice(15)) {
        act("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as "
            "its lid opens. As you begin to construct an Andrenex Adreneline "
            "Injector you screw up and end up breaking the water tight seal. "
            "The adreneline leaks out and is wasted.@n",
            TRUE, ch, 0, 0, TO_CHAR);
        act("@C$n@W holds a steel case up and opens it. The case hisses as its "
            "lid opens. @C$n@W wastes no time as $e reaches into the case and "
            "begins constructing something. A frown forms on $s face as it "
            "appears that $e has failed.@n",
            TRUE, ch, 0, 0, TO_ROOM);
        extract_obj(aid_obj);
      } else {
        act("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as "
            "its lid opens. Your knowledge in basic medical devices and "
            "treatments helps you as you successfully construct an Adrenex "
            "Adreneline Injector@n",
            TRUE, ch, 0, 0, TO_CHAR);
        act("@C$n@W holds a steel case up and opens it. The case hisses as its "
            "lid opens. @C$n@W wastes no time as $e reaches into the case and "
            "begins constructing something. A moment later $e holds up a "
    ...
    if (GET_SKILL(ch, SKILL_FIRST_AID) < 50) {
      send_to_char(ch,
                   "You need at least a skill level of 50 in First Aid.\r\n");
      return;
    } else {
      if (GET_SKILL(ch, SKILL_FIRST_AID) < axion_dice(10)) {
        act("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as "
            "its lid opens. As you go to put the salve ingredients into the "
            "kit's salve compartment and set the temperature you accidentally "
            "set it too high. The salve is burned and ruined. Yes you managed "
            "to burn a burn salve.@n",
            TRUE, ch, 0, 0, TO_CHAR);
        act("@C$n@W holds a steel case up and opens it. The case hisses as its "
            "lid opens. @C$n@W wastes no time as $e reaches into the case and "
            "begins constructing something. A frown forms on $s face as it "
        ...
        act("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as "
            "its lid opens. Your knowledge in basic medical devices and "
            "treatments helps you as you successfully boil a burn salve to "
            "perfection and it is automatically placed in a jar.@n",
            TRUE, ch, 0, 0, TO_CHAR);
        act("@C$n@W holds a steel case up and opens it. The case hisses as its "
            "lid opens. @C$n@W wastes no time as $e reaches into the case and "
            "begins constructing something. A moment later $e holds up a jar "
            "of burn salve!@n",
            TRUE, ch, 0, 0, TO_ROOM);
        aid_prod = read_object(num2, VIRTUAL);
        obj_to_char(aid_prod, ch);
    ...
    if (GET_SKILL(ch, SKILL_FIRST_AID) < 40) {
      send_to_char(ch,
                   "You need at least a skill level of 40 in First Aid.\r\n");
      return;
    } else {
      if (GET_SKILL(ch, SKILL_FIRST_AID) < axion_dice(5)) {
        act("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as "
            "its lid opens. As you complete the Antitoxin Injector you notice "
            "that you didn't seal the syringe properly and it all leaks out.@n",
            TRUE, ch, 0, 0, TO_CHAR);
        act("@C$n@W holds a steel case up and opens it. The case hisses as its "
            "lid opens. @C$n@W wastes no time as $e reaches into the case and "
            "begins constructing something. A frown forms on $s face as it "
        ...
        act("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as "
            "its lid opens. Your knowledge in basic medical devices and "
            "treatments helps you as you successfully assemble the Antitoxin "
            "Injector.@n",
            TRUE, ch, 0, 0, TO_CHAR);
        act("@C$n@W holds a steel case up and opens it. The case hisses as its "
            "lid opens. @C$n@W wastes no time as $e reaches into the case and "
    ...
    if (GET_SKILL(ch, SKILL_FIRST_AID) < 40) {
      ...
      if (GET_SKILL(ch, SKILL_FIRST_AID) < axion_dice(15)) {
        act("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as "
            "its lid opens. As you complete a vial of Formula 82 you notice "
            "that you read the mixture measurements wrong. You dispose of the "
            "vile vial immediately.@n",
            TRUE, ch, 0, 0, TO_CHAR);
        act("@C$n@W holds a steel case up and opens it. The case hisses as its "
            "lid opens. @C$n@W wastes no time as $e reaches into the case and "
            "begins constructing something. A frown forms on $s face as it "
        ...
        act("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as "
            "its lid opens. Your knowledge in basic medical devices and "
            "treatments helps you as you successfully assemble a vial of "
            "Formula 82.@n",
            TRUE, ch, 0, 0, TO_CHAR);
        act("@C$n@W holds a steel case up and opens it. The case hisses as its "
            "lid opens. @C$n@W wastes no time as $e reaches into the case and "
...
ACMD(do_aura) {
  ...
  if (!strcasecmp(arg, "light")) {
    if (GET_SKILL(ch, SKILL_FOCUS) < 75 ||
        GET_SKILL(ch, SKILL_CONCENTRATION) < 75) {
      send_to_char(ch, "You need at least a skill level of 75 in Focus and "
                       "Concentration to use this.\r\n");
      return;
    } else {
      if (PLR_FLAGGED(ch, PLR_AURALIGHT)) {
        ...
        send_to_char(ch,
                     ...
                     "provide light to the surrounding area!\r\n",
                     aura_types[GET_AURA(ch)]);
        char bloom[MAX_INPUT_LENGTH];
        sprintf(bloom,
                "@wA %s aura flashes up brightly around $n@w as they provide "
                "light to the area.@n",
                aura_types[GET_AURA(ch)]);
