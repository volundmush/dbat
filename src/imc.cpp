...
char *imc_strip_colors(const char *txt) {
    ...
    for (color = first_imc_color; color; color = color->next)
        strlcpy(tbuf, imcstrrep(tbuf, color->imctag, ""), LGST);

    for (color = first_imc_color; color; color = color->next)
        strlcpy(tbuf, imcstrrep(tbuf, color->mudtag, ""), LGST);
    return tbuf;
}
...
const char *color_itom(const char *txt, CHAR_DATA *ch) {
    ...
    if (IMCIS_SET(IMCFLAG(ch), IMC_COLORFLAG)) {
        strlcpy(tbuf, txt, LGST);
        for (color = first_imc_color; color; color = color->next)
            strlcpy(tbuf, imcstrrep(tbuf, color->imctag, color->mudtag), LGST);
    } else
        strlcpy(tbuf, imc_strip_colors(txt), LGST);
...
const char *color_mtoi(const char *txt) {
    ...
    strlcpy(tbuf, txt, LGST);
    for (color = first_imc_color; color; color = color->next)
        strlcpy(tbuf, imcstrrep(tbuf, color->mudtag, color->imctag), LGST);
...
char *imc_mudof(const char *src) {
    ...
    if (!(person = strchr(src, '@')))
        strlcpy(mud, src, SMST);
    else
        strlcpy(mud, person + 1, SMST);
    return mud;
}
...
char *imcgetname(const char *from) {
    ...
    if (!strcasecmp(mud, this_imcmud->localname))
        strlcpy(buf, imc_nameof(name), SMST);
    else
        strlcpy(buf, from, SMST);
...
void imc_write_buffer(const char *txt) {
    ...
    * Copy.
    */
    strncpy(this_imcmud->outbuf + this_imcmud->outtop, output,
            length);   /* Leave this one alone! BAD THINGS(TM) will happen if you don't! */
...
void imc_write_packet(IMC_PACKET *p) {
    ...
    */
    snprintf(txt, IMC_BUFF_SIZE, "%s %lu %s %s %s", p->from, ++imc_sequencenumber, this_imcmud->localname, p->type,
             p->to);
...
PFUN(imc_recv_chanwho) {
    ...
    if (!c->local_name)
        ...
        strlcpy(cwho, get_local_chanwho(c), IMC_BUFF_SIZE);

        if ((!strcasecmp(cwho, "") || !strcasecmp(cwho, "Nobody")) && !strcasecmp(q->to, "*"))
            return;
        strlcpy(buf, cwho, IMC_BUFF_SIZE);
    }
...
char *imccenterline(const char *string, int length) {
    ...
    strlcpy(stripped, imc_strip_colors(string), 300);
...
char *multiline_center(char *splitme) {
    ...
    while (1) {
        ...
        if (strstr(arg, "<center>")) {
            strlcpy(arg, imcstrrep(arg, "<center>", ""), SMST);
            strlcpy(arg, imccenterline(arg, 78), SMST);
        }
...
char *process_who_head(int plrcount) {
    ...
    strlcpy(head, whot->head, LGST);
    snprintf(pcount, SMST, "%d", plrcount);

    strlcpy(head, imcstrrep(head, "<%plrcount%>", pcount), LGST);
    strlcpy(head, multiline_center(head), LGST);
...
char *process_who_tail(int plrcount) {
    ...
    strlcpy(tail, whot->tail, LGST);
    snprintf(pcount, SMST, "%d", plrcount);

    strlcpy(tail, imcstrrep(tail, "<%plrcount%>", pcount), LGST);
    strlcpy(tail, multiline_center(tail), LGST);
...
char *process_plrline(char *plrrank, char *plrflags, char *plrname, char *plrtitle) {
    ...
    strlcpy(pline, whot->immline, LGST);
    strlcpy(pline, imcstrrep(pline, "<%charrank%>", plrrank), LGST);
    strlcpy(pline, imcstrrep(pline, "<%charflags%>", plrflags), LGST);
    strlcpy(pline, imcstrrep(pline, "<%charname%>", plrname), LGST);
    strlcpy(pline, imcstrrep(pline, "<%chartitle%>", plrtitle), LGST);
    imcstrlcat(pline, "\n", LGST);
...
char *process_who_template(char *head, char *tail, char *plrlines, char *immlines, char *plrheader, char *immheader) {
    ...
    strlcpy(master, whot->master, LGST);
    strlcpy(master, imcstrrep(master, "<%head%>", head), LGST);
    strlcpy(master, imcstrrep(master, "<%tail%>", tail), LGST);
    strlcpy(master, imcstrrep(master, "<%plrheader%>", plrheader), LGST);
    strlcpy(master, imcstrrep(master, "<%immheader%>", immheader), LGST);
    strlcpy(master, imcstrrep(master, "<%plrline%>", plrlines), LGST);
    strlcpy(master, imcstrrep(master, "<%immline%>", immlines), LGST);
...
char *imc_assemble_who(void) {
    ...
    for (d = first_descriptor; d; d = d->next) {
        ...
        if (person && d->connected == CON_PLAYING) {
            ...
            if (!plr) {
                strlcpy(plrheader, whot->plrheader, SMST);
                plr = true;
            }

            strlcpy(rank, imcrankbuffer(person), SMST);

            if (IMCAFK(person))
                strlcpy(flags, "AFK", SMST);
            else
                strlcpy(flags, "---", SMST);

            strlcpy(name, CH_IMCNAME(person), SMST);
            strlcpy(title, color_mtoi(CH_IMCTITLE(person)), SMST);
            strlcpy(plrline, process_plrline(rank, flags, name, title), SMST);
    ...
    for (d = first_descriptor; d; d = d->next) {
        ...
        if (person && d->connected == CON_PLAYING) {
            ...
            if (!imm) {
                strlcpy(immheader, whot->immheader, SMST);
                imm = true;
            }
            ...
            strlcpy(name, CH_IMCNAME(person), SMST);
            strlcpy(title, color_mtoi(CH_IMCTITLE(person)), SMST);
            strlcpy(immline, process_immline(rank, flags, name, title), SMST);
            imcstrlcat(immlines, immline, LGST);
        }
    }

    strlcpy(head, process_who_head(pcount), LGST);
    strlcpy(tail, process_who_tail(pcount), LGST);
    strlcpy(master, process_who_template(head, tail, plrlines, immlines, plrheader, immheader), LGST);
...
void imc_process_who(char *from) {
    ...
    strlcpy(whoreply, imc_assemble_who(), IMC_BUFF_SIZE);
...
void imc_loop(void) {
    ...
    */
    ...
    if (this_imcmud->desc > 0 && this_imcmud->outtop > 0 && FD_ISSET(this_imcmud->desc, &out_set) &&
        !imc_write_socket()) {
        this_imcmud->outtop = 0;
        imc_shutdown(true);
    }
...
void imc_readbans(void) {
    ...
    while (!feof(inf) && !ferror(inf)) {
        strlcpy(temp, imcfread_word(inf), SMST);
...
char *parse_who_header(char *head) {
    ...
    strlcpy(newhead, head, LGST);
    strlcpy(newhead, imcstrrep(newhead, "<%mudfullname%>", this_imcmud->fullname), LGST);
    strlcpy(newhead, imcstrrep(newhead, "<%mudtelnet%>", this_imcmud->ihost), LGST);
    strlcpy(newhead, imcstrrep(newhead, "<%mudport%>", iport), LGST);
    strlcpy(newhead, imcstrrep(newhead, "<%mudurl%>", this_imcmud->www), LGST);
...
char *parse_who_tail(char *tail) {
    ...
    snprintf(iport, SMST, "%d", this_imcmud->iport);
    strlcpy(newtail, tail, LGST);
    strlcpy(newtail, imcstrrep(newtail, "<%mudfullname%>", this_imcmud->fullname), LGST);
    strlcpy(newtail, imcstrrep(newtail, "<%mudtelnet%>", this_imcmud->ihost), LGST);
    strlcpy(newtail, imcstrrep(newtail, "<%mudport%>", iport), LGST);
    strlcpy(newtail, imcstrrep(newtail, "<%mudurl%>", this_imcmud->www), LGST);
...
IMC_CMD(imcwho) {
    ...
    */
    if (!strcasecmp(argument, this_imcmud->localname) && IMCPERM(ch) >= IMCPERM_IMM) {
        ...
        /*imc_to_char( "Don't who your own mud.\r\n", ch);*/
        return;
    }
    ...
    */
    ...
    for (p = first_rinfo; p; p = p->next, count++) {
        if (!strcasecmp(p->network, "unknown"))
            strlcpy(netname, this_imcmud->network, SMST);
        else
            strlcpy(netname, p->network, SMST);
        /*
       ...
       */
        if (p->path && p->path[0] != '\0') {
            if ((start = strchr(p->path, '!')) != nullptr) {
                ...
                strlcpy(serverpath, p->path, LGST);
        }
        imcpager_printf(ch, "\r\n~%c%-15.15s ~B%-40.40s ~g%-15.15s ~G%s",
...
IMC_CMD(imcignore) {
    ...
    imc_to_char("IMCignore is disabled at this time.\r\n", ch);
    return;
...
IMC_CMD(imchelp) {
    ...
    if (!argument || argument[0] == '\0') {
        strlcpy(buf, "~gHelp is available for the following commands:\r\n", LGST);
        imcstrlcat(buf, "~G---------------------------------------------\r\n", LGST);
...
IMC_CMD(imc_other) {
    ...
    strlcpy(buf, "~gThe following commands are available:\r\n", LGST);
    imcstrlcat(buf, "~G-------------------------------------\r\n\r\n", LGST);
    for (perm = IMCPERM_MORT; perm <= IMCPERM(ch); perm++) {
        col = 0;
        snprintf(buf + strlen(buf), LGST - strlen(buf), "\r\n~g%s commands:~G\r\n", imcperm_names[perm]);
...
const char *imc_send_social(CHAR_DATA *ch, const char *argument, int telloption) {
    ...
    */
    ...
    snprintf(msg, LGST, "%s", (char *) imc_act_string(color_mtoi(socbuf), ch, skeleton));
    if (skeleton)
...
bool imc_command_hook(CHAR_DATA *ch, const char *command, char *argument) {
    ...
    if (IMCPERM(ch) <= IMCPERM_NONE)
        return false;
    /*
    * tbaMUD parser leaves leading spaces after splitting one argument
    */
    ...
    */
    ...
    if (!imc_hasname(IMC_LISTEN(ch), c->local_name)) {
        imc_printf(ch,
                   "You are not currently listening to %s. Use the imclisten command to listen to this channel.\r\n",
                   c->local_name);
        return true;
    }
