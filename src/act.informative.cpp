...
ACMD(do_mimic) {
  ...
  if (!GET_SKILL(ch, SKILL_MIMIC)) {
    send_to_char(
        ch, "You do not know how to mimic the appearance of other races.\r\n");
    return;
  }
  ...
  if (!strcasecmp(arg, "stop")) {
    ...
    act("@mYou concentrate for a moment and release the illusion that was "
        "mimicing another race.@n",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@M$n@m concentrates for a moment and SUDDENLY $s appearance changes "
        "some what!@n",
        TRUE, ch, 0, 0, TO_ROOM);
    ch->mimic = 0;
  }
  ...
  if (!race) {
    send_to_char(ch, "That is not a race you can change into. Enter mimic "
                     "without arugments for the mimic menu.\r\n");
    return;
  }
  ...
  if (race->getID() == ch->mimic) {
    send_to_char(
        ch,
        "You are already mimicing that race. To stop enter 'mimic stop'\r\n");
    return;
    ...
    decCurKI(ch, cost);
    act("@mYou concentrate and attempt to create an illusion to obscure your "
        "racial features. However you frown as you realize you have failed.@n",
        TRUE, ch, 0, 0, TO_CHAR);
    act("@M$n@m concentrates and the light around them seems to shift and "
        "blur. It stops a moment later and $e frowns.@n",
        TRUE, ch, 0, 0, TO_ROOM);
    return;
  } else {
    ...
    decCurKI(ch, cost);
    sprintf(buf,
            "@M$n@m concentrates for a moment and $s features start to blur as "
            "light bends around $m. Now $e appears to be %s @M%s!@n",
            AN(RACE(ch)), LRACE(ch));
    send_to_char(ch,
                 "@mYou concentrate for a moment and your features start to "
                 "blur as you use your ki to bend the light around your body. "
                 "You now appear to be %s %s.@n\r\n",
                 AN(RACE(ch)), LRACE(ch));
