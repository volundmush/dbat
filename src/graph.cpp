...
ACMD(do_sradar) {
  ...
  if (noship == FALSE &&
      room_sector_type_get(obj_room_get(vehicle)) != SECT_SPACE) {
  ...
  if (!*arg) {
    ...
    /*
     send_to_char(ch, "     @D----------------[@C Radar
     @D]-----------------@n\r\n"); send_to_char(ch, "    @D|@GEE@D:@w Earth
     @gNN@D:@w Namek   @YVV@D:@w Vegeta       @D|@n\r\n"); send_to_char(ch, "
     @D|@CFF@D:@w Frigid   @mKK@D:@w Konack  @BAA@D:@w Aether       @D|@n\r\n");
     send_to_char(ch, "    @D|@MYY@D:@w Yardrat  @CKK@D:@w Kanassa @mAA@D:@w
     Arlia        @D|@n\r\n"); send_to_char(ch, "    @D|@WBB@D:@w Buoy @m&&@D:@w
     Nebula  @yQQ@D:@w Asteroid     @D|@n\r\n"); send_to_char(ch, "
     @D|@b@1**@n@D:@w Wormhole @DSS@D:@w Station  @r#@D: @wUnknown Ship
...
ACMD(do_radar) {
  ...
  if (IS_NPC(ch)) {
    ...
    act("$n holds up a dragon radar and pushes its button.", FALSE, ch, 0, 0,
        TO_ROOM);
    while (num < 20000) {
      if (room_by_id(room)) {
        room_contents_iterate(room_by_id(room), [&](auto obj) {
          if (OBJ_FLAGGED(obj, ITEM_FORGED)) {
            ...
            case BFS_ALREADY_THERE:
              send_to_char(
                  ch,
                  "@D<@G%d@D>@w The radar detects a dragonball right here!\r\n",
                  fcount);
              break;
            case BFS_NO_PATH:
              send_to_char(ch,
                           "@D<@G%d@D>@w The radar detects a faint dragonball "
                           "signal, but can not direct you further.\r\n",
                           fcount);
              break;
            default:
              send_to_char(
                  ch,
                  "@D<@G%d@D>@w The radar detects a dragonball %s of here.\r\n",
                  fcount, dirs[dir]);
              break;
            }
            found = TRUE;
        ...
        room_people_iterate(room_by_id(room), [&](auto tch) {
          ...
          char_inventory_iterate(tch, [&](auto obj) {
            if (OBJ_FLAGGED(obj, ITEM_FORGED)) {
              ...
              case BFS_ALREADY_THERE:
                send_to_char(ch,
                             "@D<@G%d@D>@w The radar detects a dragonball "
                             "right here!\r\n",
                             fcount);
                break;
              case BFS_NO_PATH:
                send_to_char(
                    ch,
                    "@D<@G%d@D>@w The radar detects a faint dragonball signal, "
                    "but can not direct you further.\r\n",
                    fcount);
                break;
              default:
                send_to_char(ch,
                             "@D<@G%d@D>@w The radar detects a dragonball %s "
                             "of here.\r\n",
                             fcount, dirs[dir]);
                break;
              }
    ...
    if (found == FALSE) {
      send_to_char(
          ch, "The radar didn't detect any dragonballs on the planet.\r\n");
      return;
    }
  }
}
...
ACMD(do_track) {
  ...
  if (GET_SUPPRESS(ch) <= 20 && GET_SUPPRESS(ch) > 0) {
    send_to_char(ch, "You are concentrating too hard on suppressing your "
                     "powerlevel at this level of suppression.\r\n");
    return;
  }
  ...
  if (!*arg && !FIGHTING(ch)) {
    ...
    act("$n looks at $N@n intently for a moment.", TRUE, ch, 0, vict,
        TO_NOTVICT);
    if (!IS_ANDROID(vict)) {
      if (GET_ALIGNMENT(vict) > 50 && GET_ALIGNMENT(vict) < 200) {
        ...
        send_to_char(ch,
                     "You sense an extremely pure and good ki from them.\r\n");
        ...
        send_to_char(ch,
                     "You sense slightly mild indefinable ki from them.\r\n");
      }
      if (GET_HIT(vict) > GET_HIT(ch) * 50) {
        send_to_char(ch, "Their power is so huge it boggles your mind and "
                         "crushes your spirit to fight!\n");
      } else if (GET_HIT(vict) > GET_HIT(ch) * 25) {
        send_to_char(ch, "Their power is so much larger than you that you "
                         "would die like an insect.\n");
        ...
        send_to_char(ch,
                     "Their power is more than twice as large as your own.\n");
        ...
        send_to_char(ch,
                     "Their power is about a quarter of your own or larger.\n");
        ...
        send_to_char(ch,
                     "Their power is about a tenth of your own or larger.\n");
      } else if (GET_HIT(vict) >= GET_HIT(ch) * 0.01) {
        send_to_char(ch, "Their power is less than a tenth of your own.\n");
      } else if (GET_HIT(vict) < GET_HIT(ch) * 0.01) {
        send_to_char(ch, "Their power is less than 1 percent of your own. What "
                         "a weakling...\n");
      }
    } else {
      send_to_char(
          ch, "You can't sense their powerlevel as they are a machine.\r\n");
    }
    return;
  }
    ...
    for (i = descriptor_list; i; i = i->next) {
      if (STATE(i) != CON_PLAYING) {
        ...
        if (readIntro(ch, i->character) == 1) {
          send_to_char(ch, "@D[@Y%d@D] @CYou sense @c%s@C with ", (count + 1),
                       get_i_name(ch, i->character));
        } else {
          send_to_char(ch, "@D[@Y%d@D] @CYou sense ", (count + 1));
        }
        /* How strong is the one we sense? */
        if (GET_HIT(i->character) > GET_HIT(ch) * 50) {
          send_to_char(ch, "a power so huge it boggles your mind and crushes "
                           "your spirit to fight!\n");
        } else if (GET_HIT(i->character) > GET_HIT(ch) * 25) {
          send_to_char(ch, "a power so much larger than you that you would die "
                           "like an insect.\n");
        } else if (GET_HIT(i->character) > GET_HIT(ch) * 10) {
          send_to_char(ch,
                       "a power that is many times larger than your own.\n");
        } else if (GET_HIT(i->character) > GET_HIT(ch) * 5) {
          send_to_char(ch,
                       "a power that is a great deal larger than your own.\n");
        } else if (GET_HIT(i->character) > GET_HIT(ch) * 2) {
          send_to_char(
              ch, "a power that is more than twice as large as your own.\n");
        } else if (GET_HIT(i->character) > GET_HIT(ch)) {
          send_to_char(ch,
                       "a power that is about twice as large as your own.\n");
        } else if (GET_HIT(i->character) == GET_HIT(ch)) {
          send_to_char(ch, "a power that is exactly as strong as you.\n");
        } else if (GET_HIT(i->character) >= GET_HIT(ch) * 0.75) {
          send_to_char(
              ch, "a power that is about a quarter of your own or larger.\n");
        } else if (GET_HIT(i->character) >= GET_HIT(ch) * 0.5) {
          send_to_char(ch,
                       "a power that is about half of your own or larger.\n");
        } else if (GET_HIT(i->character) >= GET_HIT(ch) * 0.25) {
          send_to_char(
              ch, "a power that is about a quarter of your own or larger.\n");
        } else if (GET_HIT(i->character) >= GET_HIT(ch) * 0.1) {
          send_to_char(
              ch, "a power that is about a tenth of your own or larger.\n");
        } else if (GET_HIT(i->character) >= GET_HIT(ch) * 0.01) {
          send_to_char(ch, "a power that is less than a tenth of your own.\n");
        } else if (GET_HIT(i->character) < GET_HIT(ch) * 0.01) {
          send_to_char(ch, "a power that is less than 1 percent of your own. "
                           "What a weakling...\n");
        }
        ...
        if (GET_ALIGNMENT(i->character) >= 500) {
          send_to_char(
              ch, "@wYou sense an extremely pure and good ki from them.@n\n");
          ...
          send_to_char(ch,
                       "@wYou sense slightly pure and good ki from them.@n\n");
        } else if (GET_ALIGNMENT(i->character) > -50) {
          send_to_char(
              ch, "@wYou sense a slightly mild indefinable ki from them.@n\n");
        } else if (GET_ALIGNMENT(i->character) > -200) {
          send_to_char(
              ch, "@wYou sense a slightly sour and evil ki from them.@n\n");
  ...
  if (GET_HIT(vict) < (GET_HIT(ch) * 0.001) + 1) {
    if (char_room_get(ch) == char_room_get(vict)) {
      if (!read_sense_memory(ch, vict)) {
        send_to_char(ch,
                     "Their powerlevel is too weak for you to sense properly, "
                     "but you will recognise their ki signal from now on.\r\n");
        sense_memory_write(ch, vict);
      } else {
        send_to_char(
            ch, "Their powerlevel is too weak for you to sense properly.\r\n");
      }
    } else {
      send_to_char(
          ch, "Their powerlevel is too weak for you to sense properly.\r\n");
  ...
  if ((GET_SKILL_BASE(ch, SKILL_SENSE) == 100) &&
      (!(room_flagged(room, ROOM_YARDRAT)) &&
       (room_flagged(vict_room, ROOM_YARDRAT)))) {
    ...
    if (vict = get_char_vis(ch, arg, NULL, FIND_CHAR_WORLD)) {
      ...
      free(blah);
             ...
             (!(room_flagged(room, ROOM_EARTH)) &&
              (room_flagged(vict_room, ROOM_EARTH)))) {
    ...
    if (vict = get_char_vis(ch, arg, NULL, FIND_CHAR_WORLD)) {
      ...
      free(blah);
             ...
             (!(room_flagged(room, ROOM_VEGETA)) &&
              (room_flagged(vict_room, ROOM_VEGETA)))) {
    ...
    if (vict = get_char_vis(ch, arg, NULL, FIND_CHAR_WORLD)) {
      ...
      free(blah);
             ...
             (!(room_flagged(room, ROOM_NAMEK)) &&
              (room_flagged(vict_room, ROOM_NAMEK)))) {
    ...
    if (vict = get_char_vis(ch, arg, NULL, FIND_CHAR_WORLD)) {
      ...
      free(blah);
             ...
             (!(room_flagged(room, ROOM_FRIGID)) &&
              (room_flagged(vict_room, ROOM_FRIGID)))) {
    ...
    if (vict = get_char_vis(ch, arg, NULL, FIND_CHAR_WORLD)) {
      ...
      free(blah);
             ...
             (!(room_flagged(room, ROOM_AETHER)) &&
              (room_flagged(vict_room, ROOM_AETHER)))) {
    ...
    if (vict = get_char_vis(ch, arg, NULL, FIND_CHAR_WORLD)) {
      ...
      free(blah);
             ...
             (!(room_flagged(room, ROOM_KONACK)) &&
              (room_flagged(vict_room, ROOM_KONACK)))) {
    ...
    if (vict = get_char_vis(ch, arg, NULL, FIND_CHAR_WORLD)) {
      ...
      free(blah);
             ...
             (!(room_flagged(room, ROOM_KANASSA)) &&
              (room_flagged(vict_room, ROOM_KANASSA)))) {
    ...
    if (vict = get_char_vis(ch, arg, NULL, FIND_CHAR_WORLD)) {
      ...
      free(blah);
             ...
             (!(room_flagged(room, ROOM_ARLIA)) &&
              (room_flagged(vict_room, ROOM_ARLIA)))) {
    ...
    if (vict = get_char_vis(ch, arg, NULL, FIND_CHAR_WORLD)) {
      ...
      free(blah);
             ...
             (!(char_planet_zenith(ch))) && (char_planet_zenith(vict))) {
    ...
    if (GET_SKILL(ch, SKILL_SENSE) < rand_number(1, 101)) {
      ...
      send_to_char(
          ch, "You sense them %s faintly from here, but are unsure....\r\n",
          dirs[dir]);
    ...
    case BFS_ERROR:
      ...
      break;
    case BFS_ALREADY_THERE:
      act("You look at $N@n intently for a moment.", TRUE, ch, 0, vict,
          TO_CHAR);
      act("$n looks at you intently for a moment.", TRUE, ch, 0, vict, TO_VICT);
      act("$n looks at $N intently for a moment.", TRUE, ch, 0, vict,
          TO_NOTVICT);
      if (!IS_ANDROID(vict)) {
        if (GET_ALIGNMENT(vict) > 50 && GET_ALIGNMENT(vict) < 200) {
          send_to_char(ch,
                       "You sense slightly pure and good ki from them.\r\n");
        } else if (GET_ALIGNMENT(vict) > 200 && GET_ALIGNMENT(vict) < 500) {
          send_to_char(ch, "You sense a pure and good ki from them.\r\n");
        } else if (GET_ALIGNMENT(vict) >= 500) {
          send_to_char(
              ch, "You sense an extremely pure and good ki from them.\r\n");
        } else if (GET_ALIGNMENT(vict) < -50 && GET_ALIGNMENT(vict) > -200) {
          send_to_char(ch,
                       "You sense slightly sour and evil ki from them.\r\n");
        } else if (GET_ALIGNMENT(vict) < -200 && GET_ALIGNMENT(vict) > -500) {
          send_to_char(ch, "You sense a sour and evil ki from them.\r\n");
        } else if (GET_ALIGNMENT(vict) <= -500) {
          send_to_char(ch, "You sense an extremely evil ki from them.\r\n");
      ...
      if (!IS_ANDROID(vict)) {
        if (GET_HIT(vict) > GET_HIT(ch) * 50) {
          ...
          send_to_char(ch,
                       "Their power is a great deal larger than your own.\n");
        } else if (GET_HIT(vict) > GET_HIT(ch) * 2) {
          send_to_char(
              ch, "Their power is more than twice as large as your own.\n");
        } else if (GET_HIT(vict) > GET_HIT(ch)) {
          send_to_char(ch,
                       "Their power is about twice as large as your own.\n");
        } else if (GET_HIT(vict) == GET_HIT(ch)) {
          send_to_char(ch, "Their power is exactly as strong as you.\n");
        } else if (GET_HIT(vict) >= GET_HIT(ch) * 0.75) {
          send_to_char(
              ch, "Their power is about a quarter of your own or larger.\n");
        } else if (GET_HIT(vict) >= GET_HIT(ch) * 0.5) {
          send_to_char(ch,
                       "Their power is about half of your own or larger.\n");
        } else if (GET_HIT(vict) >= GET_HIT(ch) * 0.25) {
          send_to_char(
              ch, "Their power is about a quarter of your own or larger.\n");
        } else if (GET_HIT(vict) >= GET_HIT(ch) * 0.1) {
          ...
          send_to_char(ch, "Their power is less than 1 percent of your own. "
                           "What a weakling...\n");
        }
        if (!read_sense_memory(ch, vict)) {
          send_to_char(ch,
                       "You will remember their ki signal from now on.\r\n");
          sense_memory_write(ch, vict);
        }
      } else {
        send_to_char(
