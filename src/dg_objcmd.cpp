...
OCMD(do_dupe) {
    ...
    obj->item_flags.set(ITEM_DUPLICATE, true);
}
...
OCMD(do_dgoload) {
    ...
    if (is_abbrev(arg1, "mob")) {
        ...
        if (SCRIPT(obj)) { /* It _should_ have, but it might be detached. */
            add_var(&(SCRIPT(obj)->global_vars), "lastloaded", mob->getUID(true).c_str(), 0);
        }
        ...
        if (SCRIPT(obj)) { /* It _should_ have, but it might be detached. */
            add_var(&(SCRIPT(obj)->global_vars), "lastloaded", object->getUID(true).c_str(), 0);
        }
