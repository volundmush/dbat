  ...
  *name = '\0';
  ...
  while (message) {
    ...
    *(tmstr + strlen(tmstr) - 1) = '\0';
    ...
    if (msgcount < 1) {
    sprintf(buf, "@D[%s] (@C%2d@D) : @W%6.10s @D(@G%-10s@D) ::@w %-45s\r\n", yesno ? "@GX@D" : "@RX@D", ++msgcount, tmstr, CAP(name), MESG_SUBJECT(message) ? MESG_SUBJECT(message) : "No Subject");
    }
    else {
