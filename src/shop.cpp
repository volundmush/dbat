...
/* local functions */
static char *read_shop_message(int mnum, room_vnum shr, FILE *shop_f,
                               const char *why);
...
static void shopping_sell(char *arg, struct char_data *ch,
                          struct char_data *keeper, struct shop_data *shop);
static struct obj_data *get_selling_obj(struct char_data *ch, char *name,
...
static void shopping_buy(char *arg, struct char_data *ch,
                         struct char_data *keeper, struct shop_data *shop);
static void shopping_app(char *arg, struct char_data *ch,
                         struct char_data *keeper, struct shop_data *shop);
...
static int is_ok_char(struct char_data *keeper, struct char_data *ch,
                      struct shop_data *shop);
static int is_ok_obj(struct char_data *keeper, struct char_data *ch,
                     struct obj_data *obj, struct shop_data *shop);
static int is_open(struct char_data *keeper, struct shop_data *shop, int msg);
static int is_ok(struct char_data *keeper, struct char_data *ch,
                 struct shop_data *shop, struct obj_data *obj, int msg);
...
static int buy_price(struct obj_data *obj, struct shop_data *shop,
                     struct char_data *keeper, struct char_data *buyer);
static int sell_price(struct obj_data *obj, struct shop_data *shop,
                      struct char_data *keeper, struct char_data *seller);
static char *list_object(struct obj_data *obj, int cnt, int oindex,
                         struct shop_data *shop, struct char_data *keeper,
                         struct char_data *seller);

static int add_to_list(struct shop_buy_data *list, int type, int *len,
                       int *val);
...
const char *shop_bits[] = {"WILL_FIGHT", "USES_BANK", "ALLOW_STEAL", "\n"};
...
static int is_ok_char(struct char_data *keeper, struct char_data *ch,
  ...
  if ((GET_ALIGNMENT(ch) > 0 && NOTRADE_GOOD(shop)) ||
      (GET_ALIGNMENT(ch) < 0 && NOTRADE_EVIL(shop)) ||
      (GET_ALIGNMENT(ch) == 0 && NOTRADE_NEUTRAL(shop))) {
  ...
  if ((IS_ROSHI(ch) && NOTRADE_WIZARD(shop)) ||
      (IS_PICCOLO(ch) && NOTRADE_CLERIC(shop)) ||
      (IS_KRANE(ch) && NOTRADE_ROGUE(shop)) ||
      (IS_BARDOCK(ch) && NOTRADE_MONK(shop)) ||
      (IS_GINYU(ch) && NOTRADE_PALADIN(shop)) ||
      (IS_NAIL(ch) && NOTRADE_FIGHTER(shop)) ||
      (IS_KABITO(ch) && NOTRADE_BARBARIAN(shop)) ||
      (IS_FRIEZA(ch) && NOTRADE_SORCERER(shop)) ||
      (IS_ANDSIX(ch) && NOTRADE_BARD(shop)) ||
      (IS_DABURA(ch) && NOTRADE_RANGER(shop)) ||
      (IS_TAPION(ch) && NOTRADE_DRUID(shop)) ||
      (IS_NAIL(ch) && NOTRADE_FIGHTER(shop)) ||
  ...
  if ((IS_HUMAN(ch) && NOTRADE_HUMAN(shop)) ||
      (IS_ICER(ch) && NOTRADE_ICER(shop)) ||
      (IS_SAIYAN(ch) && NOTRADE_SAIYAN(shop)) ||
      (IS_KONATSU(ch) && NOTRADE_KONATSU(shop))) {
...
static int is_ok_obj(struct char_data *keeper, struct char_data *ch,
  ...
  if (OBJ_FLAGGED(obj, ITEM_BROKEN) && NOTRADE_BROKEN(shop)) {
    snprintf(buf, sizeof(buf), "%s %s", GET_NAME(ch), MSG_NO_BUY_BROKEN);
    do_tell(keeper, buf, cmd_tell, 0);
    return (FALSE);
  }
  if (OBJ_FLAGGED(obj, ITEM_FORGED)) {
    snprintf(buf, sizeof(buf), "%s that piece of junk is an obvious forgery!",
             GET_NAME(ch));
    do_tell(keeper, buf, cmd_tell, 0);
    return (FALSE);
  }
...
static void evaluate_operation(struct stack_data *ops,
    ...
    if (isspace(*ptr))
      ...
      if ((temp = find_oper_num(*ptr)) == NOTHING) {
        ...
        while (*ptr && !isspace(*ptr) && find_oper_num(*ptr) == NOTHING)
          ptr++;
        strncpy(name, end,
                ptr - end); /* strncpy: OK (name/end:MAX_STRING_LENGTH) */
        name[ptr - end] = '\0';
...
int shop_producing(struct obj_data *item, struct shop_data *shop) {
  ...
  for (counter = 0; SHOP_PRODUCT(shop, counter) != NOTHING; counter++)
    if (same_obj(item, obj_proto_by_id(SHOP_PRODUCT(shop, counter))))
      return (TRUE);
  return (FALSE);
}
  ...
  do {
    ...
    if (!obj) {
      if (msg) {
        ...
        snprintf(buf, sizeof(buf), shop->no_such_item1, GET_NAME(ch));
        do_tell(keeper, buf, cmd_tell, 0);
      }
      return (NULL);
    }
...
static int sell_price(struct obj_data *obj, struct shop_data *shop,
                      struct char_data *keeper, struct char_data *seller) {
  float sell_cost_modifier = shop->profit_sell;
  float buy_cost_modifier = shop->profit_buy;
...
static void shopping_app(char *arg, struct char_data *ch,
  ...
  if (!*arg) {
    ...
    snprintf(buf, sizeof(buf), "%s What do you want to appraise?",
             GET_NAME(ch));
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  ...
  act("@C$N@W hands you @G$p@W for a moment and let's you examine it before "
      "taking it back.@n",
      TRUE, ch, obj, keeper, TO_CHAR);
  act("@c$N@W hands @C$n@W @G$p@W for a moment and let's $m examine it before "
      "taking it back.@n",
  ...
  if (GET_SKILL(ch, SKILL_APPRAISE) < rand_number(1, 101)) {
    send_to_char(ch,
                 "@wYou were completely stumped about the worth of %s@n\r\n",
...
static void shopping_buy(char *arg, struct char_data *ch,
  ...
  if ((buynum = transaction_amt(arg)) < 0) {
    ...
    snprintf(buf, sizeof(buf),
             "%s A negative amount?  Try selling me something.", GET_NAME(ch));
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  if (!*arg || !buynum) {
  ...
  if (buy_price(obj, shop, keeper, ch) > GET_GOLD(ch) &&
      !ADM_FLAGGED(ch, ADM_MONEY)) {
    char actbuf[MAX_INPUT_LENGTH];

    snprintf(actbuf, sizeof(actbuf), shop->missing_cash2, GET_NAME(ch));
  ...
  while (obj &&
         (GET_GOLD(ch) >= buy_price(obj, shop, keeper, ch) ||
          ADM_FLAGGED(ch, ADM_MONEY)) &&
    ...
    charged = buy_price(obj, shop, keeper, ch);
    goldamt += charged;
    if (!ADM_FLAGGED(ch, ADM_MONEY))
      char_stat_mod(ch, "money", -charged);
    else {
      send_to_imm("IMM PURCHASE: %s has purchased %s for free.", GET_NAME(ch),
                  obj->short_description);
      log_imm_action("IMM PURCHASE: %s has purchased %s for free.",
  ...
  if (bought < buynum) {
    ...
    if (!obj || !same_obj(last_obj, obj))
      snprintf(buf, sizeof(buf), "%s I only have %d to sell you.", GET_NAME(ch),
               bought);
    else if (GET_GOLD(ch) < buy_price(obj, shop, keeper, ch))
      snprintf(buf, sizeof(buf), "%s You can only afford %d.", GET_NAME(ch),
               bought);
    else if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch))
      snprintf(buf, sizeof(buf), "%s You can only hold %d.", GET_NAME(ch),
               bought);
    else if (IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj) > CAN_CARRY_W(ch))
      snprintf(buf, sizeof(buf), "%s You can only carry %d.", GET_NAME(ch),
               bought);
    else
      snprintf(buf, sizeof(buf), "%s Something screwy only gave you %d.",
               GET_NAME(ch), bought);
    do_tell(keeper, buf, cmd_tell, 0);
  }
  if (!ADM_FLAGGED(ch, ADM_MONEY))
  ...
  snprintf(tempbuf, sizeof(tempbuf), shop->message_buy, GET_NAME(ch), goldamt);
  ...
  if (SHOP_USES_BANK(shop))
    if (GET_GOLD(keeper) > MAX_OUTSIDE_BANK) {
      SHOP_BANK(shop) += (GET_GOLD(keeper) - MAX_OUTSIDE_BANK);
      char_stat_set(keeper, "money", MAX_OUTSIDE_BANK);
    }
}
...
static struct obj_data *get_selling_obj(struct char_data *ch, char *name,
  ...
  if (!(obj = get_obj_in_list_vis(ch, name, NULL, inv_for_char(ch)))) {
    if (msg) {
      ...
      snprintf(tbuf, sizeof(tbuf), shop->no_such_item2, GET_NAME(ch));
  ...
  case OBJECT_NOVAL:
    snprintf(buf, sizeof(buf),
             "%s You've got to be kidding, that thing is worthless!",
             GET_NAME(ch));
    break;
  case OBJECT_NOTOK:
    snprintf(buf, sizeof(buf), shop->do_not_buy, GET_NAME(ch));
    break;
  case OBJECT_DEAD:
...
static void shopping_sell(char *arg, struct char_data *ch,
                          struct char_data *keeper, struct shop_data *shop) {
  char tempstr[MAX_INPUT_LENGTH], name[MAX_INPUT_LENGTH],
      tempbuf[MAX_INPUT_LENGTH];
  struct obj_data *obj;
  ...
  if ((sellnum = transaction_amt(arg)) < 0) {
    ...
    snprintf(buf, sizeof(buf), "%s A negative amount?  Try buying something.",
             GET_NAME(ch));
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  ...
  if (GET_OBJ_TYPE(obj) == ITEM_PLANT &&
      GET_OBJ_VAL(obj, VAL_WATERLEVEL) <= -10) {
    char buf[MAX_INPUT_LENGTH];
  ...
  if (!(is_ok_obj(keeper, ch, obj, shop)))
    return;

  if (GET_GOLD(keeper) + shop->bankAccount <
      sell_price(obj, shop, keeper, ch)) {
    char buf[MAX_INPUT_LENGTH];

    snprintf(buf, sizeof(buf), shop->missing_cash1, GET_NAME(ch));
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  while (obj &&
         GET_GOLD(keeper) + shop->bankAccount >=
             sell_price(obj, shop, keeper, ch) &&
         sold < sellnum) {
    int charged = sell_price(obj, shop, keeper, ch);

    goldamt += charged;
    char_stat_mod(keeper, "money", -charged);

    sold++;
    obj_from_char(obj);
    slide_obj(obj, keeper, shop); /* Seems we don't use return value. */
  ...
  if (sold < sellnum) {
    ...
    if (!obj)
      snprintf(buf, sizeof(buf), "%s You only have %d of those.", GET_NAME(ch),
               sold);
    else if (GET_GOLD(keeper) + shop->bankAccount <
             sell_price(obj, shop, keeper, ch))
      snprintf(buf, sizeof(buf), "%s I can only afford to buy %d of those.",
               GET_NAME(ch), sold);
    else
      snprintf(buf, sizeof(buf), "%s Something really screwy made me buy %d.",
               GET_NAME(ch), sold);
  ...
  snprintf(tempbuf, sizeof(tempbuf), shop->message_sell, GET_NAME(ch), goldamt);
  do_tell(keeper, tempbuf, cmd_tell, 0);
  ...
  if (GET_GOLD(ch) + goldamt > GOLD_CARRY(ch)) {
    ...
    char_stat_mod(ch, "money_bank", goldamt);
    send_to_char(ch, "You couldn't hold all of the money. The rest was "
                     "deposited for you.\r\n");
  } else {
    char_stat_mod(ch, "money", goldamt);
  }

  if (GET_GOLD(keeper) < MIN_OUTSIDE_BANK) {
    goldamt = MIN(MAX_OUTSIDE_BANK - GET_GOLD(keeper), shop->bankAccount);
    shop->bankAccount -= goldamt;
    char_stat_mod(keeper, "money", goldamt);
  }
}
...
static void shopping_value(char *arg, struct char_data *ch,
  ...
  if (!*arg) {
    snprintf(buf, sizeof(buf), "%s What do you want me to evaluate??",
             GET_NAME(ch));
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  ...
  snprintf(buf, sizeof(buf), "%s I'll give you %d zenni for that!",
           GET_NAME(ch), sell_price(obj, shop, keeper, ch));
  do_tell(keeper, buf, cmd_tell, 0);
}

static char *list_object(struct obj_data *obj, int cnt, int aindex,
                         struct shop_data *shop, struct char_data *keeper,
                         struct char_data *ch) {
  ...
  if (shop_producing(obj, shop))
    ...
    sprintf(quantity, "%d",
            cnt); /* sprintf: OK (for 'quantity >= 11', 32-bit int) */
  ...
  snprintf(result, sizeof(result), " %2d)  %9s %-*s %3d %13s\r\n", aindex,
           quantity, count_color_chars(itemname) + 36, itemname, displevel,
           add_commas(buy_price(obj, shop, keeper, ch)));
  return (result);
}
...
int ok_shop_room(struct shop_data *shop, room_vnum room) {
  ...
  for (mindex = 0; SHOP_ROOM(shop, mindex) != NOWHERE; mindex++)
    if (SHOP_ROOM(shop, mindex) == room)
      return (TRUE);
  return (FALSE);
}
...
SPECIAL(shop_keeper) {
  ...
  shop_iterate([&](auto s) {
    if (SHOP_KEEPER(s) == GET_MOB_VNUM(keeper)) {
      shop = s;
      return false;
    }
    return true;
  ...
  if (SHOP_FUNC(shop)) /* Check secondary function */
    if ((SHOP_FUNC(shop))(ch, me, cmd, argument))
      return (TRUE);
  ...
  if (!ok_shop_room(shop, char_room_vnum_get(ch)))
    return (0);
  ...
  if (CMD_IS("steal")) {
    ...
    if (!SHOP_ALLOW_STEAL(shop)) {
      ...
      act(argm, FALSE, ch, 0, keeper, TO_ROOM);
  ...
  if (CMD_IS("buy")) {
    ...
    shopping_app(argument, ch, keeper, shop);
    return (TRUE);
  }
  return (FALSE);
}
...
int ok_damage_shopkeeper(struct char_data *ch, struct char_data *victim) {
  if (!IS_MOB(victim) ||
      mob_proto_special_get(GET_MOB_VNUM(victim)) != shop_keeper)
    return (TRUE);
  ...
  bool protected_keeper = false;
  shop_iterate([&](auto shop) {
    if (GET_MOB_VNUM(victim) == SHOP_KEEPER(shop) && !SHOP_KILL_CHARS(shop)) {
...
static int add_to_list(struct shop_buy_data *list, int type, int *len,
                       int *val) {
  if (*val != NOTHING &&
      *val >= 0) { /* necessary after changing to unsigned v/rnums -- Welcor */
    if (*len < MAX_SHOP_OBJ) {
      ...
      for (tindex = 0; *item_types[tindex] != '\n'; tindex++)
        if (!strncasecmp(item_types[tindex], buf, strlen(item_types[tindex]))) {
          ...
          strcpy(
              buf,
              buf +
                  strlen(item_types[tindex])); /* strcpy: OK (always smaller) */
          break;
        }
  ...
  while (!done) {
    ...
    if (*buf == '#') { /* New shop */
      ...
      for (count = 0; count < temp; count++)
        shop->producing[count] = BUY_TYPE(list[count]);

      read_line(shop_f, "%f", &shop->profit_buy, shop);
      read_line(shop_f, "%f", &shop->profit_sell, shop);

      temp = read_type_list(shop_f, list, new_format, MAX_TRADE, shop);
      CREATE(shop->type, struct shop_buy_data, temp);
      for (count = 0; count < temp; count++) {
        SHOP_BUYTYPE(shop, count) = BUY_TYPE(list[count]);
        SHOP_BUYWORD(shop, count) = BUY_WORD(list[count]);
      }

      shop->no_such_item1 = read_shop_message(0, SHOP_NUM(shop), shop_f, buf2);
      shop->no_such_item2 = read_shop_message(1, SHOP_NUM(shop), shop_f, buf2);
      shop->do_not_buy = read_shop_message(2, SHOP_NUM(shop), shop_f, buf2);
      shop->missing_cash1 = read_shop_message(3, SHOP_NUM(shop), shop_f, buf2);
      shop->missing_cash2 = read_shop_message(4, SHOP_NUM(shop), shop_f, buf2);
      shop->message_buy = read_shop_message(5, SHOP_NUM(shop), shop_f, buf2);
      shop->message_sell = read_shop_message(6, SHOP_NUM(shop), shop_f, buf2);
      read_line(shop_f, "%d", &shop->temper1, shop);
      read_line(shop_f, "%ld", &shop->bitvector, shop);
      read_line(shop_f, "%hd", &shop->keeper, shop);
      ...
      while (temp < SW_ARRAY_MAX)
        shop->with_who[temp++] = 0;

      temp = read_list(shop_f, list, new_format, 1, LIST_ROOM, shop);
      CREATE(shop->in_room, room_vnum, temp);
      for (count = 0; count < temp; count++)
        shop->in_room[count] = BUY_TYPE(list[count]);

      read_line(shop_f, "%d", &shop->open1, shop);
      read_line(shop_f, "%d", &shop->close1, shop);
      read_line(shop_f, "%d", &shop->open2, shop);
      read_line(shop_f, "%d", &shop->close2, shop);
...
void assign_the_shopkeepers(void) {
  ...
  shop_iterate([&](auto shop) {
    auto proto = mob_proto_by_id(SHOP_KEEPER(shop));
    if (!proto)
      return true;
...
static char *customer_string(struct shop_data *shop, int detailed) {
  ...
  while (*trade_letters[sindex] != '\n' && len + 1 < sizeof(buf)) {
    if (detailed) {
      if (!IS_SET_AR(shop->with_who, flag)) {
        nlen = snprintf(buf + len, sizeof(buf) - len, ", %s",
                        trade_letters[sindex]);
      ...
      buf[len++] =
          (IS_SET_AR(shop->with_who, flag) ? '_' : *trade_letters[sindex]);
      buf[len] = '\0';
  ...
  shop_iterate([&](auto shop) {
    ...
    if (!(shop_nr % (PAGE_LENGTH - 2))) {
      ...
      if (len + headerlen + 1 >= sizeof(buf))
        return false;
      strcpy(buf + len,
             list_all_shops_header); /* strcpy: OK (length checked above) */
      len += headerlen;
    }
...
static void list_detailed_shop(struct char_data *ch, struct shop_data *shop) {
  ...
  column = 12; /* ^^^ strlen ^^^ */
  for (sindex = 0; shop->in_room[sindex] != NOWHERE; sindex++) {
    char buf1[128];
    int linelen, temp;
    auto in_room = shop->in_room[sindex];
    struct room_data *room = room_by_id(in_room);
  ...
  send_to_char(ch, "Customers:  %s\r\n",
               (ptrsave = customer_string(shop, TRUE)) ? ptrsave : "None");

  send_to_char(ch, "Produces:   ");
  column = 12; /* ^^^ strlen ^^^ */
  for (sindex = 0; shop->producing[sindex] != NOTHING; sindex++) {
    char buf1[128];
    int linelen;
  ...
  send_to_char(ch, "\r\nBuys:       ");
  column = 12; /* ^^^ strlen ^^^ */
  for (sindex = 0; SHOP_BUYTYPE(shop, sindex) != NOTHING; sindex++) {
    char buf1[128];
    size_t linelen;
    ...
    linelen = snprintf(
        buf1, sizeof(buf1), "%s (#%d) [%s]",
        item_types[SHOP_BUYTYPE(shop, sindex)], SHOP_BUYTYPE(shop, sindex),
        SHOP_BUYWORD(shop, sindex) ? SHOP_BUYWORD(shop, sindex) : "all");
  ...
  {
    char buf1[128];
    sprintbit(SHOP_BITVECTOR(shop), shop_bits, buf1, sizeof(buf1));
...
void show_shops(struct char_data *ch, char *arg) {
  ...
  if (!*arg)
    ...
    if (!strcasecmp(arg, ".")) {
      shop_iterate([&](auto s) {
        if (ok_shop_room(s, char_room_vnum_get(ch))) {
          shop = s;
          return false;
        }
        return true;
