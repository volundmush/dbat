...
static int do_simple_enter(BaseCharacter *ch, Object *obj, int need_specials_check)
{
    ...
    if (ROOM_FLAGGED(dest_room, ROOM_TUNNEL) &&
        num_pc_in_room(getEntity<Room>(dest_room)) >= CONFIG_TUNNEL_SIZE)
    {
    ...
    ch->removeFromLocation();
    ch->addToLocation(getEntity(dest_room));
    ...
    {
        ch->removeFromLocation();
        ch->addToLocation(getEntity(was_in));
        return 0;
    }
...
static int check_swim(BaseCharacter *ch)
{
    ...
    if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_SPACE))
    {
        ...
        if (!can)
            ch->sendf("You do not have enough ki to fly through space. You are drifting helplessly.\r\n");
        return can;
    }
    else
    {
        ...
        if (!can)
            ch->sendf("You are too tired to swim!\r\n");
        return can;
    }
}
