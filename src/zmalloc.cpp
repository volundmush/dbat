...
void zfree2(unsigned char *what, char *file, int line);
...
unsigned char *zmalloc(int len, char *file, int line)
{
  ...
  if (zmalloclogging > 2)
    fprintf(zfd,"zmalloc: 0x%4p  %d bytes %s:%d\n",ret,len,file,line);
...
void zfree2(unsigned char *what, char *file, int line)
{
  ...
  if (!what) {
    fprintf(zfd,"zfree2: ERR: Null pointer free'd: %s:%d.\n", file, line);
    return;
  }
  ...
  for (m = memlist[GET_ZBUCKET(what)]; m; m = m->next) {
    if (m->addr == what) {
      ...
      if (zmalloclogging > 2) {
	fprintf(zfd,"zfree2: Freed 0x%p %d bytes mallocd at %s:%d, freed at %s:%d\n",
      ...
      if (m->frees > 1) {
        fprintf(zfd,"zfree2: ERR: multiple frees! 0x%p %d bytes\n"
  ...
  if (!gotit) {
    fprintf(zfd,"zfree2: ERR: attempt to free unallocated memory 0x%p at %s:%d.\n",
		what, file, line);
  }
  if (gotit > 1) {
    /* this shouldn't happen, eh? */
    fprintf(zfd,"zfree2: ERR: Multiply-allocd memory 0x%p.\n", what);
  }
}
