...
if eval "test \"`echo '$ac_cv_prototype_'$ac_safe`\" = yes";

then
AC_MSG_RESULT(yes)

else
AC_DEFINE(builtin(format, NEED_ % s_PROTO, translit($1, 'a-z', 'A-Z')),
, Check for
a prototype
to $1
.)
...
dnl @@@t1="MAKE_PROTO_SAFE($1)";
t2 = "MAKE_PROTO_NAME($t1)";
literals = "$literals $t2"
@@@])
...
whether crypt
needs over
10 characters], ac_cv_unsafe_crypt, [
if
test ${ac_cv_header_crypt_h - no}
=
yes;
then
