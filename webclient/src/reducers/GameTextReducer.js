...
function circleToAnsi(entry, colors = null) {
  const customColors = {...DEFAULT_COLORS, ...(colors || {})};

  function replaceColor(match, code) {
      if (code === "@") {
          return "@";
      } else if (code === "x") {
...
export default function GameTextReducer(state = initialState, action) {
    switch (action.type) {
      case 'Game.Text':
        if(action.payload.data?.length === 0) {
          return state;
        }
        let ansi = circleToAnsi(action.payload.data);
        let html = RavensGleaning.html(ansi);

        return {
          ...state,
          events: [...state.events, html],
        };
      default:
        return state;
    }
